...
Manifest Admission 仍会验证 Distribution Identity 与 RinCode Version Compatibility；Stable Discovery List
...
from rincode import __version__ as _installed_rincode_version
from rincode.plugin.manifest import PluginManifest

logger = logging.getLogger(__name__)


_MANIFEST_FILENAME = "rincode-plugin.toml"


class PluginCompatibilityError(RuntimeError):
    """Plugin Manifest 排除了 Installed RinCode Version 时抛出。
...
class PluginDiscovery:
    ...
    实例还持有待验证的 RinCode Version，所有 Scan 共享同一兼容标准。
    """
    ...
    def __init__(
        ...
        *,
        ...
        rincode_version: str = _installed_rincode_version,
    ) -> None:
        self._bundled_dir = bundled_dir
        self._user_dir = user_dir
        self._project_dir = project_dir
        self._entry_points_group = entry_points_group
        self._rincode_version = rincode_version
    ...
    def _scan_dir(
        ...
        """在 ``<root>/<plugin_id>/rincode-plugin.toml`` 查找 File-based Plugins。

        Subdir Name 只是 Informational，Canonical Plugin ID 来自 Manifest。两者 Mismatch 会记录日志但仍
        加载；解析失败则 Warning + Skip。每个成功 Manifest 都立即验证 RinCode Compatibility，返回的
    ...
    def _scan_entry_points(self, group: str) -> list[DiscoveredPlugin]:
        ...
        Entry-point Value 指出拥有 ``rincode-plugin.toml`` 的 Package。Discovery 通过 Owning Distribution 的
        Installed File Inventory 定位 Manifest，**不 Import Plugin Package**。每项还验证 Distribution
        Name/Version 与 Manifest Identity、以及 RinCode Compatibility；Identity/Compatibility Error 向上
    ...
    def _validate_compatibility(self, manifest: PluginManifest) -> None:
        if not manifest.rincode:
            return
        try:
            compatible = Version(self._rincode_version) in SpecifierSet(manifest.rincode)
        except (InvalidSpecifier, InvalidVersion) as exc:
            raise PluginCompatibilityError(
                f"plugin {manifest.id!r} has invalid RinCode compatibility metadata; reinstall a compatible plugin"
            ) from exc
        if not compatible:
            raise PluginCompatibilityError(
                f"plugin {manifest.id!r} {manifest.version} requires RinCode {manifest.rincode}, but installed RinCode is "
                f"{self._rincode_version}; install a compatible plugin version or set memory.backend to null"
            )
