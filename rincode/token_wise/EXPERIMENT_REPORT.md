...
> routing, API names, and results are a dated snapshot, not a current RinCode
> release contract. Variant names record experiment provenance.

_Originally driven through the then-current ``AgentLoop.process_direct`` path.
The current Runtime entry is ``AgentLoop.run_turn``; these measurements have
not been rerun on that path._
...
- Turns per variant: 6
- Historical driver: ``AgentLoop.process_direct`` (removed)
