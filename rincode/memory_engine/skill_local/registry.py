...
``rincode > nanobot > openclaw``。当前优先级实际为 Workspace > 按配置顺序的 External/Extra Dirs > Builtin，
...
``rincode/memory_engine/skills/``；Builtin Markdown 与 Code 同在 ``memory_engine`` Package。
...
from rincode.memory_engine.skill_local.types import SkillMeta

# 默认内置 Skill 目录与旧版 ``SkillsLoader`` 使用的路径一致，使替换可直接完成。
#
# 解析为 ``rincode/memory_engine/skills/``；内置 Markdown 库与 Skill 代码一起位于 memory_engine 包内。
...
def _parse_nested_metadata(raw: str) -> dict:
    """从 ``metadata`` JSON Blob 提取 RinCode-compatible Namespaced Metadata。

    Lookup Order First Match Wins：``rincode`` > ``nanobot`` > ``openclaw``。Empty、Invalid JSON、Non-dict 或
    ...
    if not isinstance(data, dict):
        return {}
    for key in ("rincode", "nanobot", "openclaw"):
