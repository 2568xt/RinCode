...
Contract Test Classes 采用 Lazy Export，避免 Production ``import rincode.memory_engine`` 被仅开发环境的
...
from rincode.memory_engine.backend import Memory, MemoryBackend
from rincode.memory_engine.base import AssembledContext, TokenBudget

if TYPE_CHECKING:
    from rincode.memory_engine.contract_test import (
...
# 如果在此处急切导入，每次 ``import rincode.memory_engine`` 都会引入 pytest，导致未安装
# pytest 的生产环境（如打包后的 `rincode`）以 ``ModuleNotFoundError`` 失败。通过 PEP 562
# 延迟暴露，只在测试套件真正访问这些类时解析，此时 pytest 已可用。
def __getattr__(name: str):
    if name in ("LifecycleContractTests", "MemoryBackendContractTests"):
        from rincode.memory_engine import contract_test
