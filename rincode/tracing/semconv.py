"""RinCode Tracing Standard 的 Semantic Conventions。
...
Shape 读取 Runtime 对象，保持 Framework-agnostic；只要 Documented Shapes 不变，就能承受 RinCode Refactor。
...
def _provider_label(model: str | None, provider_class: str | None) -> str | None:
    ...
    RinCode 通过同一 ``LiteLLMProvider`` Class 访问多个 Gateways，Class Name 会隐藏真实 Backend。LiteLLM 在
...
def _llm_input_payload(
    ...
    RinCode 传入 ONE Flat ``messages`` List，包含 System + Prior Turns + Current。函数拆成 Three
...
def _skill_read_path(name: str, params: Any) -> str | None:
    ...
    这是 Discovery→Injection Follow-through：RinCode Summary Mode 告诉 Agent 用 ``read_file`` 读取 Skill Body，
...
def spine_turn_failed(
    ...
    *,
    ...
    ``start_dispatched`` 表示启动生命周期调用已交给 sink，决定 Terminal Event 是否为
    ``TurnFailed``；``runner_started`` 额外区分 runner 是否已经获得控制权。sink 是异步回调而非
    事务存储，所以前者为真并不证明消费者实际持久化了 ``TurnStarted``。
    """
    error_class = type(exc).__name__
    outcome = SPINE_PROVIDER_FAILED if error_class == "ProviderTurnError" else SPINE_ERROR
    attrs: dict[str, Any] = {
        "spine.outcome": outcome,
        "spine.terminal_event": "TurnFailed" if start_dispatched else "TurnStarted",
        "spine.error_class": error_class,
        "spine.start_dispatched": start_dispatched,
        "spine.runner_started": runner_started,
    }
...
def spine_turn_cancelled(*, start_dispatched: bool, runner_started: bool) -> dict[str, Any]:
    return {
