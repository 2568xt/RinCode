...
from rincode.session.manager import Session
from rincode.utils.helpers import ensure_dir, safe_filename
...
_EXPORT_SCHEMA = "rincode.session.export.v1"
...
def default_export_path(workspace: Path, key: str) -> Path:
    ...
    Path 是 ``<workspace>/exports/<safe key>.rincode-session.json``；Key 经 safe_filename，函数不创建
    Directory、不检查冲突，也不写 File。Caller 可传 Custom Dest 覆盖。
    """

    return Path(workspace) / "exports" / f"{safe_filename(key)}.rincode-session.json"
