...
from rincode.agent.context import ContextBuilder
from rincode.agent.loop.recovery import (
    POST_TOOL_NUDGE,
    RecoveryAction,
    RecoveryLimits,
...
from rincode.agent.tools.ask_user import AskUserTool
from rincode.agent.tools.base import ToolResult
from rincode.agent.tools.execution import ToolExecution, ToolExecutionContext, ToolInvocation
from rincode.agent.tools.file_search import FindTool, GrepTool
from rincode.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from rincode.agent.tools.message import MessageTool
from rincode.agent.tools.registry import ToolRegistry
from rincode.agent.tools.shell import ExecTool
from rincode.agent.tools.skill import SkillReadTool
from rincode.agent.tools.spawn import SpawnTool
from rincode.agent.tools.web import WebFetchTool, WebSearchTool
from rincode.call_efficiency.pricing import resolve_context_window
from rincode.memory_engine.base import TokenBudget
from rincode.memory_engine.consolidate.consolidator import MemoryConsolidator, MemoryStore
from rincode.providers.base import ErrorClassification, LLMProvider, LLMResponse, ToolCallRequest
from rincode.sandbox import SandboxConfig, SandboxExecutor, SandboxInitError, build_executor
from rincode.session.manager import Session, SessionManager
from rincode.spine.message import Media
from rincode.spine.turn import Origin
from rincode.tracing import semconv, trace
from rincode.utils.helpers import estimate_prompt_tokens
from rincode.utils.persisted_payload import sanitize_persisted_payload

# 刻意在 ``__init__`` 和 ``_assemble_context_messages`` 内延迟导入 ``rincode.context_engine``，以打破运行时
# 循环导入：``rincode.agent.__init__`` 急切加载 AgentLoop，而 ``rincode.context_engine.curator``
# 又从 ``rincode.agent.context`` 导入 ``ContextBuilder``。如果在此模块顶层导入，会重新进入
# 只初始化了一部分的包，并在 ``TurnContext`` 上抛出 ImportError。

if TYPE_CHECKING:
    from rincode.agent.hook import CompositeHook
    from rincode.agent.tools.base import Tool
    from rincode.call_efficiency import CallEfficiency
    ...
    from rincode.config.schema import ChannelsConfig, ExecToolConfig
    from rincode.context_engine import ContextEngine
    from rincode.context_engine.factory import ContextEngineFactory
    from rincode.memory_engine.backend import MemoryBackend
    from rincode.proactive_engine.schedulers.cron.service import CronService
    from rincode.routing.router import ModelRouter
    from rincode.sandbox.debug_server import SandboxDebugServer
    from rincode.spine.runner import Drain, Emit, TurnOutcome
    from rincode.spine.turn import TurnRequest
    from rincode.token_wise.base import UsageSnapshot
    from rincode.token_wise.registry import StrategyRegistry
...
class AgentLoop:
    ...
    def __init__(
        ...
        from rincode.agent.hook import CompositeHook
        from rincode.call_efficiency import CallEfficiency
        from rincode.config.schema import ExecToolConfig
        from rincode.token_wise.registry import StrategyRegistry
        ...
        #
        # 延迟导入 ``rincode.context_engine`` 的原因见模块顶部关于 ``rincode.agent.__init__`` 循环导入的说明。
        if context_config is None:
            from rincode.config.rincode import ContextConfig

            context_config = ContextConfig()
        if context_engine_factory is None:
            from rincode.context_engine import build_context_engine
        ...
        if runtime_config is None:
            from rincode.config.rincode import RuntimeConfig
        ...
        if self._checkpoint_active(runtime_config.checkpoint.policy, interactive):
            from rincode.agent.loop.checkpoint import CheckpointService
    ...
    def _register_default_tools(self) -> None:
        ...
        函数内延迟导入是为避开 `rincode.agent.__init__` 的循环导入边界。
        """
        ...
        if self.cron_service:
            # 延迟导入：CronTool 所在模块会导入 rincode.agent.tools.base，触发 rincode.agent.__init__，
            # 后者又导入当前循环模块。在函数作用域导入可打破循环，因为执行
            # _register_default_tools 时 loop.py 已完全加载。
            from rincode.proactive_engine.schedulers.cron.tool import CronTool
        ...
        if cfg is not None and cfg.enabled:
            from rincode.agent.tools.tool_search import (
    ...
    async def _assemble_context_messages(
        ...
        *,
        ...
        导入边界。
        """
        from rincode.context_engine import TurnContext  # 延迟导入，原因见模块说明
    ...
    async def _dispatch_backend_store(
        ...
        ``memory.maybe_consolidate`` 执行 RinCode compaction，本方法负责插件 Backend。传入的
    ...
    async def _start_debug_server(self) -> None:
        ...
        主请求仍可运行，同时避免稍后 `rincode sandbox` 只看到“套接字不存在”却没有原因。
        """
        ...
        if cfg.backend == "none":
            ...
            return
        try:
            from rincode.config.paths import get_data_dir
            from rincode.sandbox.debug_server import SandboxDebugServer
            ...
            # 用户已明确选择调试模式；此处静默失败会让用户在稍后看到 `rincode sandbox`
    ...
    async def _connect_mcp(self) -> None:
        ...
        try:
            ...
            from rincode.agent.tools.mcp import connect_mcp_servers
    ...
    async def _run_agent_loop(
        ...
        # 持久化和返回前移除临时的空响应恢复脚手架。RinCode 会用
    ...
    def replace_provider(self, provider: LLMProvider, *, model: str | None = None) -> None:
        from rincode.call_efficiency.provider import CallEfficiencyProvider
    ...
    async def _process_message(
        ...
        """
        from rincode.agent.hook import AgentHookContext
        ...
        if self.enable_personalization and origin is not Origin.SUBAGENT:
            ...
            from rincode.agent.personalizer import Personalizer
    ...
    async def run_turn(
        ...
        *,
        ...
        """
        from rincode.proactive_engine.schedulers.cron.tool import CronTool
        from rincode.spine.events import (
            MediaOut,
            Notice,
            NoticeKind,
            Reasoning,
        ...
        from rincode.spine.message import Media
        from rincode.spine.runner import TurnOutcome
