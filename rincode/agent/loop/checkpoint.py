...
from rincode.product import WORKSPACE_STATE_DIRNAME
...
_GIT_IDENT = (
    ...
    "user.name=RinCode",
    "-c",
    "user.email=checkpoint@rincode.local",
    "-c",
...
_DEFAULT_EXCLUDES = """\
# RinCode shadow-git default excludes (see checkpoint.py).
        ...
        try:
            if not (self._git_dir / "HEAD").exists():
                ...
                if rc != 0:
                    ...
                    return False
                # 在影子 Git 旁写入可发现性提示，让注意到 ``.rincode/`` 的用户无需搜索代码库即可识别它。
                # 该操作尽力而为，写入失败不影响主流程。
                try:
                    notice = self._git_dir.parent / "NOTICE.txt"
                    notice.write_text(
                        "This directory is created by RinCode's runtime "
                        ...
                        "in your RinCode config (typically "
                        "~/.rincode/config.json, or whichever file you "
                        "passed via --config).\n",
                        encoding="utf-8",
                    )
                except OSError:
