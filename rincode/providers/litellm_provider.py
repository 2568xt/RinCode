...
from rincode.providers.base import LLMProvider, LLMResponse, StreamDelta, ToolCallRequest
from rincode.providers.litellm_setup import import_litellm
from rincode.providers.registry import find_by_model, find_gateway

litellm = import_litellm()
acompletion = litellm.acompletion

# LiteLLM 的异步日志 worker（LoggingWorker）会把队列绑定到单一 event loop。
# RinCode 每个 Turn 使用新 loop（每次调用 asyncio.run），所以下一个 Turn 会重置
# 队列，所有待处理的 ``Logging.async_*_handler`` 协程都会在未 await 的情况下
# 被丢弃。Python 随后打印 ``coroutine ... was never awaited`` RuntimeWarning，
# 并污染 Ink TUI 渲染。被丢弃的是 LiteLLM 自身的成功/失败日志回调，RinCode 不依赖
# 它们。过滤范围只限于 LiteLLM 的 ``Logging`` handler；宽泛的
# ``coroutine '.*'`` 模式也会掩盖 RinCode 自身真正的未 await 错误。
...
# LiteLLM 而非 RinCode。这里显式覆盖默认值；用户提供的 extra_headers 优先级更高。
_OPENROUTER_ATTRIBUTION: dict[str, str] = {
    "X-Title": "RinCode Agent Harness",
    "X-OpenRouter-Title": "RinCode Agent Harness",
...
class LiteLLMProvider(LLMProvider):
    ...
    def _sanitize_messages(
        ...
        drop_reasoning_only_assistants: bool = False,
    ) -> list[dict[str, Any]]:
        ...
        对要求 Reasoning Replay 的 Provider，所有无 reasoning_content 的 Assistant 补空 String；这也覆盖
        没有 Tool Calls 的收尾 Assistant。
        所有 Assistant ``tool_calls[].id`` 与 Tool ``tool_call_id`` 通过同一 id_map 同步缩短，
        否则 Strict Provider 会拒绝断裂关联。

        ``drop_reasoning_only_assistants`` 用于不携带 tools 的 DeepSeek 请求：移除没有 content
        和 tool_calls 的 Assistant 占位消息，避免上游拒绝无有效消息体的历史；带 Tool Calls 的
        Assistant 始终保留。
        ...
        for clean in sanitized:
            if (
                drop_reasoning_only_assistants
    ...
    async def chat(
        ...
        kwargs: dict[str, Any] = {
            "model": model,
            "messages": self._sanitize_empty_content(
                self._sanitize_messages(
                    messages,
                    extra_keys=extra_msg_keys,
                    require_reasoning_content_replay=require_reasoning_content_replay,
                    drop_reasoning_only_assistants=require_reasoning_content_replay and not tools,
                )
            ),
        ...
        if reasoning_effort:
            if require_reasoning_content_replay and "deepseek-v4" in model and reasoning_effort != "none":
                # Older LiteLLM maps effort to thinking.enabled and discards its level.
                # extra_body reaches DeepSeek unchanged and preserves explicit thinking settings.
                kwargs["extra_body"] = {**kwargs.get("extra_body", {}), "reasoning_effort": reasoning_effort}
            else:
    ...
    def _parse_response(self, response: Any) -> LLMResponse:
        """把 LiteLLM Completion Response 解析为 RinCode Standard LLMResponse。
