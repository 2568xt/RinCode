...
This module bundles all ``rincode channels ...`` subcommands:
...
:mod:`rincode.config.update_channels`. Command bodies do not import
...
def _warn_empty_credentials(name: str) -> None:
    ...
    """
    from rincode.config.update_channels import (
        channel_field_specs,
        get_channel_config,
    )
...
def _parse_channel_flags(extra_args: list[str], channel_name: str) -> dict:
    ...
    :func:`rincode.config.update_channels.channel_field_specs`. Unknown
    ...
    while i < len(extra_args):
        ...
        if flag.startswith("no-") and value is None:
            ...
            if key not in specs:
                raise typer.BadParameter(
                    f"Unknown field '--no-{flag[3:]}'. Run 'rincode channels show {channel_name}' for available flags."
                )
            out[key] = False
            continue
        ...
        if key not in specs:
            raise typer.BadParameter(
                ...
                f"Run 'rincode channels show {channel_name}' for available flags."
            )
...
def _register_config_commands(channels_app: typer.Typer) -> None:
    ...
    Call once at module-init time from ``rincode/cli/commands.py`` after
    ...
    """
    # `rincode channels`（无子命令）应打印帮助，而不是 "Missing command"。
    ...
    if "channels list" not in base_help:
        channels_app.info.help = f"{base_help}\n\nRun 'rincode channels list' to see available channels."
    ...
    def channels_enable_cmd(
        ...
        Examples:
            rincode channels enable feishu --app-id X --app-secret Y
            rincode channels enable qq --app-id X --secret Y
        """
    ...
    def channels_set_cmd(
        ...
        Examples:
            rincode channels set feishu --group-policy open
            rincode channels set wecom --welcome-message "Hello"
        """
        if _help_requested(ctx.args):
            _print_schema_table(name)
            raise typer.Exit(0)
        ...
        from rincode.config.update_channels import set_channel_fields
    ...
    def channels_get_cmd(
        ...
        from rincode.config.update_channels import get_channel_config
    ...
    def channels_reset_cmd(
        ...
        """
        from rincode.config.update_channels import get_channel_config, reset_channel
...
# ---------------------------------------------------------------------------
# channels_app：`rincode channels ...` 的顶层 Typer 命令组
# ---------------------------------------------------------------------------
...
def channels_login(
    ...
    if not spec.capabilities.interactive_login:
        console.print(
            ...
            f"Configure it with:\n[cyan]rincode channels set {channel_name}[/cyan]"
        )
        return
