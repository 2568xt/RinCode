"""``rincode plugins`` — inspect installed memory / context plugins.

Reads ``RinCodeConfig.plugins`` + the live :class:`PluginRegistry`
...
from rincode.cli._helpers import load_runtime_config
...
def register(app: typer.Typer) -> None:
    ...
    def plugins(
        config_path: Optional[str] = typer.Option(
            ...
            "--config",
            "-c",
            help="Path to config file (default: ~/.rincode/config.json)",
        ),
        ...
        # 延迟导入，避免每次调用 ``rincode --help`` 都承担插件发现成本。
...
def _render_plugin_table(
    ...
    *,
    ...
    if not discovered:
        console.print(
            "[yellow]No plugins discovered.[/yellow] Reinstall RinCode from the "
            "same distribution source, or run [bold]uv sync[/bold] from a "
            "RinCode source checkout. Set "
            "[bold]memory.backend[/bold] to [bold]null[/bold], or drop a "
            "third-party manifest under [bold]~/.rincode/plugins/[/bold].",
        )
        return

    table = Table(
        title="RinCode plugins",
        show_header=True,
...
def _source_label(source) -> str:
    ...
    from rincode.plugin import Source
