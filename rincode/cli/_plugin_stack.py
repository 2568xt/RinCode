...
if TYPE_CHECKING:
    ...
    from rincode.memory_engine import MemoryBackend

logger = logging.getLogger(__name__)

_MYNA_PLUGIN_ID = "myna-memory"
_MYNA_BACKEND = "myna"
_MYNA_FACTORY = "myna.integrations.rincode:make_backend"
        ...
        try:
            ...
            if getattr(exc, "code", None) in {"configuration_invalid", "myna_not_initialized"}:
                raise MynaSetupError(
                    "Myna is not initialized or its configuration is invalid; run 'myna init' in the RinCode "
...
def plugin_discovery_sources() -> dict:
    """Resolve the trusted discovery sources scanned by RinCode hosts.

    Shared by :func:`build_plugin_registry` (live boot) and the
    ``rincode plugins`` CLI command so both see the same set:

    - bundled - ``rincode/plugin/memory/`` inside the package;
    - user - operator-managed ``~/.rincode/plugins/``;
    - entry points - installed distributions in the ``rincode.plugins`` group.

    ``project_dir`` remains explicit and disabled. A checkout must not gain an
    executable startup hook merely by containing ``.rincode/plugins``.
    """
    import rincode

    return {
        "bundled_dir": Path(rincode.__path__[0]) / "plugin" / "memory",
        "user_dir": get_product_home() / "plugins",
        "project_dir": None,
        "entry_points_group": "rincode.plugins",
    }


def build_plugin_registry(
    config: "RinCodeConfig",
) -> PluginRegistry:
    ...
    - **bundled** — ``rincode/plugin/memory/<id>/`` shipped inside the
      RinCode package.
    - **user** - ``~/.rincode/plugins/<id>/`` drop-in directories.
    - **entry_points** - the ``rincode.plugins`` group, where
...
def inspect_memory_backend(config: "RinCodeConfig") -> MemoryBackendStatus:
...
def _validate_myna_identity(registry: PluginRegistry) -> None:
    try:
        ...
        raise PluginNotFoundError(
            "configured Myna plugin is unavailable; install a RinCode-compatible Myna adapter from the myna-memory distribution and run "
            "'myna init' in the RinCode workspace, or set memory.backend to null"
        ) from exc
    if identity != (_MYNA_PLUGIN_ID, _MYNA_FACTORY):
        raise PluginIdentityError(
            "Myna plugin manifest identity is invalid; reinstall a RinCode-compatible Myna adapter from the official myna-memory distribution "
            "before starting RinCode"
        )
