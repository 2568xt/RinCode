"""Local slash-command handling for the interactive ``rincode run`` REPL.
...
``rincode cron`` CLI command functions directly
...
``~/.rincode/config.json``. Every config write is shell-only, including
...
_CRON_SHELL_ONLY = {
    ...
    "run inside the REPL event loop. Use `rincode cron run` in a shell.",
}
...
def _handle_cron(args: list[str], console: "Console") -> bool:
    from rincode.cli import cron_commands as cc
    ...
    if sub == "config":
        if rest and rest[0] in ("set", "reset"):
            console.print(
                ...
                "config). Use `rincode cron config ...`.[/yellow]"
            )
            return True
        _invoke(
            console,
...
def _print_cron_help(console: "Console") -> None:
    console.print(
        "[bold]/cron[/bold] — scheduled jobs (~/.rincode/cron/jobs.json)\n"
