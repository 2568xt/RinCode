"""``rincode cron`` subapp — operator-facing CLI for the CronService
state at ``~/.rincode/cron/jobs.json``.
...
- ``cron add`` stores ``channel="cli"`` / ``to="direct"`` by default
  ...
  ``cron.forward_channels`` (see ``rincode cron config``).
"""
...
from rincode.proactive_engine.schedulers.cron.types import CronJob, CronSchedule

cron_app = typer.Typer(
    help="Inspect and manage scheduled cron jobs (~/.rincode/cron/jobs.json)",
...
def _suppress_info_logs() -> None:
    """Mute RinCode subsystem INFO logs so the CLI table output stays
    clean. Set ``RINCODE_CLI_DEBUG=1`` to keep all logs."""
...
def cron_add(
    ...
    For interactive use, prefer ``rincode run`` — the LLM understands
    ...
    ``rincode cron config get`` for current routing.
    """
    # 校验调度方式：三者必须且只能选择一个。
    schedule_flags = [
        ("--cron", cron),
...
cron_config_app = typer.Typer(
    help="Read / write the cron section of ~/.rincode/config.json",
    no_args_is_help=True,
)
...
def cron_config_set(
    ...
    from rincode.config.update import update_cron_config
...
def cron_config_reset(
    ...
    from rincode.config.update import reset_cron_config
