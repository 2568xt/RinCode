"""``rincode doctor`` — health check (static + optional --probe).

Default mode is zero-network, millisecond-fast. ``--probe`` sends one
chat exchange via :func:`rincode.cli._helpers.send_probe`.
...
from rincode.cli._helpers import print_probe_troubleshooting, send_probe
...
def _gather_static_checks() -> DoctorReport:
    ...
    memory = inspect_memory_backend(load_rincode_config(config_path))
    ...
    from rincode.cli._gateway_lock import read_status
...
def _render_human_output(report: DoctorReport) -> None:
    ...
    if not paths.config_exists:
        console.print("\n[yellow]⚠ RinCode is not configured.[/yellow] Run [cyan]rincode onboard[/cyan] to set it up.")
        return

    if not report.config_loaded:
        console.print("\n[red]✗ Config schema invalid.[/red] Run [cyan]rincode onboard --reset[/cyan] to recreate it.")
        return
    ...
    if code == 0:
        ...
        console.print("Run [cyan]rincode provider list[/cyan] / [cyan]rincode provider set[/cyan] to fix routing.")
        ...
        console.print("[red]✗ Native TUI bundle is missing.[/red] Reinstall RinCode or rebuild the TUI.")
...
def register(app: typer.Typer) -> None:
    ...
    def doctor(
        ...
        """Health-check RinCode config, routing, and (optionally) the LLM."""
