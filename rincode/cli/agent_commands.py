...
- The interactive ``rincode run`` REPL command body (multiline paste,
...
def _init_prompt_session() -> None:
    ...
    from rincode.config.paths import get_cli_history_path
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} RinCode[/cyan]")
    console.print(body)
    console.print()
...
def register(app: typer.Typer) -> None:
    ...
    def agent(
        ...
        logs: bool = typer.Option(False, "--logs/--no-logs", help="Show RinCode runtime logs during chat"),
    ):
        ...
        from rincode.cli.onboard_commands import _is_config_populated

        if message is None and _stdout_isatty() and not _is_config_populated():
            from rincode.cli.onboard_commands import ensure_configured_or_onboard
        ...
        # load_runtime_config 必须最先运行：它调用 set_config_path()，使之后的 load_rincode_config()
        # 从 --config 而非默认 ~/.rincode/config.json 读取。否则 --config 中的 skill_forge 会被静默忽略。
        ...
        if resume is not None:
            from rincode.cli.session_commands import resolve_session
            ...
            from rincode.cli.session_commands import resolve_session_cross_channel
        ...
        if logs:
            logger.enable("rincode")
        else:
            logger.disable("rincode")
        ...
        def _thinking_ctx():
            ...
            return console.status("[dim]RinCode is thinking...[/dim]", spinner="dots")
        ...
        if message:
            ...
            from rincode.cli._repl_spine import build_repl
            ...
            # 段错误。该风险存在时，rincode.cli.commands.run 的退出汇合点会硬退出、跳过收尾，
            # 因此此路径只需正常返回。
        else:
            # 交互模式——用户轮次通过 spine（submit -> lane -> hub -> CliOutlet）运行；
            # Cron 轮次使用同一 spine 和 hub。
            from rincode.cli._repl_spine import build_repl, run_repl_loop
            ...
            async def run_interactive():
                ...
                try:
                    ...
                    def _slash(command: str) -> bool:
                        from rincode.cli._repl_slash import handle_repl_slash
