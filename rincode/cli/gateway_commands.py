...
Spawns the RinCode gateway: agent loop + channel manager + cron service.
...
def build_model_routing(config, provider):
    ...
    if config.routing.backend == "knn":
        from rincode.providers.per_model_provider import PerModelProvider
    ...
    from rincode.routing.types import RoutingProfileName
...
def _build_gateway_channels(config) -> set[str]:
    ...
    them (the TUI / ``rincode run`` session), so a TUI-set reminder always
...
def register(app: typer.Typer) -> None:
    ...
    def gateway(
        ...
        """Start the RinCode gateway."""
        from rincode.channels.manager import ChannelManager
        from rincode.cli._runtime_assembly import assemble_runtime
        from rincode.config.paths import get_cron_dir, resolve_service_paths
        ...
        # load_runtime_config 必须最先运行：它调用 set_config_path()，使后续 load_rincode_config()
        ...
        from rincode.cli._gateway_lock import GatewayAlreadyRunningError, acquire
        ...
        try:
            ...
            console.print(
                f"[red]✗[/red] RinCode gateway already running for this instance "
        ...
        console.print(f"{__logo__} Starting RinCode gateway on port {port}...")
        ...
        async def run():
            ...
            try:
                ...
                from rincode.cli._gateway_spine import build_gateway
                ...
                # 线程地址，而无需从会话 ID 重建。
                from rincode.spine import Text as _Text
                ...
                from rincode.spine import Text
                from rincode.spine.turn import BusyPolicy
