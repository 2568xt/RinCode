"""``rincode tracing`` — open the tracing dashboard.

The dashboard is a dependency-free Node viewer bundled under
``rincode/tracing/viewer/``. Instrumentation itself runs in-process (installed at
CLI startup, see :mod:`rincode.tracing`); this command only launches the viewer
that reads the captured spans from ``~/.rincode/traces``.

``rincode tracing`` (bare) lazily starts the viewer if it is not already running,
then opens the browser. It reuses RinCode's Node discovery (:func:`find_node`),
so it needs the same Node >= 22 that ``rincode`` already requires.
...
from rincode.tracing import config as tracing_config
