"""``rincode session`` subapp - manage conversation sessions.
...
def session_create(
    ...
    The id materialises on first use when ``rincode run --session`` is
    called with it.
...
def session_resume(
    ...
    The printed key can be fed directly to ``rincode run --session``.
...
def session_fork(
    ...
    the new child id (first line) for use with ``rincode run --session``.
    """
    manager = _open_manager()
    key = resolve_session(manager, id_or_prefix)
