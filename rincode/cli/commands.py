...
- Top-level commands (each exposes a ``register(app)`` function):
    - ``run``      → ``rincode/cli/agent_commands.py``
    - ``doctor``   → ``rincode/cli/doctor_commands.py``
    - ``evolve``   → ``rincode/cli/evolve_commands.py``
    - ``maintain`` → ``rincode/cli/maintenance_commands.py``
    - ``gateway``  → ``rincode/cli/gateway_commands.py``
    - ``onboard``  → ``rincode/cli/onboard_commands.py``
    - ``status``   → ``rincode/cli/status_commands.py``

- Subcommand groups (each exposes a typer ``*_app`` instance):
    - ``channels`` → ``rincode/cli/channel_commands.py``
    - ``cron``     → ``rincode/cli/cron_commands.py``
    - ``provider`` → ``rincode/cli/provider_commands.py``
    - ``sessions`` → ``rincode/cli/session_commands.py``
    - ``skills``   → ``rincode/cli/skill_commands.py``

Shared helpers used across multiple command modules live in
``rincode/cli/_helpers.py``.
"""
...
from rincode import __logo__, __version__

app = typer.Typer(
    name="rincode",
    help=f"{__logo__} RinCode - compact, composable, benchmark-gated Agent Harness with an opt-in evolution path",
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} RinCode v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """RinCode - compact, composable, benchmark-gated Agent Harness with an opt-in evolution path.

    Bare ``rincode`` runs the startup gate and enters the native TUI.
    """
    if ctx.invoked_subcommand is not None:
        return
    from rincode.cli.tui_commands import launch_tui
...
from rincode.cli.channel_commands import channels_app
from rincode.cli.cron_commands import cron_app
from rincode.cli.provider_commands import provider_app
from rincode.cli.sandbox_commands import sandbox_app
from rincode.cli.skill_commands import skill_app
...
from rincode.cli.session_commands import session_app
...
def run() -> None:
    ...
    Rust/tokio background thread — see :mod:`rincode.cli._exit`). Any command that
