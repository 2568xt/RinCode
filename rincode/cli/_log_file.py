...
writes its logs next to its own config rather than always to ``~/.rincode``.

Env vars:
    RINCODE_CLI_DEBUG=1  — additionally mirror DEBUG+ to stderr.
"""
...
def redirect_loguru_to_file(
    ...
    *,
    ...
    if os.environ.get("RINCODE_CLI_DEBUG"):
