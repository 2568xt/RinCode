...
def warn_about_pending_cli_reminders(cron_service, config: Config) -> None:
    ...
    if not config.cron.forward_channels:
        console.print(
            ...
            "`rincode cron config set forward_channels '*'` to broadcast to "
...
def check_provider_credentials(config: Config) -> None:
    ...
    if provider_name == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.rincode/config.json under providers.azure_openai section")
    ...
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and (spec.is_oauth or spec.is_local)):
        ...
        console.print("Set one in ~/.rincode/config.json under providers section")
        raise typer.Exit(1)


def make_provider(config: Config):
    """Create the appropriate LLM provider from config."""
    from rincode.providers.azure_openai_provider import AzureOpenAIProvider
    from rincode.providers.base import GenerationSettings
    from rincode.providers.openai_codex_provider import OpenAICodexProvider
    ...
    if provider_name == "openai_codex" or model.startswith("openai-codex/"):
        ...
        # Provider config 中的显式 body 追加项优先于内置默认项；空配置仍不向 LiteLLM 传参。
        default_extra_body = {}
        if provider_name == "openrouter" and "qwen" in (model or "").lower():
            default_extra_body = {"reasoning": {"enabled": False}}
        configured_extra_body = dict(p.extra_body or {}) if p else {}
        extra_body = {**default_extra_body, **configured_extra_body} or None
...
def print_probe_troubleshooting(provider: str | None) -> None:
    ...
    if provider:
        console.print(
            f"  [dim]·[/dim] [cyan]rincode provider test {provider}[/cyan] — re-check credentials without spending tokens"
        )
        console.print(
            f"  [dim]·[/dim] [cyan]rincode provider get {provider}[/cyan] — inspect what's actually stored on disk"
        )
    console.print(
        "  [dim]·[/dim] Check the model id in [cyan]~/.rincode/config.json[/cyan] "
...
def print_deprecated_memory_window_notice(config: Config) -> None:
    ...
    if config.agents.defaults.should_warn_deprecated_memory_window:
        console.print(
            ...
            "[cyan]rincode onboard[/cyan] to refresh your config template."
        )
