"""把 Feishu/Lark 消息接入 RinCode Runtime，并把 Runtime 回复交付回会话。
...
from rincode.channels.adapters.feishu import cards, content
from rincode.channels.base import ChannelBase
from rincode.channels.errors import transient_network
from rincode.channels.media import save_media_bytes
from rincode.channels.transcribe import transcribe_audio
from rincode.config.schema import FeishuConfig
from rincode.spine.delivery import TerminalDeliveryError
...
class FeishuChannel(ChannelBase):
    ...
    ``ChannelManager`` 管理实例生命周期，``rincode.spine`` 消费已发布的入站消息。
    ...
    def _on_message_sync(self, data: Any) -> None:
        """把 lark WebSocket（WS）线程收到的事件安全地投递到 RinCode 主事件循环。
