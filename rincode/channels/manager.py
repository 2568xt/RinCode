...
from rincode.product import DISTRIBUTION_NAME
...
def _missing_dep_hint(modname: str) -> str:
    ...
    Editable Dev Checkout 使用 ``uv sync``；Wheel/Tool Install 提示从 RinCode Source Checkout 安装所需 Extra。PEP 610
    ``direct_url.json`` 区分两者；Wheel 记录 ``archive_info`` 且无 ``dir_info``，因此用 ``.get`` Chain 避免
    KeyError。该函数运行在 Channel 已失败的路径，Missing/Corrupt Metadata 必须降级 Source Install Hint，Never
    Raise。
    """
    editable = False
    try:
    ...
    return (
        "From a RinCode source checkout, install channel dependencies into the RinCode environment: "
        f"uv pip install '.[channel-{modname}]'"
    )
