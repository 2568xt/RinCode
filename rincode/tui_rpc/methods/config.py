...
``~/.rincode/config.json``，例如 ``tui.theme`` 对应
``{"tui": {"theme": "..."}}``，使 legacy ``rincode.config.loader`` 无需额外 schema
转换就能读取同一文件。
...
from rincode.providers.registry import find_by_model
...
def _load_config() -> dict[str, Any]:
    ...
    """
    from rincode.config.loader import ConfigReadError, read_raw_or_raise
