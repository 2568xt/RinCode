...
from rincode.utils.helpers import detect_image_mime

if TYPE_CHECKING:
    from rincode.memory_engine.backend import Memory
...
def identity_text(workspace: Path, state: Path | None = None) -> str:
    ...
    RinCode Guidelines，以及 `_language_directive` 的语言规则。它只描述当前运行现场，不读取这些
