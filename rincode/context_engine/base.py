...
``rincode.context_engine`` 当前只有一个 shipping implementation：:class:`ContextAssembler`。
AgentLoop 经 :func:`rincode.context_engine.build_context_engine` 构建它，并且始终只持有一个
...
:mod:`rincode.agent.context`；后者的 :class:`ContextBuilder` 是 Engine 会调用的低层构建工具，
不是本轮窗口所有者。``AssembledContext`` 与 ``TokenBudget`` 仍位于
:mod:`rincode.memory_engine.base`，因为它们早于本 ABC；``MemoryEngine`` 与 ``ContextEngine`` 是
...
from rincode.memory_engine.base import AssembledContext, TokenBudget

if TYPE_CHECKING:
    # 避免运行时导入：``curator`` 会反向从本模块导入 ``ContextEngine``，
    # 因此仅在类型提示中引用 ``TurnContext``，以打破循环依赖。
    from rincode.context_engine.curator import TurnContext
...
class ContextEngine(ABC):
    ...
    当前唯一实现 :class:`ContextAssembler <rincode.context_engine.assembler.ContextAssembler>`
