"""RinCode Feature Configuration，在 Base `Config` 上组合独立 Feature Blocks。

Usage：

    from rincode.config import RinCodeConfig, load_rincode_config

    cfg = load_rincode_config()
    if cfg.context.engine == "curator":
        ...

Design 上，``RinCodeConfig`` Compose Base ``Config`` 而非 Subclass，使 Base Schema 不变，Feature Blocks 可
...
from rincode.config.loader import (
    EXTENSION_KEYS,
    _migrate_config,
    get_config_path,
)
from rincode.config.loader import load_config as load_base_config
from rincode.config.schema import Config as BaseConfig
...
class PluginsConfig(_Base):
    ...
    ``disabled`` 是按 Plugin ID（匹配 ``rincode-plugin.toml`` 的 ``id``）索引的 User Opt-out List。``config``
...
class CheckpointConfig(_Base):
    ...
    shadow_dir: str = ".rincode/shadow.git"
...
class TracingConfig(_Base):
    """In-tree ``rincode.tracing`` Observability Config。

    Default On；每个 ``rincode`` Command 在 AgentLoop 构造前安装 Non-invasive Instrumentation。
    ``RINCODE_TRACING=0`` 是覆盖此 Block 的 Explicit Env Kill-switch。``rincode tracing`` 或 TUI ``/tracing``
...
class RinCodeConfig(_Base):
    """RinCode Root Config，把 Base `Config` 与 Feature Extensions Compose 在一起。
...
def load_rincode_config(config_path: Path | None = None) -> RinCodeConfig:
    """从同一 JSON File 加载 Base Config 与 RinCode Extension Blocks。
    ...
    return RinCodeConfig(base=base, **overrides)
