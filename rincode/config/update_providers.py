...
from rincode.config.schema import ProvidersConfig
from rincode.config.update import (
    _annotation_str,
    _coerce_value,
    _field_default,
    _flatten_instance,
...
from rincode.providers.registry import ProviderSpec, find_by_name
...
def _provider_spec(name: str) -> ProviderSpec:
    ...
    if spec is None:
        raise KeyError(f"No registry entry for provider '{name}'. Add a ProviderSpec to rincode/providers/registry.py.")
    return spec
    ...
    if spec.is_oauth:
        ...
        if forbidden:
            raise RuntimeError(
                ...
                f"{forbidden} directly. Run: rincode provider login "
                f"{name.replace('_', '-')}"
            )
