"""RinCode Base Agent Runtime 的 Pydantic Configuration Schema。
...
from rincode.product import DEFAULT_WORKSPACE_SPEC, get_default_workspace
...
class ProviderConfig(Base):
    ...
    extra_body: dict[str, Any] = Field(default_factory=dict)  # Provider 专属请求体扩展
...
class ProvidersConfig(Base):
    ...
    `rincode.providers.registry` 完成；声明 Block 不自动 Import/Connect Provider。
    """
...
class Config(BaseSettings):
    """RinCode Base Agent Runtime 的 Root Configuration。

    组合 Agents、Channels、Providers、Gateway、Tools、Routing、Cron 与 UI Language，并支持 ``RINCODE_`` Env
    Prefix + ``__`` Nested Delimiter。RinCode Feature Extensions 通过 `load_rincode_config` 另行 Compose。
    """
    ...
    def skill_forge(self):
        ...
        Extension Blocks 由 ``load_rincode_config`` 加载，不通过 Base `Config`。该 Property 只为访问 Plain
        ``config.skill_forge`` / ``Config.skill_forge`` 的 Older Code 保留；每次返回 Default Object，不代表 User Extension Values。
        """
        from rincode.config.rincode import SkillForgeConfig
