...
from rincode.spine.events import RunnerEvent, TurnEnded, TurnEvent, TurnFailed, TurnStarted
from rincode.spine.runner import Emit, TurnOutcome, TurnRunner
from rincode.spine.turn import BusyPolicy, Origin, TurnRequest
...
_TERMINAL_SINK_TIMEOUT = 1.0  # 终态 sink 最多占用 Worker 的时间（秒）


def _consume_terminal_task(task: asyncio.Task) -> None:
...
class Lane:
    ...
    async def _emit_terminal(self, event: TurnFailed | TurnEnded) -> None:
        # 终态只能创建一次 sink 调用。把它放进独立 task 后，调用方的第二次 cancel 不会
        # 同时取消 sink；等待仍有明确上限，避免不可靠的 channel sink 把 Lane 永久挂住。
        sink_task = asyncio.create_task(self._sink(event))
        deadline = asyncio.get_running_loop().time() + _TERMINAL_SINK_TIMEOUT
        while not sink_task.done():
            remaining = deadline - asyncio.get_running_loop().time()
            if remaining <= 0:
                break
            try:
                await asyncio.wait_for(asyncio.shield(sink_task), timeout=remaining)
            except asyncio.CancelledError:
                # 终态清理优先于后续 cancel；只在 deadline 内继续等待同一个调用。
                continue
            except asyncio.TimeoutError:
                break
            except Exception:
                # 读取 task 结果会消费 sink 的异常，下面不再重复调用或等待。
        ...
        # 超时后取消这一次调用并立即让 Worker 继续。sink 不是事务存储；超时只表示调度器
        # 停止等待，不把“已尝试”冒充为“已持久化”。Python 协程无法被强制杀死；若 sink
        # 吞掉 CancelledError，done callback 会在它最终结束时消费结果，但不会再阻塞 Lane。
        sink_task.cancel()
        sink_task.add_done_callback(_consume_terminal_task)
        return
    ...
    async def _run_turn(self, req: TurnRequest) -> TurnOutcome | None:
        ...
        outcome: TurnOutcome | None = None
        # `start_dispatched` records that the lifecycle event was handed to the
        # sink.  A sink is an async callback rather than a transactional event
        # store: it may have accepted the event and then suspended, or it may
        # fail before making the event observable.  `runner_started` is kept
        # separate so tracing can distinguish those cases.
        ...
        with trace.span(
            ...
            try:
                ...
                turn_span.set(
                    semconv.spine_turn_cancelled(
                        start_dispatched=start_dispatched,
                        runner_started=runner_started,
                    )
                )
                if start_dispatched:  # sink 已接收启动调用；启动前取消不发出事件
