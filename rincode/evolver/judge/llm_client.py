...
:class:`LitellmBackend` 包装 ``rincode.providers`` 的 ``LLMProvider``，包括 self-hosted Qwen-397B；
...
class LitellmBackend(JudgeLLMBackend):
    """委托 RinCode ``LLMProvider`` 的 Judge backend。
    ...
    def __init__(
        self,
        provider: Any,  # rincode.providers.base.LLMProvider，延迟确定类型
        *,
...
class OpenRouterBackend(JudgeLLMBackend):
    ...
    不经 litellm 是为了让 OpenRouter API key 与 RinCode main provider 分离，并独立核算 external-LLM
    ...
    # 对空内容和暂时性 HTTP 错误重试。OpenRouterBackend 绕过 RinCode 提供商栈，无法直接获得
    ...
    async def call(
        ...
        *,
        ...
        headers = {
            ...
            "X-Title": "RinCode Harness Evolver",
        }
