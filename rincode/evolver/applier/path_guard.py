...
file，例如 ``rincode/agent/loop/main.py``，以及 trailing slash directory subtree，例如
``rincode/evolver/``，后者匹配目录自身及全部 descendant。``MUTABLE_OVERRIDES`` 优先，可从较宽
...
IMMUTABLE_PATTERNS: tuple[str, ...] = (
    ...
    "rincode/eval_engine/adapter/",
    "rincode/eval_engine/judge/",
    ...
    # ── L3——能力契约 ─────────────────────────────────────────────────────
    "rincode/agent/loop/main.py",
    "rincode/agent/context/",
    "rincode/agent/tools/base.py",
    "rincode/agent/tools/registry.py",
    "rincode/agent/personalizer/",
    "rincode/agent/subagent/",
    # RinCode 使用 spine 代替上游智能体 API；运行器是每轮必经的桥梁，属于同一契约层级。
    "rincode/agent/spine_runner.py",
    "rincode/spine/",
    "rincode/providers/",
    "rincode/session/",
    "rincode/memory_engine/",
    "rincode/context_engine/",
    "rincode/sandbox/",
    "rincode/security/",
    # 配置：schema/loader（.py）不可变，值文件（.yaml/.json）可变。
    "rincode/config/__init__.py",
    "rincode/config/rincode.py",
    "rincode/config/loader.py",
    "rincode/config/paths.py",
    "rincode/config/schema.py",
    "rincode/config/update.py",
    "rincode/config/update_channels.py",
    "rincode/config/update_providers.py",
    # ── L4——审计/数据完整性 ──────────────────────────────────────────────
    "rincode/eval_engine/hooks/tool_audit_hook.py",
    # ── L5——测试、依赖和 CI ──────────────────────────────────────────────
    "tests/",
    "pyproject.toml",
    "uv.lock",
