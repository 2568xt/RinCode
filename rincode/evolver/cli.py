"""``rincode evolve``：opt-in Beta self-evolution 命令入口。
...
CLI 只解析参数并委托 :mod:`rincode.evolver.launch.runner`。命令返回 0 只表示对应 command 完成；
...
from rincode.product import CLI_NAME


def build_parser() -> argparse.ArgumentParser:
    """构造 ``rincode evolve`` 的 argparse command tree。
...
def main(argv: list[str] | None = None) -> int:
    ...
    from rincode.evolver.launch import runner
