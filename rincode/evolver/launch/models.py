...
- ``{provider: openai_compat, base_url, model, ...}`` -> one OpenAI-compatible
  chat endpoint (:func:`rincode.evolver.orchestrator.providers.openai_compat.make_call_fn`).
- ``{provider: claude_cli, model, ...}`` -> ``claude -p`` subprocess per call.
- ``{provider: rincode, model?, api_base?, api_key_env?}`` -> RinCode's
  ``LitellmProvider`` bridged to a sync call_fn; ``model`` omitted falls back
  to RinCode's ``agents.defaults.model`` - so a config file with no
  ``models:`` section evolves with whatever model RinCode itself is running.
...
def make_rincode_call_fn(
    ...
    *,
    ...
    provider = LiteLLMProvider(
        api_key=api_key,
        api_base=api_base,
        default_model=model or _rincode_default_model(),
    )
    ...
    def call(messages: list) -> str:
        ...
        if not isinstance(content, str) or not content.strip():
            raise RuntimeError("RinCode provider returned empty content")
        return content
...
def _build_call_fn(spec: dict) -> CallFn:
    ...
    kind = spec.get("provider", "rincode")
    kwargs = {k: v for k, v in spec.items() if k != "provider"}
    if kind == "openai_compat":
        from rincode.evolver.orchestrator.providers.openai_compat import make_call_fn
    ...
    if kind == "claude_cli":
        ...
        from rincode.evolver.orchestrator.providers.claude_cli import make_claude_call_fn
        ...
        if shutil.which(claude_bin) is None:
            raise ValueError(
                ...
                "openai_compat/rincode"
            )
        return make_claude_call_fn(model, **kwargs)
    if kind == "rincode":
        model = kwargs.pop("model", None)
        return make_rincode_call_fn(model, **kwargs)
    raise ValueError(f"unknown model provider {kind!r} (expected openai_compat / claude_cli / rincode)")
...
def describe_models(models_cfg: dict) -> dict:
    ...
    RinCode default model 读取失败时写 ``<rincode default>``。返回值供 provenance，不用于创建 provider。
    """
    out = {}
    for role in ("driver", "design", "verdict"):
        ...
        if (
            out[role].get("provider", "rincode") == "rincode"
            and "model" not in out[role]
            and "inherit" not in out[role]
            and "omitted" not in out[role]
        ):
            try:
                out[role]["model"] = _rincode_default_model()
            except Exception:  # noqa: BLE001 — 描述仅尽力生成
                out[role]["model"] = "<rincode default>"
    return out


__all__ = ["CallFn", "build_call_fn", "build_role_call_fns", "describe_models", "make_rincode_call_fn"]
