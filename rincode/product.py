"""集中定义 RinCode Product Identity 与 Persistent-state Roots。

模块统一保存产品名、CLI 名、Distribution Name、Logo 与 `.rincode` 状态目录约定，并提供从
`RINCODE_HOME`、用户 Home 和 Workspace 派生实际路径的函数。Project State Directory 使用解析后的
...
PRODUCT_NAME = "RinCode"
PRODUCT_LOGO = "✦"
CLI_NAME = "rincode"
DISTRIBUTION_NAME = "rincode-harness"
GLOBAL_STATE_DIRNAME = ".rincode"
WORKSPACE_STATE_DIRNAME = ".rincode"
DEFAULT_WORKSPACE_SPEC = "~/.rincode/workspace"


def get_product_home() -> Path:
    override = os.environ.get("RINCODE_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    return Path.home() / GLOBAL_STATE_DIRNAME
