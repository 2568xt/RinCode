...
感谢你愿意帮助改进 RinCode。第一次贡献不需要先理解整个 Runtime；请选择一个已经完成
需求澄清、带有明确验收命令的 Issue，从一个可验证的小改动开始。

## 从哪里开始

1. 与本地 RinCode 分支维护者确认任务，选择带有
...
| 目标分支 | 用途 |
| --- | --- |
| `main` | 本地 RinCode 分支基线、通用 Runtime、文档和贡献基础设施 |
...
RinCode 使用 Python 3.12、Node.js 22 和 `uv`。依赖安装与常用验证命令：
...
本地 RinCode 问题请先联系本分支维护者。能够在上游 Runtime 复现的 Bug 可转为上游 Gitee Issue，
