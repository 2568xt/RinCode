#!/bin/sh
# RinCode 国内一键安装脚本（macOS/Linux）。
#
#   默认从当前 RinCode 源码安装；独立脚本需显式设置 RINCODE_WHEEL_URL。
#   本地：git clone ... && cd rincode && ./install.sh
#
# 目标：让全新机器无需手工步骤即可从任意目录运行 `rincode`。脚本具备幂等性，
# 会探测已有内容并只补齐缺项：
#   1. uv            （Python 工具链与包管理器）
#   2. Node.js >= 22 （TUI 运行时；系统缺少时私有安装）
#   3. rincode          （作为全局 uv 工具安装到 ~/.local/bin/rincode）
#
...
# --- 配置 -----------------------------------------------------------------
MIN_NODE_MAJOR=22
RINCODE_HOME="${RINCODE_HOME:-${HOME:?HOME is required, or set RINCODE_HOME explicitly}/.rincode}"
NODE_RUNTIME_DIR="$RINCODE_HOME/runtime"
RINCODE_NODE_MIRROR="${RINCODE_NODE_MIRROR:-https://mirrors.aliyun.com/nodejs-release}"
RINCODE_NODE_CHECKSUM_BASE="${RINCODE_NODE_CHECKSUM_BASE:-https://nodejs.org/dist}"
RINCODE_NPM_REGISTRY="${RINCODE_NPM_REGISTRY:-https://registry.npmmirror.com}"
RINCODE_PYPI_INDEX="${RINCODE_PYPI_INDEX:-https://pypi.tuna.tsinghua.edu.cn/simple}"
RINCODE_UV_INSTALL_URL="${RINCODE_UV_INSTALL_URL:-https://astral.sh/uv/install.sh}"
RINCODE_INSTALL_TMP=""
...
gitee_curl() {
  printf 'Authorization: Bearer %s\n' "$RINCODE_GITEE_TOKEN" | curl -fsSL -H @- "$@"
}
cleanup() {
  if [ -n "$RINCODE_INSTALL_TMP" ] && [ -d "$RINCODE_INSTALL_TMP" ]; then
    rm -rf "$RINCODE_INSTALL_TMP"
  fi
}
trap cleanup EXIT
...
ensure_uv() {
  ...
  curl -fsSL "$RINCODE_UV_INSTALL_URL" | sh
...
# 索引不可达时回退到固定版本。
latest_node_v22() {
  idx="$(curl -fsSL "$RINCODE_NODE_MIRROR/index.json" 2>/dev/null || true)"
  ver="$(printf '%s' "$idx" | tr ',' '\n' | grep -o '"version":"v22\.[0-9.]*"' \
         | head -n1 | sed 's/.*"v/v/; s/"$//')"
  [ -n "$ver" ] && printf '%s' "$ver" || printf 'v22.20.0'
}

# 输出 RinCode 配置的首个私有 Node 二进制路径；不存在时返回非零。遍历 glob 可避免
...
ensure_node() {
  ...
  if pn="$(private_node_bin)"; then
    ok "RinCode private Node already present ($pn)"
    return
  fi
  ...
  url="$RINCODE_NODE_MIRROR/${ver}/${pkg}.tar.gz"
  ...
  curl -fsSL "$RINCODE_NODE_CHECKSUM_BASE/${ver}/SHASUMS256.txt" -o "$tmp/SHASUMS256.txt" \
  ...
  # RinCode 的 find_node() 会自动匹配 ~/.rincode/runtime/node-*/bin/node，因此无需
  # 修改 PATH。
}

# --- 3. 安装 RinCode ----------------------------------------------------------
install_rincode() {
  # 本地模式：从 RinCode 源码检出运行时，以可编辑模式安装工作树，符合开发者预期；
  # 独立脚本则要求显式提供 RinCode wheel。
  script_dir="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
  if [ -f "$script_dir/pyproject.toml" ] && grep -q '^name = "rincode-harness"' "$script_dir/pyproject.toml" 2>/dev/null; then
    info "Local RinCode source detected; editable install: $script_dir"
    ...
    if [ ! -f "$script_dir/ui-tui/dist/entry.js" ]; then
      ...
      if [ -n "$node_bin" ] && [ -x "$node_bin" ]; then
        ...
        if PATH="$node_dir:$PATH" command -v npm >/dev/null 2>&1; then
          ...
          ( cd "$script_dir/ui-tui" && PATH="$node_dir:$PATH" npm ci --registry "$RINCODE_NPM_REGISTRY" && PATH="$node_dir:$PATH" npm run build )
        else
          warn "Found node but not npm; skipping TUI build; rincode may not work"
        fi
      else
        warn "No usable node found; skipping TUI build; rincode may not work"
      fi
    fi
    install_result=0
    UV_DEFAULT_INDEX="$RINCODE_PYPI_INDEX" uv tool install --force -e "$script_dir[channels]" || install_result=$?
    if [ "$install_result" -ne 0 ]; then
      warn "Channel dependencies failed to install; installed base rincode only. Some channels stay unavailable (see: rincode channels list)."
      UV_DEFAULT_INDEX="$RINCODE_PYPI_INDEX" uv tool install --force -e "$script_dir"
    fi
  else
    # 脱离源码运行时必须显式提供可信的 RinCode wheel；此模式不从包索引自动下载。
    wheel_url="${RINCODE_WHEEL_URL:-}"
    [ -n "$wheel_url" ] || die "Run from a RinCode source checkout or set RINCODE_WHEEL_URL to a trusted RinCode wheel."
    wheel_name="${wheel_url%%\?*}"
    wheel_name="${wheel_name##*/}"
    case "$wheel_name" in
      rincode_harness-*.whl) ;;
      *) die "Expected a rincode_harness wheel, got: $wheel_name" ;;
    esac
    wheel_source="$wheel_url"
    if [ -n "${RINCODE_GITEE_TOKEN:-}" ]; then
      case "$wheel_url" in
        https://gitee.com/*)
          RINCODE_INSTALL_TMP="$(mktemp -d)"
          wheel_name="${wheel_url%%\?*}"
          wheel_name="${wheel_name##*/}"
          case "$wheel_name" in
            *.whl) ;;
          ...
          wheel_path="$RINCODE_INSTALL_TMP/$wheel_name"
    ...
    install_result=0
    UV_DEFAULT_INDEX="$RINCODE_PYPI_INDEX" uv tool install --force "rincode-harness[channels] @ $wheel_source" || install_result=$?
    if [ "$install_result" -ne 0 ]; then
      warn "Channel dependencies failed to install; installed base rincode only. Some channels stay unavailable (see: rincode channels list)."
      UV_DEFAULT_INDEX="$RINCODE_PYPI_INDEX" uv tool install --force "rincode-harness @ $wheel_source"
    fi
  fi
  # 确保后续 shell 的 PATH 包含 uv 工具目录 ~/.local/bin。
  uv tool update-shell || true
  ok "RinCode installed"
}
...
main() {
  ...
  printf '\n    \033[1mrincode onboard --skip-memory\033[0m    # configure Provider and first Turn\n'
  printf '    \033[1mrincode\033[0m            # enter the TUI\n'
  printf '    \033[1mrincode run\033[0m -m "hello"\n\n'
