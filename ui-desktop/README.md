...
| 操作 | 行为 |
| --- | --- |
| 切换项目或对话 | 侧栏按项目列出已发送并保存的对话；点击旧对话可以继续。空白的新对话不会作为历史保存。输入草稿按项目和对话分别保留，切换后恢复，发送后清空当前草稿；草稿只保存在内存，重启应用后不会恢复。 |
...
回复支持代码块、链接与 Markdown 表格；表格支持列对齐、行内代码和转义竖线，宽表格可以横向滚动。工具记录与思考内容可展开查看。生成时默认跟随最新输出；向上滚动阅读历史会暂停跟随，回到底部后恢复。切换对话时从该对话底部开始。
