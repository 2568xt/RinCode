"""Run ONE AppWorld task through a minimal RinCode AgentLoop, then grade.

Cross-venv design (see tool.py): AppWorld (pydantic v1) runs as an HTTP
``environment`` server; this runner (RinCode, pydantic v2) never imports
...
async def _run(args) -> dict:
    ...
    try:
        ...
        try:
            # RinCode 的 AgentLoop 由 Spine 驱动，没有 process_direct。通过 run_turn 运行一个
            ...
            req = TurnRequest(
                ...
                # RinCode 的 SessionManager 按 ':' 将对话键拆为 <channel>/<chat_id>.jsonl。添加固定
...
def main(argv=None) -> int:
    ...
    p.add_argument("--config", required=True, help="RinCode runtime config JSON.")
