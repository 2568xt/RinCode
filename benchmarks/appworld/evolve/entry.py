...
def build(ctx: LaunchContext) -> BenchBundle:
    ...
    try:
        ...
        from rincode.config.loader import load_config as _load_subject_config

        with contextlib.redirect_stdout(io.StringIO()):
            _load_subject_config(config_path)
    except ValueError as exc:
        raise ValueError(
            f"bench_config.config_path {config_path} is not a valid RinCode "
    ...
    if test_ids:
        ...
        def unseal(records: list[dict], orch) -> dict:
            ...
            from rincode.evolver.orchestrator.sealed.runner import unseal_retention
