...
pydantic v1, RinCode v2). CLI (from the real ``batch.py``)::
...
class AppWorldConfig:
    ...
    appworld_root: Path  # RinCode 检出或工作树，包含批量模块。
    data_root: Path  # AppWorld 安装目录，包含 data/ 与虚拟环境。
    appworld_bin: Path
    appworld_python: Path
    python_exe: str  # RinCode 使用的 Pydantic v2 虚拟环境 Python。
...
def run_eval(
    ...
    *,
    ...
    ``python -m rincode...`` import the candidate's harness from that checkout
