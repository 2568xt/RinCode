...
- **induce**: the bench-neutral open-ended map-reduce in
  :mod:`rincode.evolver.orchestrator.nodes.taxonomy` discovers a taxonomy from
...
WHERE_CLASSES = {
    ...
    "agent_hook_new": "a NEW lifecycle hook rincode/agent/hook/<name>.py (runtime intervention: finalize/recovery/verify).",
    "agent_hook_wire": "benchmarks/appworld/agent_cli.py hook wiring (construct the hook and pass it to AgentLoop, unconditionally).",
    "agent_loop": "rincode/agent/loop/ (loop-level change — use sparingly).",
