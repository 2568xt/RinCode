...
from rincode.evolver.orchestrator.nodes.taxonomy import (
    TaxonomySpec,
    add_failure_mode,
    coerce_mode,
    empty_failure_map,
...
from rincode.evolver.orchestrator.providers.claude_agentic import run_agentic_session
...
def build_analysis_workspace(
    ...
    lines += [
        ...
        f"- transcripts: sessions/appworld/<task>_{exp}_k<n>.jsonl (RinCode layout) or att/<task>_{exp}_k<n>/sessions/",
        "- harness source (the code a fix would patch): harness/benchmarks/appworld/, harness/rincode/agent/",
    ]
