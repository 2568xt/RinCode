...
from rincode.evolver.tree.node import AppliedPatch, PatchComponent
...
def _patch_where(label: CandidateLabel, paths: list[str]) -> PatchWhere:
    ...
    if label is CandidateLabel.prompt:
        if all(path.startswith("rincode/templates/") for path in paths):
...
def _candidate_label(paths: list[str]) -> CandidateLabel:
    if paths and all(path.startswith("rincode/memory_engine/skills/") for path in paths):
        return CandidateLabel.skill
    if paths and all(path.startswith("rincode/templates/") for path in paths):
