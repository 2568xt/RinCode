...
def default_sess_path(runs_root: Path, ws_root: Path, tid: str, exp: str, k: int) -> Path | None:
    ...
    if legacy.exists():
        return legacy
    # RinCode SessionManager 将对话键 "appworld:<tid>_<exp>_k<k>" 持久化为
    # sessions/<channel>/<chat_id>.jsonl。
    rincode = ws_root / "sessions" / "appworld" / f"{tid}_{exp}_k{k}.jsonl"
    return rincode if rincode.exists() else None
