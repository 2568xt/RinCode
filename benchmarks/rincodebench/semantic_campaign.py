...
from benchmarks.rincodebench.canonical import (
    canonical_bytes,
    canonical_digest,
)
from benchmarks.rincodebench.packs.memory_skill.fixtures import (
    anonymous_item_id,
)
...
from benchmarks.rincodebench.packs.memory_skill.semantic_runtime import (
...
SEMANTIC_SCHEMA = "rincode.rincodebench.semantic-addendum.v1"
SEMANTIC_SCHEMA_V2 = "rincode.rincodebench.semantic-addendum.v2"
...
DEFAULT_OUTPUT_ROOT = Path(".rincode/evidence/rincodebench-semantic")
DEFAULT_OUTPUT_ROOT_V2 = Path(".rincode/evidence/rincodebench-semantic-v2")
APPROVAL_ENV = "RINCODEBENCH_SEMANTIC_PAID_APPROVAL"
APPROVAL_ENV_V2 = "RINCODEBENCH_SEMANTIC_V2_PAID_APPROVAL"
    ...
    *,
    ...
    if experiment_root.exists():
        ...
        if (
            ...
            or existing.get("rincode_commit") != preflight.rincode_commit
    ...
    fixture = semantic_fixture(track, version=version)
    with tempfile.TemporaryDirectory(prefix=f"rincode-semantic-{track}-") as directory:
...
def rebuild_semantic_report(experiment_root: Path) -> dict[str, Any]:
    ...
    if not plan_digest or any(
        record.get("schema") != f"rincode.rincodebench.semantic-retrieval-record.{version}"
    ...
    payload = {
        "schema": f"rincode.rincodebench.semantic-report.{version}",
        ...
        "rincode_commit": preflight.get("rincode_commit"),
    ...
    cv_payload = {
        "schema": f"rincode.rincodebench.semantic-cv-metrics.{version}",
        "experiment_id": manifest["experiment_id"],
...
def _load_embedding_settings() -> Any:
    ...
    from rincode.config.update_everos import configure_everos_env
...
def _semantic_budget_ledger_path(
    ...
    *,
    ...
    root = (
        budget_root if budget_root is not None else get_product_home() / "evidence" / "rincodebench" / "semantic-approvals"
    )
...
def _inspect_semantic_budget_ledger(
    ...
    *,
    ...
    expected_high_water = {
        "schema": "rincode.rincodebench.provider-budget-high-water.v1",
...
def _validate_calibration_report(
    ...
    *,
    ...
    if (
        ...
        or report.get("rincode_commit") != rincode_commit
...
def _semantic_record_seal(
    ...
    *,
    ...
    return {
        "schema": (f"rincode.rincodebench.semantic-record-seal.{_semantic_version(semantic_schema)}"),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the checkout-only RinCodeBench semantic addendum.",
    )
    parser.add_argument(
        "--mode",
    ...
    try:
        ...
        parser.exit(2, f"RinCodeBench semantic addendum aborted: {exc}\n")
