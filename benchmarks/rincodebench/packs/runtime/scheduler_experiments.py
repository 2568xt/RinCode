...
SCHEDULER_EXPERIMENT_SCHEMA = "rincode.rincodebench.runtime-scheduler-experiments.v1"
...
def _make_work(
    *,
    ...
    conversation_id = f"rincodebench-scheduler:{conversation}"
    ...
    return _Work(
        request=TurnRequest(
            origin=origin,
            source=Source(
                channel="rincodebench-scheduler",
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(".rincode/evidence/rincodebench-runtime-scheduler"),
    )
...
async def _main() -> int:
    ...
    logger.disable("rincode.spine.scheduler")
    try:
        ...
        logger.enable("rincode.spine.scheduler")
