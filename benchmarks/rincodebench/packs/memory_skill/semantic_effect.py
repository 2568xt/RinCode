...
from benchmarks.rincodebench.coverage import assess_pair_coverage
from benchmarks.rincodebench.host import RecordingOutlet, RuntimeTrialHost
from benchmarks.rincodebench.isolation import TrialIsolation
...
from benchmarks.rincodebench.usage import (
    RecordingProvider,
    UsageRecorder,
    usage_scope,
)
from rincode.config.rincode import MemoryConfig, RinCodeConfig
from rincode.config.schema import Config
from rincode.context_engine.assembler import ContextAssembler
from rincode.context_engine.segments import (
    BootstrapSegmentBuilder,
    IdentitySegmentBuilder,
    MemorySegmentBuilder,
)
from rincode.context_engine.segments.curator import CuratorSegmentBuilder
from rincode.memory_engine import Memory
from rincode.plugin import PluginContext, ServiceLocator
from rincode.providers.base import LLMProvider, LLMResponse, ToolCallRequest
from rincode.spine import ChatType, Origin, Source, TurnRequest
...
_TASK_SCHEMA = "rincode.rincodebench.semantic-memory-effect-tasks.v1"
...
class ScriptedSemanticMemoryEffectRunner:
    ...
    async def run(self, context: TrialContext) -> TrialExecution:
        return await _run_semantic_memory_effect_trial(
            ...
            provider_spec={
                "mode": "scripted",
                "provider_identity": "scripted/rincodebench-semantic-memory-effect",
...
class ProductionSemanticMemoryEffectRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self._config = config
        self._rincode_config = rincode_config
    ...
    async def run(self, context: TrialContext) -> TrialExecution:
        ...
        with tempfile.TemporaryDirectory(
            prefix="rincodebench-semantic-memory-private-",
        ) as private_root:
                ...
                json.dumps(
                    {
                        ...
                        "rincode_config": self._rincode_config.model_dump(mode="json"),
                    },
                    sort_keys=True,
                ),
                encoding="utf-8",
...
async def _run_semantic_memory_effect_trial(
    ...
    *,
    ...
    isolation = TrialIsolation.create(
        (context.experiment.output_root / ".rincodebench-semantic-memory-effect" / context.experiment_id),
        attempt_id,
    )
    isolation.prepare()
    ...
    *,
    ...
    environment = {
        ...
        **(private_environment or {}),
        ...
        "RINCODE_TRACING_DIR": str(isolation.trace_root),
    }
    ...
    process = await asyncio.create_subprocess_exec(
        sys.executable,
        "-m",
        "benchmarks.rincodebench.packs.memory_skill.semantic_effect",
        "--worker-stage",
        stage,
        "--spec",
        str(spec_path),
...
class _RecordingProductionEverosBackend:
    def __init__(
        ...
        *,
        ...
        from rincode.plugin.memory.everos.backend import EverosBackend
    ...
    async def start(self) -> None:
        if self._drive_production_lifespan:
            ...
            return
        from rincode.plugin.memory.everos.backend import (
...
class _ScriptedSemanticEffectProvider(LLMProvider):
    def __init__(self, task: SemanticMemoryEffectTask) -> None:
        super().__init__(api_key="rincodebench")
        self._task = task
        self._calls = 0
        self.memory_observed = False

    def get_default_model(self) -> str:
        return "scripted/rincodebench-semantic-memory-effect"
...
async def _evaluation_stage(
    *,
    ...
    outlet = RecordingOutlet("rincodebench-semantic-memory-effect")
    conversation = f"{task.task_id}:evaluation"
    request = TurnRequest(
        origin=Origin.USER,
        source=Source(
            channel="rincodebench-semantic-memory-effect",
...
def _runtime_dependencies(
    *,
    ...
    rincode_config = _freeze_rincode_config(
        RinCodeConfig.model_validate(payload["rincode_config"]),
        config,
        task,
    )
    ledger = ProviderBudgetLedger(
    ...
    return config, rincode_config, delegate, ledger, model
...
def _freeze_rincode_config(
    rincode_config: RinCodeConfig,
    config: Config,
    task: SemanticMemoryEffectTask,
) -> RinCodeConfig:
    rincode_config.base = config
    rincode_config.memory.backend = None
    rincode_config.memory.user_id = task.workspace_id
    rincode_config.memory.agent_id = task.workspace_id
    rincode_config.memory.memory_top_k = 5
    rincode_config.skill_forge.rewrite_enabled = False
    rincode_config.skill_forge.llm_gate_enabled = False
    rincode_config.token_wise.smart_routing.enabled = False
    rincode_config.runtime.checkpoint.policy = "never"
    return rincode_config
...
def _configured_embedding_environment() -> tuple[
    ...
    from everos.config import load_settings
    from rincode.config.update_everos import get_everos_config_path
