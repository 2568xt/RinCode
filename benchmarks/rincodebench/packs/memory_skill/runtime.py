...
from benchmarks.rincodebench.usage import RecordingProvider, UsageRecorder, usage_scope
...
from rincode.memory_engine.skill_forge import (
    LocalSkillSource,
    RouterHit,
    SkillForgeRouter,
)
...
class ScriptedCrossSessionProvider(LLMProvider):
    ...
    def get_default_model(self) -> str:
        return "scripted/rincodebench-memory-skill"
...
async def run_runtime_stage(
    *,
    ...
    provider_spec = provider_spec or {
        "mode": "scripted",
        "provider_identity": "scripted/rincodebench-memory-skill",
    ...
    outlet = RecordingOutlet("rincodebench-memory-skill")
    conversation = f"{task.task_id}:{stage}"
    request = TurnRequest(
        origin=Origin.USER,
        source=Source(
            channel="rincodebench-memory-skill",
...
def _runtime_dependencies(
    *,
    ...
    if provider_spec["mode"] == "scripted":
        ...
        return (
            config,
            _freeze_rincode_config(RinCodeConfig(base=config), config, task),
            delegate,
            None,
    ...
    rincode_config = RinCodeConfig.model_validate(payload["rincode_config"])
    rincode_config = _freeze_rincode_config(rincode_config, config, task)
...
def _freeze_runtime_config(
    ...
    *,
    ...
    if stage == "learning":
        disabled_tools.extend(("write_file", "skill_read"))
