"""Historical EverOS-backed RinCodeBench path retained without a current Runtime."""
...
class RuntimeCrossProcessRunner:
    ...
    async def run(self, context: TrialContext) -> TrialExecution:
        ...
        with tempfile.TemporaryDirectory(
            prefix="rincodebench-memory-skill-provider-",
        ) as private_root:
...
async def _run_cross_process_trial(
    ...
    *,
    ...
    isolation = TrialIsolation.create(
        context.experiment.output_root / ".rincodebench-memory-skill" / context.experiment_id,
        attempt_id,
    )
    isolation.prepare()
...
async def _run_stage(
    *,
    ...
    process = await asyncio.create_subprocess_exec(
        sys.executable,
        "-m",
        "benchmarks.rincodebench.packs.memory_skill.worker",
        "--stage",
        stage,
        "--spec",
        str(spec_path),
