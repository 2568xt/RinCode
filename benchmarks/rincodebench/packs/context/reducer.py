...
from benchmarks.rincodebench.schema import JsonValue
...
_SUPPORTED_PACKS = {
    ...
    "context-expansion-20260909": (21, 7, 3),
}
_EXPANSION_PACK_ID = "context-expansion-20260909"
...
def reduce_context_artifacts(
    *,
    ...
    observed_pair_keys: set[tuple[str, int]] = set()
    ...
    for pair in pair_results:
        ...
        if pack_id == _EXPANSION_PACK_ID:
            observed_pair_keys.add((task_id, repetition))
        control = trials.get(
    ...
    trial_pack_ids = {pack_id for pack_id, _task_id, _repetition, _variant_id in trials}
    denominator_pack_ids = set(pair_pack_ids)
    if _EXPANSION_PACK_ID in trial_pack_ids:
        denominator_pack_ids.add(_EXPANSION_PACK_ID)
    expected_pairs = sum(_SUPPORTED_PACKS[pack_id][0] for pack_id in denominator_pack_ids)
    planned_pair_keys: set[tuple[str, int]] = set()
    for pack_id in denominator_pack_ids:
    ...
    strict_expansion_valid = True
    if _EXPANSION_PACK_ID in denominator_pack_ids:
        expansion_task_ids = {
            task_id for pack_id, task_id, _repetition, _variant_id in trials if pack_id == _EXPANSION_PACK_ID
        }
        expansion_planned_pair_keys = {
            (task_id, repetition)
            for task_id in expansion_task_ids
            for repetition in range(_SUPPORTED_PACKS[_EXPANSION_PACK_ID][2])
        }
        expansion_trial_pair_keys = {
            (task_id, repetition)
            for pack_id, task_id, repetition, _variant_id in trials
            if pack_id == _EXPANSION_PACK_ID
        }
        expansion_measurement_keys = {
            (measurement.task_id, measurement.repetition)
            for measurement in measurements
            if (measurement.task_id, measurement.repetition) in expansion_planned_pair_keys
        }
        if expansion_trial_pair_keys != expansion_planned_pair_keys:
            strict_expansion_valid = False
            structural_findings.append(
                "context_expansion_trial_keys_do_not_match_plan",
            )
        if observed_pair_keys != expansion_planned_pair_keys:
            strict_expansion_valid = False
            structural_findings.append(
                "context_expansion_pair_keys_do_not_match_plan",
            )
        expected_expansion_pairs = _SUPPORTED_PACKS[_EXPANSION_PACK_ID][0]
        if (
            len(measurements) != expected_expansion_pairs
            or len(valid_measurements) != expected_expansion_pairs
            or expansion_measurement_keys != expansion_planned_pair_keys
        ):
            strict_expansion_valid = False
            structural_findings.append(
                "context_expansion_measurements_incomplete",
            )
    ...
    measurement_valid = (
        coverage_valid and usage_complete and single_axis_valid and not structural_findings and strict_expansion_valid
    )
    ...
    return {
        ...
        **capability_metrics,
        "context.result_scope": (
            "exploratory_eight_task_pack"
            if pair_pack_ids == {"context"}
            else "formal_expansion_20260909"
            if denominator_pack_ids == {_EXPANSION_PACK_ID}
