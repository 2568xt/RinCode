...
class ContextPack:
    def __init__(
        ...
        *,
        ...
        tasks: Iterable[ContextTask] | None = None,
        pack_id: str | None = None,
    ) -> None:
        self.track = ContextTrack(track)
        self._runner = runner
        loaded_tasks = tuple(load_context_tasks(self.track) if tasks is None else tasks)
        if not loaded_tasks:
            raise ValueError("Context pack requires at least one task")
        if any(task.track is not self.track for task in loaded_tasks):
            raise ValueError("Context pack tasks must use the selected track")
        if len({task.task_id for task in loaded_tasks}) != len(loaded_tasks):
            raise ValueError("Context pack tasks must have unique task IDs")
        self._tasks = {task.task_id: task for task in loaded_tasks}
        default_pack_id = "context" if self.track is ContextTrack.FORMAL else "context-calibration"
        self._pack_id = pack_id or default_pack_id
        self._task_set_digest = (
            context_task_set_digest(self.track)
            if tasks is None
        ...
        return PackDefinition(
            ...
            identity={
                ...
                "result_scope": (
                    ...
                    if self._pack_id == "context"
                    else "calibration_only"
                    if self._pack_id == "context-calibration"
