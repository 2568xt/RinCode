    ...
    async def __call__(
        ...
        *,
        ...
        isolation = TrialIsolation.create(
            context.experiment.output_root / ".rincodebench-context" / context.experiment_id,
            attempt_id,
        )
        isolation.prepare()
        config, rincode_config = _trial_configs(
            self._config,
            self._rincode_config,
        ...
        outlet = RecordingOutlet("rincodebench-context")
...
def _trial_configs(
    ...
    *,
    ...
    trial_rincode = rincode_config.model_copy(deep=True)
    trial_rincode.base = trial_config
    trial_rincode.context.curator_model = model
    trial_rincode.context.protect_first_n = CONTEXT_BENCHMARK_PROTECT_FIRST_N
    trial_rincode.memory.backend = None
    trial_rincode.skill_forge.enabled = False
    trial_rincode.skill_forge.router.enabled = False
    trial_rincode.skill_forge.rewrite_enabled = False
    trial_rincode.skill_forge.llm_gate_enabled = False
    trial_rincode.token_wise.smart_routing.enabled = False
    trial_rincode.runtime.checkpoint.policy = "never"
    return trial_config, trial_rincode
...
def _conversation_id(task: ContextTask, context: TrialContext) -> str:
    return (
        f"rincodebench-context:{task.task_id}-{context.variant.variant_id}-"
...
def _turn_request(
    ...
    return TurnRequest(
        origin=Origin.USER,
        source=Source(
            channel="rincodebench-context",
            chat_id=conversation.removeprefix("rincodebench-context:"),
