...
from rincode.config.schema import Config, MCPServerConfig
from rincode.providers.base import (
    ErrorClassification,
    GenerationSettings,
    LLMProvider,
    LLMResponse,
...
class _ScriptedToolMCPProvider(LLMProvider):
    ...
    def get_default_model(self) -> str:
        return "rincodebench/tool-mcp-deterministic"
...
async def _run_runtime_trial(
    *,
    ...
    disclosure = str(context.variant.settings["tool_disclosure"])
    isolation_root = context.experiment.output_root / ".rincodebench-tool-mcp" / context.experiment_id
    ...
    config, rincode_config = _runtime_config(
        workspace=isolation.workspace,
        receipt_path=receipt_path,
        provider=provider,
        progressive=disclosure == "progressive_disclosure",
    )
    outlet = RecordingOutlet("rincodebench-tool-mcp")
    started = time.perf_counter()
    observation = None
    ...
    with patch.dict("os.environ", environment, clear=False):
        ...
        try:
            ...
            catalog_payloads = [
                ...
                if definition["function"]["name"].startswith("mcp_rincodebench_catalog_probe_")
            ]
        finally:
            await host.close()
    latency_ms = (time.perf_counter() - started) * 1_000
...
async def run_mcp_transport_smoke(root: Path) -> MCPTransportSmokeResult:
    ...
    try:
        await connect_mcp_servers(
            {"rincodebench": _mcp_server_config(receipt_path)},
            registry,
            stack,
        )
...
def _runtime_config(
    *,
    ...
    config.tools.mcp_servers = {"rincodebench": _mcp_server_config(receipt_path)}
...
def _mcp_server_config(receipt_path: Path) -> MCPServerConfig:
    ...
    return MCPServerConfig(
        ...
        env={"RINCODEBENCH_MCP_RECEIPTS": str(receipt_path)},
        tool_timeout=10,
    )
...
def _turn_request(task: ToolMCPTask) -> TurnRequest:
    return TurnRequest(
        origin=Origin.USER,
        source=Source(
            channel="rincodebench-tool-mcp",
            chat_id=task.task_id,
            sender_id="rincodebench",
            chat_type=ChatType.DM,
        ),
        text=task.prompt,
        message_id=f"{task.task_id}-message",
        conversation=f"rincodebench-tool-mcp:{task.task_id}",
    )
...
def _aggregate_usage(
    ...
    records = [record for record in records if record.get("provider_dispatched", True)]
