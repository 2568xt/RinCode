...
_PACK_POLICIES = {
    ...
    "tool-mcp-expansion-20260909": (21, 7, 3),
}
_EXPANSION_PACK_ID = "tool-mcp-expansion-20260909"
...
def reduce_tool_mcp_claim_from_artifacts(
    *,
    ...
    denominator_pack_ids = set(selected_pack_ids)
    if _EXPANSION_PACK_ID in trial_pack_ids:
        denominator_pack_ids.add(_EXPANSION_PACK_ID)
    expected_pairs = sum(_PACK_POLICIES[pack_id][0] for pack_id in denominator_pack_ids)
    ...
    if _EXPANSION_PACK_ID in denominator_pack_ids:
        ...
        expansion_planned_pair_keys = {
            ...
            for repetition in range(_PACK_POLICIES[_EXPANSION_PACK_ID][2])
        }
        ...
        if expansion_trial_pair_keys != expansion_planned_pair_keys:
            ...
            structural_findings.append(
                "tool_mcp_expansion_trial_keys_do_not_match_plan",
            )
        if observed_pair_keys != expansion_planned_pair_keys:
            strict_expansion_valid = False
            structural_findings.append(
                "tool_mcp_expansion_pair_keys_do_not_match_plan",
            )
        expected_expansion_pairs = _PACK_POLICIES[_EXPANSION_PACK_ID][0]
        if (
            ...
            strict_expansion_valid = False
            structural_findings.append(
                "tool_mcp_expansion_measurements_incomplete",
            )
    ...
    measurement_valid = coverage_valid and not structural_findings and strict_expansion_valid
