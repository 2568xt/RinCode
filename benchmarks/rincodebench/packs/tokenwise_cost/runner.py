...
from rincode.call_efficiency.models import CallRecord
...
from rincode.providers.base import GenerationSettings, LLMResponse
...
def load_deepseek_key(*, config_path: Path | None = None) -> str:
    ...
    path = config_path or Path.home() / ".rincode" / "config.json"
...
async def run_formal_campaign(
    *,
    ...
    if len(rincode_commit) != 40:
        raise CampaignError("rincode_commit must be a full Git commit")
    ...
    manifest = {
        "schema": "rincode.rincodebench.call-efficiency-cost.manifest.v2",
    ...
    report["campaign"]["rincode_commit"] = rincode_commit
...
def verify_retained_campaign(
    *,
    ...
    checks = {
        "manifest_schema": manifest.get("schema") == "rincode.rincodebench.call-efficiency-cost.manifest.v2",
        "manifest_digest": expected_plan_digest == canonical_digest(manifest_payload),
        "rincode_commit": manifest.get("rincode_commit") == rincode_commit,
    ...
    checks["raw_trial_schema"] = all(
        row.get("schema") == "rincode.rincodebench.call-efficiency-cost.trial.v2" for row in raw_rows
    )
    ...
    rebuilt["campaign"]["rincode_commit"] = rincode_commit
    ...
    verifier = {
        "schema": "rincode.rincodebench.call-efficiency-cost.verifier.v1",
        "checks": checks,
    ...
    _write_json(
        ...
        {
            "schema": "rincode.rincodebench.call-efficiency-cost.claim.v1",
...
async def run_cache_preflight(
    *,
    ...
    report = {
        "schema": "rincode.rincodebench.tokenwise-cost.preflight.v1",
        "model": config.model,
        "price_snapshot": PRICE_SNAPSHOT,
        "checks": checks,
...
async def execute_live_trial(
    ...
    *,
    ...
    rincode_config = RinCodeConfig(base=runtime_config)
    ...
    rincode_config.call_efficiency.mode = "observe"
    rincode_config.call_efficiency.usage_tracking = True
    rincode_config.context.fast_path_threshold = 1.0
...
def _trial_payload(artifact: TrialArtifact, *, plan_digest: str) -> dict[str, Any]:
    return {
        "schema": "rincode.rincodebench.call-efficiency-cost.trial.v2",
        "plan_digest": plan_digest,
...
def _build_inventory(root: Path) -> dict[str, Any]:
    ...
    payload = {
        "schema": "rincode.rincodebench.call-efficiency-cost.inventory.v1",
        "artifacts": artifacts,
    }
