...
from benchmarks.rincodebench.canonical import canonical_bytes, canonical_digest
...
from rincode.spine import ChatType, Origin, Source, Text, TurnRequest
from rincode.tracing import spans as tracing_spans

MANIFEST_SCHEMA = "rincode.rincodebench.tracing-overhead.manifest.v1"
BLOCK_SCHEMA = "rincode.rincodebench.tracing-overhead.block.v1"
AGGREGATE_SCHEMA = "rincode.rincodebench.tracing-overhead.aggregate.v1"
...
async def run_campaign(
    *,
    ...
    manifest = _manifest(rincode_commit, config)
    ...
    aggregate["rincode_commit"] = rincode_commit
    ...
    return verify_campaign(
        output_root=output_root,
        expected_rincode_commit=rincode_commit,
    )


def verify_campaign(*, output_root: Path, expected_rincode_commit: str) -> dict[str, Any]:
    ...
    checks = {
        ...
        "rincode_commit": manifest.get("rincode_commit") == expected_rincode_commit,
    ...
    rebuilt["rincode_commit"] = expected_rincode_commit
    ...
    verifier = {
        "schema": "rincode.rincodebench.tracing-overhead.verifier.v1",
        "checks": checks,
    ...
    _write_json(
        ...
        {
            "schema": "rincode.rincodebench.tracing-overhead.claim.v1",
...
async def _run_arm(
    *,
    ...
    previous_tracing = os.environ.get("RINCODE_TRACING")
    previous_dir = os.environ.get("RINCODE_TRACING_DIR")
    os.environ["RINCODE_TRACING"] = "1" if enabled else "0"
    os.environ["RINCODE_TRACING_DIR"] = str(trace_dir)
    tracing_spans._store = None
    try:
        with tempfile.TemporaryDirectory(prefix="rincode-tracing-benchmark-") as tmp:
            root = Path(tmp)
        ...
        tracing_spans._store = None
        _restore_env("RINCODE_TRACING", previous_tracing)
        _restore_env("RINCODE_TRACING_DIR", previous_dir)
...
def _assemble_runtime(root: Path, provider: _TracingProvider):
    ...
    rincode_config.skill_forge.enabled = False
    rincode_config.skill_forge.router.enabled = False
    rincode_config.skill_forge.rewrite_enabled = False
    rincode_config.skill_forge.llm_gate_enabled = False
    rincode_config.call_efficiency.mode = "off"
    rincode_config.call_efficiency.enabled = False
    rincode_config.call_efficiency.usage_tracking = False
...
def _manifest(rincode_commit: str, config: TracingExperimentConfig) -> dict[str, Any]:
    if len(rincode_commit) != 40:
        raise TracingExperimentError("rincode_commit must be a full Git commit")
    payload = {
...
def _inventory(output_root: Path) -> dict[str, Any]:
    ...
    payload = {
        "schema": "rincode.rincodebench.tracing-overhead.inventory.v1",
        "files": files,
    }
...
def _current_commit() -> str:
    ...
    if git is None:
        raise TracingExperimentError("git is required to freeze the RinCode commit")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(".rincode/evidence/tracing-overhead-current"),
    )
    parser.add_argument("--rincode-commit")
...
async def _main() -> int:
    ...
    commit = args.rincode_commit or _current_commit()
    if args.mode == "plan":
        result = _manifest(commit, config)
        result["pairs"] = config.pairs
        result["turns"] = config.pairs * 2
    elif args.mode == "run":
        result = await run_campaign(output_root=args.output_root, rincode_commit=commit, config=config)
    else:
        result = verify_campaign(output_root=args.output_root, expected_rincode_commit=commit)
