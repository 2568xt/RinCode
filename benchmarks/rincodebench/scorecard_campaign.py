...
_RUNTIME_SUBJECT_PATHS = (
    ...
    "pyproject.toml",
    "uv.lock",
    "benchmarks/rincodebench/packs/runtime",
)
...
def reused_runtime_gate(
    ...
    *,
    ...
    evidence_commit = str(evidence.get("rincode_commit", ""))
    if (
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the current RinCodeBench Scorecard campaign.",
    )
    parser.add_argument(
        "action",
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(".rincode") / "evidence" / "rincodebench-scorecard-v1",
    )
    ...
    try:
        ...
        parser.exit(2, f"RinCodeBench Scorecard aborted: {exc}\n")
    print(
        json.dumps(
