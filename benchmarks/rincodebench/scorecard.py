...
def _formal_inputs(
    ...
    *,
    ...
    if (
        ...
        or identity.get("rincode_commit") != rincode_commit
...
def _current_rincode_commit() -> str:
    ...
    if git is None:
        raise RuntimeError("git is required to resolve the RinCode commit")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compute the RinCodeBench multidimensional diagnostic score.",
    )
    ...
    current_commit = _current_rincode_commit()
    formal_summary = _formal_inputs(
        args.formal_summary,
        rincode_commit=current_commit,
    )
...
def _runtime_inputs(
    ...
    *,
    ...
    commit_key = "rincode_commit" if schema == "rincode.rincodebench.runtime-evidence.v2" else "source_commit"
    if schema not in {
        "rincode.rincodebench.runtime-evidence.v2",
        "rincode.rincodebench.runtime-scheduler-experiments.v1",
        "rincode.rincodebench.runtime-live-scheduler.v2",
    }:
...
def _tokenwise_inputs(
    ...
    *,
    ...
    if report.get("schema") != "rincode.rincodebench.tokenwise-cost.report.v1":
        raise ValueError("TokenWise report has the wrong schema")
    campaign = _mapping(report.get("campaign"), "TokenWise campaign")
    if campaign.get("rincode_commit") != rincode_commit:
        raise ValueError("TokenWise report does not match the current RinCode commit")
...
def _verify_current_digest_bound_evidence(
    ...
    *,
    ...
    if evidence.get(commit_key) != rincode_commit:
        raise ValueError("evidence does not match the current RinCode commit")
