...
RinCodeBench is RinCode's checkout-only evaluation campaign. It runs the real RinCode
Runtime through frozen, single-axis comparisons and rebuilds every result from
terminal artifacts. It is not part of the `rincode` wheel and does not add a
public `rincode bench` command.
...
The paid command uses the normal RinCode configuration, but only proceeds when it
...
```bash
RINCODE_BENCH_EXECUTE_PAID=1 make rincodebench-reproduce
```
...
`.rincode/evidence/rincodebench-reproduction/<rincode-commit>/`. Repeating the command
...
```bash
RINCODE_SCORECARD_FORMAL_SUMMARY=/absolute/path/to/summary.json \
RINCODE_SCORECARD_RUNTIME_EVIDENCE=/absolute/path/to/runtime-evidence.json \
RINCODE_SCORECARD_TOKENWISE_REPORT=/absolute/path/to/tokenwise-report.json \
  make rincodebench-reproduce
```
...
```bash
make rincodebench-scorecard-estimate
```

Run the paid campaign with:

```bash
make rincodebench-scorecard-ship
```

Runtime evidence is not required to measure the independent Context and
Tool/MCP Packs. When `RINCODE_SCORECARD_RUNTIME_EVIDENCE` is supplied, the
campaign verifies that its RinCode product code, dependency identity, and Runtime
...
RINCODE_SCORECARD_RUNTIME_EVIDENCE=/absolute/path/to/runtime-evidence.json \
RINCODE_SCORECARD_TOKENWISE_REPORT=/absolute/path/to/tokenwise-report.json \
RINCODE_SCORECARD_MEMORY_SUMMARY=/absolute/path/to/memory-summary.json \
RINCODE_SCORECARD_MEMORY_HANDOFF=/absolute/path/to/memory-handoff.json \
RINCODE_SCORECARD_PREREGISTERED=1 \
  make rincodebench-scorecard-score
```

Runtime, Memory, and preregistration inputs are optional. The Memory summary
is accepted only with a digest-bound handoff that names the current RinCode
...
The suite at
`benchmarks/rincodebench/suites/agent_application_ship_1.yaml` freezes:
...
Before any paid call, RinCodeBench includes existing ledger spend and two live
...
`.rincode/evidence/rincodebench/` and are not committed.
