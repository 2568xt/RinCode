"""
RinCode Agent Executor - replaces OpenClaw CLI subprocess calls.

Drives RinCode's AgentLoop.run_turn() programmatically to execute
...
DEFAULT_PROVIDER = os.environ.get("RINCODE_BENCH_PROVIDER", "custom")
DEFAULT_MODEL = os.environ.get("RINCODE_BENCH_MODEL", "deepseek-v4-flash")
...
def _session_to_openclaw_transcript(
    ...
    """
    Convert RinCode session messages (OpenAI format) to PinchBench/OpenClaw
...
async def execute_task(
    ...
    """
    Execute a single benchmark task using RinCode's AgentLoop.
    ...
    agent = AgentLoop(
        ...
        # 可注入恢复信息的渠道，也不希望任务工作区出现 ``.rincode/shadow.git``。
        interactive=False,
    )
