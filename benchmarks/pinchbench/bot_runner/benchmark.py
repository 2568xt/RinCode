...
Runs a full RinCode bot (AgentLoop run_turn) for each task,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PinchBench for RinCode (bot mode)")
    parser.add_argument(
        "--model",
...
async def run_benchmark(args: argparse.Namespace) -> None:
    ...
    logger.info("=" * 70)
    logger.info("  PinchBench for RinCode (BOT MODE)")
    ...
    run_root = Path("/tmp/pinchbench-rincode-bot")
