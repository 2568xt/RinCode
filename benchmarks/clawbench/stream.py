#!/usr/bin/env python3
"""Run RinCode on ClawBench as one persistent streaming session.
...
class RinCodeSession:
    def __init__(
        ...
        *,
        ...
        from rincode.cli.commands import _make_provider
        from rincode.config.loader import load_config, set_config_path
        ...
        self.agent = AgentLoop(
            ...
            # 注入恢复信息的渠道，也不希望任务工作区出现 ``.rincode/shadow.git``。
            interactive=False,
        )
    ...
    *,
    ...
    try:
        final_text, token_stats = await asyncio.wait_for(
            rincode.run(prepared.prompt, task_id=task.id),
...
def write_markdown(
    ...
    lines = [
        "# ClawBench RinCode Streaming Results",
        "",
        f"- Date: {now_iso()}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run RinCode against ClawBench sequentially with one persistent session.")
    ...
    parser.add_argument("--session-id", default="clawbench-stream-rincode")
    parser.add_argument("--trace-dir", default=str(DEFAULT_TRACE_DIR))
    parser.add_argument("--config", default="", help="RinCode config path")
    parser.add_argument("--model", default=os.environ.get("RINCODE_BENCH_MODEL", ""))
    parser.add_argument("--provider", default=os.environ.get("RINCODE_BENCH_PROVIDER", ""))
...
async def amain() -> int:
    ...
    try:
        for index, task in enumerate(tasks, start=1):
            ...
            write_markdown(results_md, results, args, rincode.model, summary_path)
    finally:
        await rincode.close()

    summary_path = trace_dir / f"rincode_clawbench_stream_{run_stamp}.json"
    summary = {
    ...
    tokens_csv = trace_dir / f"rincode_clawbench_stream_{run_stamp}.tokens.csv"
