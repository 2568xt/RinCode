...
This directory contains a RinCode-only ClawBench runner. It executes tasks
sequentially in one persistent RinCode session:
...
4. repeat for task 2 without clearing the RinCode session.
...
Install RinCode normally, then configure a model. The benchmark runners use
...
You can also put the same values in `~/.rincode/config.json`:
...
- `rincode_clawbench_stream_<timestamp>.json` - final summary;
- `rincode_clawbench_stream_<timestamp>.tokens.csv` - per-task token records;
