...
from the runtime package. They are not imported by `rincode/` and are
excluded from the wheel build — keep it that way.

The Markdown task cards under `pinchbench/tasks/` are executable benchmark
fixtures, not RinCode product documentation. Some intentionally probe capabilities
removed from RinCode, including image generation or remote Skill discovery. A
...
RinCodeBench Ship-1 is the checkout-only Agent application evaluation harness
under `benchmarks/rincodebench/`. Its contract is documented under
...
RinCodeBench evidence remains outside Git. Ship Completeness and Measurement
...
RinCodeBench, and V-R0 remain distinct scopes.
...
See [RinCodeBench](rincodebench/README.md) for the published experiment boundary.
...
│   └── evolve/            rincode.evolver BenchBundle plugin (entry.py)
...
│
├── rincodebench/          Agent application evaluation harness
...
The benchmark runners can use the normal `~/.rincode/config.json`, or the
...
export RINCODE_BENCH_PROVIDER="custom"
export RINCODE_BENCH_MODEL="deepseek-v4-flash"
```
...
Equivalent `~/.rincode/config.json`:
...
./benchmarks/clawbench/run.sh \
    ...
    --limit 80 \
    --session-id clawbench-stream-rincode-80 \
    --max-iterations 40
```
...
./benchmarks/clawbench/run.sh \
    --clawbench-root "$CLAW_BENCH_ROOT" \
    --limit 80 \
    --session-id clawbench-stream-rincode-curator-80 \
...
The runtime (`rincode/`) **never statically imports from `benchmarks/`** — this
is the "independent eval track" principle. The reverse is allowed and
expected: benchmarks import `rincode.agent`, `rincode.providers`, etc. directly.

One scoped exception: `rincode.evolver` loads its bench *plugins* from here by
