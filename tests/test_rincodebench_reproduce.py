...
from benchmarks.rincodebench.reproduce import (
    ReproductionConfig,
    ReproductionReport,
    StageResult,
    render_report,
...
from benchmarks.rincodebench.scorecard import DimensionScore, ScorecardResult
from rincode.utils.portable_lock import file_lock
...
def test_reproduction_report_renders_score_dimensions_and_artifact_path(tmp_path: Path) -> None:
    report = ReproductionReport(
        ...
        score=ScorecardResult(
            schema="rincode.rincodebench.multidimensional-score.v1",
...
def test_reproduction_reuses_current_evidence_without_running_commands(tmp_path: Path) -> None:
    ...
    _write_formal_summary(formal_summary, rincode_commit=rincode_commit)
    runtime_evidence = inputs / "runtime-evidence.json"
    runtime_payload = {
        "schema": "rincode.rincodebench.runtime-scheduler-experiments.v1",
        "source_commit": rincode_commit,
        "claim_eligible": True,
    }
    ...
    tokenwise_payload = {
        "schema": "rincode.rincodebench.tokenwise-cost.report.v1",
        "campaign": {"rincode_commit": rincode_commit},
    ...
    _write_formal_summary(formal_summary, rincode_commit="b" * 40)
...
def test_reproduction_runs_all_missing_stages_and_composes_score(tmp_path: Path) -> None:
    ...
    def execute(command: tuple[str, ...]) -> str:
        ...
        if "scheduler_experiments" in joined:
            ...
            path = output_root / rincode_commit / "plan" / "runs" / "scheduler-experiments.test.json"
...
def test_makefile_exposes_single_reproduction_command() -> None:
    ...
    assert "rincodebench-reproduce:" in makefile
    assert "python -m benchmarks.rincodebench.reproduce" in makefile
...
def _write_campaign_artifacts(
    ...
    *,
    ...
    outcome_root = output_root / "campaigns" / "suite" / rincode_commit / "ship"
...
def _write_formal_summary(
    ...
    *,
    ...
    spec = {
        "schema": "rincode.rincodebench.experiment.v1",
        "evidence_schema": "rincode.rincodebench.evidence.v1",
        "identity": {
