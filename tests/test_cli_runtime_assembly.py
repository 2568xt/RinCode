...
from rincode.cli._gateway_spine import GatewayTurnRunner
...
from rincode.tui_rpc.spine import TuiTurnRunner
...
def _runtime_configs(tmp_path):
    ...
    rincode_config = SimpleNamespace(
...
def test_runtime_assembly_preserves_config_and_plugin_parity(
    ...
    config, rincode_config = _runtime_configs(tmp_path)
    provider = object()
    cron_service = object()
    ...
    monkeypatch.setattr(
        "rincode.session.manager.SessionManager",
    ...
    monkeypatch.setattr("rincode.cli._plugin_stack.maybe_build_memory_backend", _build_backend)
    monkeypatch.setattr("rincode.cli._plugin_stack.build_plugin_tools", _build_tools)
    monkeypatch.setattr(
        "rincode.call_efficiency.CallEfficiency.from_config",
    ...
    assert captured == {
        ...
        "skill_forge_config": rincode_config.skill_forge,
...
def test_runtime_assembly_separates_workspace_from_state(
    ...
    config, rincode_config = _runtime_configs(tmp_path / "configured")
    paths = RuntimePaths(
        workspace=tmp_path / "project",
        state=tmp_path / "project" / ".rincode",
    )
    captured: dict = {}
    calls: list[tuple[str, object]] = []
