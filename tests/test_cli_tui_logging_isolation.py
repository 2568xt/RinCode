...
from rincode.cli._log_file import redirect_loguru_to_file  # noqa: E402
...
def _isolate_logging(tmp_path, monkeypatch):
    ...
    (b) write to the real ``~/.rincode/logs/tui.log``;
...
def test_propagation_to_root_intact_after_redirect(_isolate_logging):
    ...
    would silently drop the records from `~/.rincode/logs/tui.log`. The fix
