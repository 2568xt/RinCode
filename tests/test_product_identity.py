...
def test_python_distribution_exposes_only_the_rincode_namespace() -> None:
    import rincode

    assert rincode.__version__ != "0.0.0+unknown"
...
def test_public_distribution_cli_and_plugin_identity() -> None:
    ...
    assert metadata["name"] == "rincode-harness"
    assert metadata["scripts"] == {"rincode": "rincode.cli.commands:run"}

    runner = CliRunner()
    help_result = runner.invoke(app, ["--help"])
    version_result = runner.invoke(app, ["--version"])
    assert help_result.exit_code == 0
    assert "RinCode" in help_result.stdout
    assert version_result.exit_code == 0
    assert "RinCode v" in version_result.stdout

    assert not (repo_root / "rincode" / "plugin" / "memory" / "everos").exists()
    assert not (repo_root / "rincode" / "config" / "update_everos.py").exists()
...
def test_retired_memory_implementation_is_absent_from_executable_surfaces() -> None:
    ...
    assert "RINCODE_SCORECARD_MEMORY_" not in makefile
    assert "--memory-summary" not in makefile
    assert "--memory-handoff" not in makefile


def test_rincode_only_imports_myna_through_its_public_plugin_surface() -> None:
    repo_root = Path(__file__).resolve().parents[1]
    violations: list[str] = []
    for root in (repo_root / "rincode", repo_root / "scripts"):
        for path in root.rglob("*.py"):
            tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if alias.name.startswith("myna.") and alias.name != "myna.integrations.rincode":
                            violations.append(f"{path.relative_to(repo_root)}:{node.lineno}:{alias.name}")
                elif isinstance(node, ast.ImportFrom):
                    module = node.module or ""
                    imported = {alias.name for alias in node.names}
                    public_integration = module == "myna.integrations.rincode" and imported <= {
                        "descriptor",
                        "open_rincode_integration",
                    }
...
def test_clean_defaults_use_rincode_state(tmp_path: Path, monkeypatch) -> None:
    ...
    assert loader.get_config_path() == tmp_path / ".rincode" / "config.json"
    assert loader.load_config().agents.defaults.workspace == "~/.rincode/workspace"
    assert get_workspace_path() == tmp_path / ".rincode" / "workspace"
...
def test_foreground_runtime_uses_current_directory_without_polluting_it(
    ...
    project.mkdir()
    monkeypatch.setenv("RINCODE_HOME", str(tmp_path / "rincode-home"))
    monkeypatch.chdir(project)

    paths = resolve_foreground_paths(Config())

    assert paths.workspace == project.resolve()
    assert paths.state.parent == tmp_path / "rincode-home" / "projects"
...
def test_project_state_identity_is_stable_and_workspace_specific(
    ...
    assert get_project_state_dir(first).parent == tmp_path / "rincode-home" / "projects"
...
def test_service_runtime_keeps_configured_workspace(
    ...
    from rincode.config.paths import resolve_service_paths
    from rincode.config.schema import Config
...
def test_rincode_home_scopes_global_runtime_state(tmp_path: Path, monkeypatch) -> None:
    ...
    from rincode.config import loader
    from rincode.config.paths import get_cli_history_path, get_cron_dir, get_workspace_path
    from rincode.plugin import PluginDiscovery
    from rincode.routing.cache import BenchmarkCache
    from rincode.routing.knn_router import KNNModelRouter
    from rincode.token_wise.usage_tracker import UsageTracker
    from rincode.tracing.config import state_dir
    ...
    assert get_cli_history_path() == product_home / ".rincode_history"
    ...
    assert sources["entry_points_group"] == "rincode.plugins"
...
def test_blank_rincode_home_uses_default_state_root(tmp_path: Path, monkeypatch) -> None:
    from rincode.product import get_product_home

    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("RINCODE_HOME", "  ")

    assert get_product_home() == tmp_path / ".rincode"


@pytest.mark.asyncio
async def test_checkpoint_uses_rincode_state(tmp_path: Path) -> None:
    ...
    assert (workspace / ".rincode" / "shadow.git" / "HEAD").is_file()
