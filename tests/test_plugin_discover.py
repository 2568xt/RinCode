...
def _write_manifest(root: Path, plugin_id: str, *, extra: str = "") -> Path:
    """Drop a minimal valid manifest at ``root/<plugin_id>/rincode-plugin.toml``."""
    sub = root / plugin_id
...
class TestSingleSource:
    ...
    def test_finds_single_manifest(self, tmp_path: Path) -> None:
        ...
        assert out[0].location.name == "rincode-plugin.toml"
    ...
    def test_malformed_manifest_skipped_silently(
        ...
        sub = tmp_path / "broken"
        sub.mkdir()
        (sub / "rincode-plugin.toml").write_text(
            "not valid toml [[[",
            encoding="utf-8",
        )
    ...
    def test_incompatible_manifest_fails_closed(self, tmp_path: Path) -> None:
        _write_manifest(tmp_path, "future", extra='rincode = ">=0.2,<0.3"')

        with pytest.raises(PluginCompatibilityError, match=r"requires RinCode >=0.2,<0.3.*installed RinCode is 0.1.7"):
            PluginDiscovery(bundled_dir=tmp_path, rincode_version="0.1.7").discover()


class TestEntryPointIdentity:
    @staticmethod
    def _entry_point(tmp_path: Path, *, plugin_id: str = "myna-memory", version: str = "0.1.1rc3"):
        relative = PurePosixPath("myna/integrations/rincode/rincode-plugin.toml")
        ...
        manifest.write_text(
            textwrap.dedent(f"""
                ...
                name = "myna"
                factory = "myna.integrations.rincode:make_backend"
            """),
            encoding="utf-8",
        )
        ...
        return SimpleNamespace(
            name="myna",
            value="myna.integrations.rincode",
            dist=distribution,
        )
    ...
    def test_reads_installed_manifest_without_importing_plugin_package(
        ...
        discovered = PluginDiscovery(entry_points_group="rincode.plugins", rincode_version="0.1.7").discover()

        assert discovered[0].manifest.id == "myna-memory"
        assert discovered[0].manifest.version == "0.1.1rc3"
        assert discovered[0].manifest.rincode == ">=0.1,<0.2"
    ...
    def test_distribution_and_manifest_identity_must_match(
        ...
        with pytest.raises(PluginIdentityError, match=message):
            PluginDiscovery(entry_points_group="rincode.plugins", rincode_version="0.1.7").discover()
