...
from rincode.evolver.applier.path_guard import (
    ImmutablePathError,
    assert_patch_allowed,
    check_patch_paths,
)
from rincode.evolver.candidate_manifest import (
    LABEL_POLICIES,
    ActivationPolicy,
    CandidateLabel,
    CandidateManifest,
...
from rincode.evolver.judge.schema import PatchWhere, PatchWhy
from rincode.evolver.orchestrator.production import make_git_commit_apply_fn
from rincode.evolver.tree.node import AppliedPatch, HarnessNode, PatchComponent
from rincode.evolver.tree.store import EvolverTreeStore
...
def _candidate_repo(tmp_path: Path) -> tuple[Path, str]:
    ...
    (repo / "rincode/evolver").mkdir(parents=True)
    (repo / "rincode/evolver/kernel.py").write_text("SEALED = True\n")
    (repo / "mutable/real").mkdir(parents=True)
    (repo / "mutable/link").symlink_to("real", target_is_directory=True)
    _git(repo, "init", "-q")
    env = {
        **os.environ,
        "GIT_AUTHOR_NAME": "RinCode Test",
        "GIT_AUTHOR_EMAIL": "rincode-test@example.invalid",
        "GIT_COMMITTER_NAME": "RinCode Test",
        "GIT_COMMITTER_EMAIL": "rincode-test@example.invalid",
    }
...
def test_manifest_gate_rejects_tampered_patch_metadata() -> None:
    patch = _patch(PatchWhere.loop_override, "benchmarks/appworld/agent_cli.py")
    manifest = _manifest("candidate-1", CandidateLabel.runtime, patch)
    tampered = replace(manifest, target_files=("rincode/templates/AGENTS.md",))
...
def test_supported_label_requires_executable_evaluator_binding(
    ...
    from rincode.evolver import candidate_evidence
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            PatchWhere.skill,
            "rincode/memory_engine/skills/example/SKILL.md",
        ),
        (
...
def test_model_profile_and_route_are_config_only_and_never_weights(label: CandidateLabel) -> None:
    code_patch = _patch(PatchWhere.config, "rincode/routing/profiles.py")
...
def test_runtime_requires_human_review_and_exact_mutable_surface() -> None:
    ...
    outside = _patch(PatchWhere.loop_override, "rincode/agent/loop/recovery.py")
...
@pytest.mark.parametrize(
    ...
    [
        "../rincode/templates/AGENTS.md",
        "rincode/templates/../../pyproject.toml",
        "/tmp/candidate.py",
        r"C:\tmp\candidate.py",
...
def test_path_guard_rejects_symlink_components(tmp_path: Path) -> None:
    (tmp_path / "rincode").mkdir()
    (tmp_path / "outside").mkdir()
    (tmp_path / "rincode" / "templates").symlink_to(tmp_path / "outside", target_is_directory=True)
    target = "rincode/templates/AGENTS.md"
...
@pytest.mark.parametrize(
    "target",
    [
        "rincode/evolver/candidate_manifest.py",
        "rincode/eval_engine/engine.py",
...
def test_g5_rejection_creates_no_candidate_commit(tmp_path: Path) -> None:
    repo, parent_sha = _candidate_repo(tmp_path)
    candidate = Candidate(
        files={"rincode/evolver/kernel.py": b"SEALED = False\n"},
        why="unsafe_fixture",
