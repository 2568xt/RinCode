...
def _patch_agent_loop_to_raise(monkeypatch: pytest.MonkeyPatch, exc: BaseException) -> None:
    ...
    ``rincode.agent.loop`` inside the function body, so we patch the module
    ...
    monkeypatch.setattr("rincode.cli._helpers.make_lazy_provider", lambda config: object())
    monkeypatch.setattr(
        "rincode.config.rincode.load_rincode_config",
        lambda: RinCodeConfig(memory=MemoryConfig(backend=None)),
    )
    monkeypatch.setattr("rincode.agent.loop.AgentLoop", _Boom)
...
def test_typeerror_in_build_raises_internal_error_minus_32603(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    points to ~/.rincode/logs/tui.log so the UI can hint the user.
    """
...
def test_attributeerror_in_build_raises_internal_error_minus_32603(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    _patch_agent_loop_to_raise(
        monkeypatch,
        AttributeError("'RinCodeConfig' object has no attribute 'foo'"),
    )
