...
def test_gateway_help_works() -> None:
    """``rincode gateway --help`` lists the documented options."""
    r = runner.invoke(app, ["gateway", "--help"])
    assert r.exit_code == 0
    assert "Start the RinCode gateway" in r.stdout
...
def test_gateway_validates_channels_before_shared_runtime_assembly(
    ...
    from rincode.cli import _runtime_assembly, gateway_commands
    ...
    monkeypatch.setattr(
        "rincode.cli._log_file.redirect_loguru_to_file",
        lambda *args, **kwargs: tmp_path / "gateway.log",
    )
    monkeypatch.setattr(
        "rincode.cli._gateway_lock.acquire",
        lambda **kwargs: object(),
    )
    monkeypatch.setattr(
        "rincode.config.rincode.load_rincode_config",
        lambda: rincode_config,
    )
    monkeypatch.setattr(
        "rincode.config.paths.get_cron_dir",
        lambda: tmp_path / "cron",
    )
    monkeypatch.setattr(
        "rincode.proactive_engine.schedulers.cron.service.CronService",
    ...
    monkeypatch.setattr(
        "rincode.channels.manager.ChannelManager",
        _build_channels,
    )
    ...
    if channel_validation_fails:
        return
    assert calls[0][0] == (config, rincode_config)
    assert calls[0][1] == {
        "provider": provider,
...
def test_gateway_refuses_second_instance(tmp_config: Path, monkeypatch) -> None:
    ...
    from rincode.cli import _gateway_lock
...
async def test_gateway_quiesces_intake_before_spine_and_stops_transports_afterward() -> None:
    from rincode.cli.gateway_commands import _cleanup_gateway
...
async def test_gateway_cancellation_at_intake_barrier_finishes_barrier_before_spine() -> None:
    from rincode.channels.intake import Intake
...
def test_gateway_log_config_defaults() -> None:
    from rincode.config.schema import GatewayConfig
...
def test_gateway_channels_excludes_tui_when_no_im_enabled() -> None:
    ...
    from rincode.cli.gateway_commands import _build_gateway_channels
...
def test_stop_dispatch_cancels_both_scheduler_and_subagents() -> None:
    ...
    from rincode.cli import gateway_commands
...
from rincode.cli.gateway_commands import build_model_routing
from rincode.config.schema import ModelEndpoint, RoutingConfig
