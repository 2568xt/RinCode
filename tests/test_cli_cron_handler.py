"""Trigger-path tests for ``make_on_cron_job`` (rincode/cli/_cron_handler.py).
...
from rincode.cli import _cron_handler
from rincode.cli._cron_handler import make_on_cron_job
from rincode.proactive_engine.schedulers import cron as cron_package
from rincode.proactive_engine.schedulers.cron.service import CronService
from rincode.proactive_engine.schedulers.cron.types import (
    CronJob,
    CronJobState,
    CronPayload,
    CronSchedule,
)
from rincode.spine import Origin, Text
...
def test_cron_production_imports_have_no_proactive_side_effect_dependencies() -> None:
    forbidden = (
        "rincode.proactive_engine.sentinel",
        "rincode.proactive_engine.schedulers.heartbeat",
        "rincode.proactive_engine.system_events",
        "rincode.proactive_engine.wake",
    )
...
async def test_trigger_dynamic_config_reload(
    ...
    monkeypatch.setattr("rincode.config.loader.load_config", _make_config)
    channel_manager = SimpleNamespace(enabled_channels=["telegram", "feishu"])
