...
class TestManifestTools:
    ...
    def test_backend_and_tool_may_share_name(self) -> None:
        ...
        from rincode.plugin import MemoryBackendContribution
...
class TestBuildPluginTools:
    def _config(self, plugin_config: dict | None = None):
        from rincode.config.rincode import RinCodeConfig, PluginsConfig

        return RinCodeConfig(plugins=PluginsConfig(config=dict(plugin_config or {})))

    def test_builds_tools_from_registry(self, tmp_path: Path) -> None:
        from rincode.cli._plugin_stack import build_plugin_tools
