...
class TestListCommand:
    def test_missing_socket_exits_1(self, tmp_path: Path) -> None:
        ...
        with patch("rincode.cli.sandbox_commands._get_socket_path", return_value=missing):
    ...
    def test_empty_vm_list_shows_no_vms(self, tmp_path: Path) -> None:
        ...
        with (
            patch("rincode.cli.sandbox_commands._get_socket_path", return_value=path),
            patch("rincode.cli.sandbox_commands._connect", mock_connect),
            patch("rincode.cli.sandbox_commands._send", mock_send),
            patch("rincode.cli.sandbox_commands._recv", mock_recv),
            patch("rincode.cli.sandbox_commands._close", mock_close),
        ):
            result = runner.invoke(sandbox_app, ["list"])
...
class TestExecCommand:
    def test_no_command_exits_1(self, tmp_path: Path) -> None:
        path = tmp_path / "debug.sock"
        path.touch()
        with patch("rincode.cli.sandbox_commands._get_socket_path", return_value=path):
    ...
    def test_sends_correct_request(self, tmp_path: Path) -> None:
        ...
        with (
            patch("rincode.cli.sandbox_commands._get_socket_path", return_value=path),
            patch("rincode.cli.sandbox_commands._connect", mock_connect),
            patch("rincode.cli.sandbox_commands._send", AsyncMock()) as mock_send,
            patch("rincode.cli.sandbox_commands._recv", recv_mock),
            patch("rincode.cli.sandbox_commands._close"),
        ):
...
class TestGetSocketPath:
    def test_reads_configured_socket_path(self, tmp_path: Path) -> None:
        from rincode.cli.sandbox_commands import _get_socket_path
        from rincode.config.schema import Config
        from rincode.sandbox.config import SandboxDebugConfig

        cfg = Config()
        cfg.tools.sandbox.debug = SandboxDebugConfig(enabled=True, socket="custom/path.sock")
        with (
            patch("rincode.config.loader.load_config", return_value=cfg),
            patch("rincode.config.paths.get_data_dir", return_value=tmp_path),
        ):
    ...
    def test_falls_back_to_default_when_config_load_fails(self, tmp_path: Path, caplog) -> None:
        ...
        with (
            ...
            patch("rincode.config.paths.get_data_dir", return_value=tmp_path),
            caplog.at_level(logging.WARNING, logger="rincode.cli.sandbox_commands"),
        ):
            result = _get_socket_path()
...
class TestRecvRobustness:
    def test_empty_response_exits_with_clean_error(self, tmp_path: Path) -> None:
        ...
        with (
            patch("rincode.cli.sandbox_commands._get_socket_path", return_value=path),
            patch(
                "rincode.cli.sandbox_commands._connect",
                AsyncMock(return_value=(mock_reader, mock_writer)),
            ),
            patch("rincode.cli.sandbox_commands._send", AsyncMock()),
            patch("rincode.cli.sandbox_commands._close"),
        ):
            result = runner.invoke(sandbox_app, ["list"])
