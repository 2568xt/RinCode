...
fixture strips every ``RINCODE_*`` variable, so an environment handshake would be
...
from rincode.providers.base import LLMResponse
from rincode.spine import ChatType, Origin, Source, Text, TurnOutcome, TurnRequest, Usage
from rincode.spine import delivery as delivery_mod
from rincode.spine.delivery import Capabilities, DeliveryHub, make_hub_sink
...
from rincode.tracing import spans as _spans
from rincode.tracing import trace
...
def evidence_root(tmp_path, monkeypatch) -> Path:
    ...
    monkeypatch.setenv("RINCODE_TRACING", "1")
    monkeypatch.setenv("RINCODE_TRACING_DIR", str(root / "traces"))
