"""Unit tests for ``rincode/cli/tracing_commands.py`` viewer-launch helpers.
...
from rincode.cli import tracing_commands as tc
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802 — BaseHTTPRequestHandler 接口
        if (
            ...
            and self.headers.get("X-RinCode-Viewer-Action") == "shutdown"
        ):
...
def test_viewer_env_does_not_add_unrelated_environment(monkeypatch):
    monkeypatch.delenv("RINCODE_VIEWER_TEST_VALUE", raising=False)

    env = tc._viewer_env(4318)

    assert "RINCODE_VIEWER_TEST_VALUE" not in env


def test_viewer_env_preserves_unrelated_operator_environment(monkeypatch):
    monkeypatch.setenv("RINCODE_VIEWER_TEST_VALUE", "operator-value")

    assert tc._viewer_env(4318)["RINCODE_VIEWER_TEST_VALUE"] == "operator-value"
