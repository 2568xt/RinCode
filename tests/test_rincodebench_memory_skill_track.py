...
from benchmarks.rincodebench import budget as rincodebench_budget
from benchmarks.rincodebench.budget import (
...
from benchmarks.rincodebench.packs.memory_skill import e2e as memory_skill_e2e
from benchmarks.rincodebench.packs.memory_skill import runtime as memory_skill_runtime
from benchmarks.rincodebench.packs.memory_skill import worker as memory_skill_worker
from benchmarks.rincodebench.packs.memory_skill.models import CrossSessionTask
from benchmarks.rincodebench.packs.memory_skill.retrieval import _FixtureMemoryBackend
from benchmarks.rincodebench.packs.memory_skill.runtime import (
    ScriptedCrossSessionProvider,
    _freeze_rincode_config,
    _freeze_runtime_config,
)
from benchmarks.rincodebench.packs.memory_skill.semantic_fixtures import (
    semantic_fixture,
)
from benchmarks.rincodebench.protocol import RetrievalContext, TrialContext
...
from benchmarks.rincodebench.schema import (
    ExecutionPolicy,
    ExperimentSpec,
    ProviderTrialBudget,
)
...
from rincode.context_engine.segments import MemorySegmentBuilder
from rincode.memory_engine.skill_forge import SkillForgeRouter
from rincode.providers.base import GenerationSettings
...
def test_memory_skill_tasks_are_loaded_from_frozen_manifests() -> None:
    ...
    manifest_path = Path(__file__).parents[1] / "benchmarks" / "rincodebench" / "tasks" / "memory_skill" / "formal.json"
    payload = json.loads(manifest_path.read_text(encoding="utf-8"))

    assert payload["schema"] == "rincode.rincodebench.memory-skill-tasks.v1"
...
def test_real_runner_freezes_model_budget_and_routing(
    ...
    rincode_config = RinCodeConfig(base=frozen)
    rincode_config.token_wise.smart_routing.enabled = True
    frozen_rincode = _freeze_rincode_config(
        rincode_config,
        frozen,
        task,
    )
    assert frozen_rincode.token_wise.smart_routing.enabled is False
...
async def test_real_runner_uses_frozen_pack_budget_and_four_call_stage_split(
    ...
    experiment = ExperimentSpec(
        ...
        identity={"rincode_commit": "1" * 40, "model": "provider/exact-model"},
...
async def test_real_provider_worker_bootstrap_uses_stage_budget_scopes(
        ...
        json.dumps(
            {
                ...
                "rincode_config": rincode_config.model_dump(mode="json"),
            },
            sort_keys=True,
        ),
        encoding="utf-8",
    ...
    delegates = []
    stage_disabled_tools: dict[str, set[str]] = {}
    stage_tool_names: dict[str, set[str]] = {}
    ...
    def make_provider(config: Config) -> ScriptedCrossSessionProvider:
        stage_disabled_tools[active_stage] = set(config.tools.disabled_tools)
    ...
    monkeypatch.setattr(
        "rincode.cli._helpers.make_provider",
        make_provider,
    )
    scopes = []
    actual_budget_scope = rincodebench_budget.provider_call_budget_scope
    ...
    original_host_build = memory_skill_runtime.RuntimeTrialHost.build

    async def recording_host_build(**kwargs):
        host = await original_host_build(**kwargs)
        stage_tool_names[active_stage] = set(host.assembly.agent_loop.tools.tool_names)
        return host

    monkeypatch.setattr(
        memory_skill_runtime.RuntimeTrialHost,
        "build",
        recording_host_build,
    )
    results = {}
    ...
    assert results["learning"]["usage"]["calls"] == 1
    assert stage_tool_names == {
        "learning": set(),
        "evaluation": {"skill_read", "write_file"},
    }
    assert "skill_read" in stage_disabled_tools["learning"]
    assert "write_file" in stage_disabled_tools["learning"]
    assert "skill_read" not in stage_disabled_tools["evaluation"]
    assert "write_file" not in stage_disabled_tools["evaluation"]
...
async def test_retrieval_records_are_anonymous_and_keep_memory_and_skill_separate(
    ...
    experiment = ExperimentSpec(
        ...
        pack_ids=(definition.pack_id,),
        output_root=tmp_path,
        identity={"rincode_commit": "a" * 40, "model": "scripted/deterministic"},
    )
    memory_suite, skill_suite = definition.retrieval_suites
...
async def test_retrieval_adapter_executes_product_segments_and_router(
    ...
    experiment = ExperimentSpec(
        ...
        pack_ids=(definition.pack_id,),
        output_root=tmp_path,
        identity={"rincode_commit": "c" * 40, "model": "scripted/deterministic"},
    )
...
async def test_formal_retrieval_micro_suite_has_separate_claim_gates(
    ...
    experiment = ExperimentSpec(
        ...
        output_root=tmp_path,
        identity={"rincode_commit": "b" * 40, "model": "scripted/deterministic"},
    )
...
async def test_artifact_reducer_separates_contract_and_real_claims(
    ...
    experiment = ExperimentSpec(
        ...
        pack_ids=(definition.pack_id,),
        output_root=tmp_path,
        identity={"rincode_commit": "f" * 40, "model": "scripted/deterministic"},
    )
    measurements = await run_retrieval_micro_suite(pack, experiment)
    retrieval_records = [
        {
