...
async def _invoke(args, socket_path):
    ...
    def _run():
        with patch("rincode.cli.sandbox_commands._get_socket_path", return_value=socket_path):
...
async def _invoke_shell(args, socket_path):
    ...
    Patches rincode.cli.sandbox_commands.sys (not the global sys) so CliRunner's
    ...
    def _run():
        try:
            with (
                patch("rincode.cli.sandbox_commands._get_socket_path", return_value=socket_path),
                patch("rincode.cli.sandbox_commands.sys", mock_sys),
                patch("tty.setraw"),
