...
from benchmarks.rincodebench.packs.tool_mcp.runner import (
    _aggregate_usage,
...
from rincode.spine import ToolEvent, ToolPhase
...
@pytest.mark.asyncio
async def test_tool_mcp_excludes_predispatch_budget_denial_from_usage_aggregate(tmp_path: Path) -> None:
    class CountingDelegate(LLMProvider):
    ...
    provider = _RecordingToolMCPProvider(
        guarded,
        model="actual/provider-model",
        generation=GenerationSettings(max_tokens=64),
    )

    with provider_call_budget_scope(
        trial_id="tool-mcp-predispatch-budget",
        max_logical_calls=4,
        max_attempts_per_call=1,
    ):
        for index in range(4):
            await provider.chat(
                messages=[{"role": "user", "content": f"call-{index}"}],
                model="actual/provider-model",
                max_tokens=64,
            )
        with pytest.raises(ProviderRequestNotDispatchedError):
            await provider.chat(
    ...
    denied = provider.call_records[-1]
    assert denied["provider_dispatched"] is False
    assert denied["usage_complete"] is False
    assert all(denied["usage"][key] is None for key in ("prompt_tokens", "completion_tokens", "total_tokens"))
    assert denied["error_category"] == "task_budget_exhausted"
    aggregate = _aggregate_usage(provider.call_records)
    assert aggregate["main_agent_model_calls"] == 4
    assert aggregate["main_agent_input_tokens"] == 40
    assert aggregate["main_agent_output_tokens"] == 8
    assert aggregate["main_agent_total_tokens"] == 48
    assert aggregate["usage_complete"] is True
    snapshot = guarded.ledger.snapshot()
...
def test_target_call_normalization_keeps_invalid_unknown_and_exact_repeats() -> None:
    ...
    events = (
        ...
        ToolEvent(
            ...
            name="tool_call",
            arguments={
                "name": "mcp_rincodebench_catalog_probe_99",
                "arguments": {},
            },
        ),
        ToolEvent(
