...
def test_the_report_passes_only_when_every_check_passes():
    ...
    assert report["schema"] == "rincode.turn.evidence.v1"
...
def test_usage_loader_accepts_legacy_and_call_efficiency_ledgers(tmp_path):
    ...
    current = {
        "schema": "rincode.call-efficiency.call.v1",
        "trace_id": "current",
        "turn_span_id": "turn-2",
    }
...
def test_call_efficiency_health_fails_when_ledger_reports_loss(tmp_path):
    (tmp_path / "call-efficiency-ledger-health.json").write_text(
        json.dumps(
            {
                "schema": "rincode.call-efficiency.ledger-health.v1",
                "status": "degraded",
                "accepted_records": 2,
                "persisted_records": 1,
                "lost_records": 1,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        json.dumps({"schema": "rincode.call-efficiency.ledger-health.v1", "status": "healthy"}),
        json.dumps(
            {
                "schema": "rincode.call-efficiency.ledger-health.v1",
                "status": "healthy",
