...
from benchmarks.rincodebench.packs.tokenwise_cost import (
    CACHE_POLICY_PREFIX_DISRUPTED,
    CACHE_POLICY_PREFIX_STABLE,
)
from benchmarks.rincodebench.packs.tokenwise_cost.live import (
    CampaignBudget,
    CampaignConfig,
    CampaignError,
    LiveTrialResult,
...
CORPUS = Path(__file__).resolve().parents[1] / "benchmarks" / "rincodebench" / "tasks" / "tokenwise_cost" / "formal.json"


def test_formal_corpus_is_sealed_and_matches_the_frozen_matrix() -> None:
    corpus = load_task_corpus(CORPUS)

    assert corpus.schema == "rincode.rincodebench.tokenwise-cost.tasks.v1"
...
def test_current_report_requires_runtime_call_records_and_positive_paired_interval(tmp_path: Path) -> None:
    ...
    assert report["schema"] == "rincode.rincodebench.call-efficiency-cost.report.v2"
    assert report["claim"]["claim_eligible"] is True
...
async def test_live_trial_crosses_runtime_assembly_and_persists_every_call_record(
    ...
    monkeypatch.setattr(
        "benchmarks.rincodebench.packs.tokenwise_cost.runner._RecordingDeepSeekProvider",
