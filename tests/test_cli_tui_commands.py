"""CLI tests for ``rincode`` commands - ``_build_tui_agent_loop`` wiring.
...
def patched_tui_loop_deps(monkeypatch: pytest.MonkeyPatch, tmp_path):
    ...
    monkeypatch.setenv("RINCODE_HOME", str(product_home))
    captured: dict[str, Any] = {}

    config = MagicMock()
    config.workspace_path = tmp_path
    config.agents.defaults.workspace = "~/.rincode/workspace"
    ...
    monkeypatch.setattr("rincode.cli._helpers.load_runtime_config", lambda _a, _b: config)
    monkeypatch.setattr("rincode.cli._helpers.make_provider", lambda _c: MagicMock())

    ec_config = MagicMock()
    ec_config.skill_forge = MagicMock()
    ec_config.runtime = MagicMock()
    monkeypatch.setattr("rincode.config.rincode.load_rincode_config", lambda: ec_config)

    monkeypatch.setattr("rincode.session.manager.SessionManager", lambda _wp: MagicMock())
    cron_dir = tmp_path / "cron"
    cron_dir.mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr("rincode.config.paths.get_cron_dir", lambda: cron_dir)
    ...
    monkeypatch.setattr("rincode.agent.loop.AgentLoop", _AgentLoopSpy)
    ...
    monkeypatch.setattr(
        "rincode.cli._plugin_stack.build_plugin_registry",
        lambda cfg: fake_registry,
    )
    monkeypatch.setattr(
        "rincode.cli._plugin_stack.maybe_build_memory_backend",
        lambda ws, cfg, *, registry=None: fake_backend,
    )
    monkeypatch.setattr(
        "rincode.cli._plugin_stack.build_plugin_tools",
...
def test_tui_adapter_uses_shared_runtime_assembly(
    ...
    from rincode.cli import _runtime_assembly
    from rincode.cli.tui_commands import _build_tui_agent_loop
    ...
    assert (
        calls[0][1]["paths"].state.parent == patched_tui_loop_deps["config"].workspace_path / "rincode-home" / "projects"
    )
...
def test_tui_build_marks_plugin_error_message_public(
    ...
    from rincode.cli.tui_commands import _build_tui_runtime
    ...
    monkeypatch.setattr("rincode.cli._runtime_assembly.assemble_runtime", _raise)
...
def test_tui_build_plugin_registry_called_once(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("rincode.cli._plugin_stack.build_plugin_registry", _spy_registry)
    monkeypatch.setattr("rincode.cli._plugin_stack.maybe_build_memory_backend", _spy_backend)
    monkeypatch.setattr("rincode.cli._plugin_stack.build_plugin_tools", _spy_tools)
...
def rpc_server_deps(monkeypatch: pytest.MonkeyPatch):
    ...
    from rincode.cli._runtime_assembly import RuntimeAssembly
    ...
    monkeypatch.setattr(
        "rincode.cli.tui_commands._build_tui_runtime",
        lambda: fake_runtime,
    )

    fake_dispatcher = MagicMock()
    fake_dispatcher.register = MagicMock()
    monkeypatch.setattr("rincode.tui_rpc.dispatcher.Dispatcher", lambda: fake_dispatcher)
    ...
    monkeypatch.setattr(
        "rincode.tui_rpc.server.RpcServer",
        lambda **kw: fake_server,
    )

    fake_emitter = MagicMock()
    monkeypatch.setattr(
        "rincode.tui_rpc.subscriptions.SubscriptionEmitter",
    ...
    monkeypatch.setattr(
        "rincode.tui_rpc.confirm_broker.ConfirmBroker",
        lambda **kw: fake_confirm_broker,
    )

    fake_question_broker = MagicMock()
    monkeypatch.setattr(
        "rincode.tui_rpc.question_broker.QuestionBroker",
    ...
    monkeypatch.setattr("rincode.tui_rpc.methods.system.system_hello", _fake_system_hello)
    monkeypatch.setattr("rincode.tui_rpc.methods.system.system_ping", AsyncMock())
    monkeypatch.setattr("rincode.tui_rpc.methods.system.system_version", AsyncMock())
    aligned_register = MagicMock()
    monkeypatch.setattr(
        "rincode.tui_rpc.methods.register_aligned_methods_except_system",
        aligned_register,
    )
    ...
    monkeypatch.setattr("rincode.tui_rpc.spine.build_tui", _fake_build_tui)

    monkeypatch.setattr("rincode.cli._cron_handler.make_on_cron_job", MagicMock())
    monkeypatch.setattr("rincode.tui_rpc.methods.turn.clear_active", MagicMock())
...
async def test_rpc_server_starts_before_runtime_build_finishes(
    ...
    monkeypatch.setattr("rincode.cli.tui_commands._build_tui_runtime", _slow_build)
...
async def test_rpc_runner_finishes_runtime_close_after_cancellation_during_turn_teardown(
    ...
    monkeypatch.setattr(
        "rincode.tui_rpc.spine.build_tui",
        lambda *args, **kwargs: (
            MagicMock(),
            MagicMock(),
            {},
...
async def test_rpc_runner_latches_spine_build_failure_and_closes_runtime(
    ...
    monkeypatch.setattr("rincode.tui_rpc.spine.build_tui", _fail_build)
...
async def test_rpc_runner_stop_called_even_when_serve_raises(rpc_server_deps, monkeypatch) -> None:
    ...
    monkeypatch.setattr("rincode.cli.tui_commands.asyncio.wait", _raising_wait)
...
async def test_rpc_runner_activates_fd_redirect_before_backend_start(rpc_server_deps, monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        "rincode.cli.tui_commands.redirect_terminal_fds_to_file",
        _spy_redirect,
    )
    monkeypatch.setattr(
        "rincode.config.paths.get_logs_dir",
        lambda: tmp_path,
    )
...
def test_is_abnormal_child_exit(exit_code: int, abnormal: bool) -> None:
    ...
    from rincode.cli.tui_commands import _is_abnormal_child_exit
