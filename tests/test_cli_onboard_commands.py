"""CLI tests for ``rincode onboard`` - the four-step first-use wizard.
...
(``rincode.config.update_providers.test_provider``) and at the first-Turn
boundary (``rincode.cli.onboard_commands.run_first_turn``).
"""
...
from rincode.cli import onboard_commands
from rincode.cli.commands import app
from rincode.config.loader import set_config_path
...
def tmp_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    modules (``rincode.config.paths`` / ``rincode.utils.helpers``) rather
    than the consumer.
    """
    ...
    set_config_path(cfg)
    monkeypatch.setattr(
        "rincode.config.paths.get_workspace_path",
        lambda: workspace,
    )
    monkeypatch.setattr(
        "rincode.utils.helpers.sync_workspace_templates",
        lambda _: None,
    )
    yield cfg
...
def stub_verify(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("rincode.config.update_providers.test_provider", _ok)
    return _ok
...
def test_onboard_help_lists_all_flags() -> None:
    """``rincode onboard --help`` exposes the full flag surface."""
...
def test_onboard_non_interactive_minimum_flags(tmp_env: Path, stub_verify, stub_step3) -> None:
    ...
    assert r.exit_code == 0, r.stdout
    assert "Welcome to the RinCode setup wizard" in r.stdout
...
def test_onboard_provider_test_failure_warns_but_continues(
    ...
    monkeypatch.setattr("rincode.config.update_providers.test_provider", _fail)
...
def test_step1_writes_via_ops_lib(tmp_env: Path, monkeypatch: pytest.MonkeyPatch, stub_verify) -> None:
    ...
    monkeypatch.setattr("rincode.config.update_providers.set_provider_fields", _spy)
    monkeypatch.setattr(onboard_commands, "_prepare_myna", lambda **_: True)
...
def test_styles_module_loads() -> None:
    """``_styles.py`` import must not crash and must export ``RINCODE_STYLE``."""
    from rincode.cli._styles import RINCODE_STYLE  # noqa: F401

    assert RINCODE_STYLE is not None
...
def test_run_first_turn_uses_public_run_command(
    ...
    assert command[:4] == [onboard_commands.sys.executable, "-m", "rincode.cli.commands", "run"]
...
def test_step1_picker_uses_catalog_when_available(tmp_env: Path, monkeypatch: pytest.MonkeyPatch, stub_step3) -> None:
    ...
    monkeypatch.setattr("rincode.config.update_providers.test_provider", _ok_with_catalog)
    monkeypatch.setattr(onboard_commands, "_check_tty_or_die", lambda non_interactive: None)
    monkeypatch.setattr(onboard_commands, "_pick_language", lambda: None)
    monkeypatch.setattr(onboard_commands, "_select_provider", lambda: "anthropic")
...
def test_run_gate_triggers_when_missing(tmp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """``rincode run`` (interactive, TTY, missing config) enters the wizard."""
    from rincode.cli import agent_commands
...
def test_run_gate_skips_when_populated(tmp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """``rincode run`` with complete config does not enter the wizard."""
    from rincode.cli import agent_commands
    ...
    monkeypatch.setattr("rincode.cli._helpers.load_runtime_config", _boom)
...
def test_run_gate_skips_oneshot_message(tmp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """``rincode run -m '...'`` (one-shot) must not enter the wizard even on a
    ...
    monkeypatch.setattr(
        "rincode.cli._helpers.load_runtime_config",
...
def test_run_gate_skips_non_tty(tmp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Non-TTY (piped) ``rincode run`` must not enter the wizard."""
    from rincode.cli import agent_commands
...
def test_tui_gate_triggers_when_missing(tmp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Bare ``rincode`` enters onboarding before launching the native TUI."""
    from rincode.cli import tui_commands
...
def test_tui_gate_skips_check_flag(tmp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """``rincode --check`` bypasses onboarding."""
    from rincode.cli import tui_commands
...
def test_memory_skip_sets_backend_null(
    ...
    assert load_rincode_config().memory.backend is None


def test_memory_step_selects_myna_without_rincode_side_overrides(
    tmp_env: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert load_rincode_config().memory.backend == "myna"
...
def _install_fake_myna_descriptor(monkeypatch: pytest.MonkeyPatch, descriptor: Any) -> None:
    ...
    rincode = types.ModuleType("myna.integrations.rincode")
    rincode.descriptor = lambda: descriptor  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "myna", package)
    monkeypatch.setitem(sys.modules, "myna.integrations", integrations)
    monkeypatch.setitem(sys.modules, "myna.integrations.rincode", rincode)
...
def test_prepare_myna_applies_consent_bound_repository_setup(
    ...
    monkeypatch.setattr(
        "rincode.cli._plugin_stack.inspect_memory_backend",
        lambda config: SimpleNamespace(state="available", error=None),
    )
    applied: list[str] = []
    descriptor = SimpleNamespace(
        protocol="myna.rincode-app-integration.v1",
...
def test_switch_provider_returns_to_picker_keeps_steps(
    ...
    monkeypatch.setattr("rincode.config.update_providers.test_provider", _verify)
    monkeypatch.setattr(onboard_commands, "_check_tty_or_die", lambda non_interactive: None)
    monkeypatch.setattr(onboard_commands, "_pick_language", lambda: None)
...
def test_load_raw_config_raises_on_malformed(tmp_env: Path) -> None:
    ...
    from rincode.config.loader import ConfigReadError
...
def test_prepare_myna_rejects_incompatible_adapter_protocol(tmp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    onboard_commands._bootstrap_empty_config()
    monkeypatch.setattr(
        "rincode.cli._plugin_stack.inspect_memory_backend",
        lambda config: SimpleNamespace(state="available", error=None),
    ...
    previews: list[Path] = []
    descriptor = SimpleNamespace(
        protocol="myna.unsupported-app-integration.v0",
        preview_setup=lambda workspace: previews.append(workspace),
    )
    _install_fake_myna_descriptor(monkeypatch, descriptor)
    messages: list[str] = []
    monkeypatch.setattr(
        onboard_commands,
        "_disable_memory_after_setup_problem",
        lambda message, **kwargs: messages.append(message) or False,
    )
    assert onboard_commands._prepare_myna(non_interactive=True, yes=True) is False
    assert previews == []
    assert messages == ["unsupported Myna setup protocol; install a RinCode-compatible Myna adapter"]


def test_prepare_myna_explains_missing_adapter(tmp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    onboard_commands._bootstrap_empty_config()
    monkeypatch.setattr(
        "rincode.cli._plugin_stack.inspect_memory_backend",
        lambda config: SimpleNamespace(state="available", error=None),
    )
    _install_fake_myna_descriptor(monkeypatch, SimpleNamespace())
    monkeypatch.setitem(sys.modules, "myna.integrations.rincode", None)
    messages: list[str] = []
    monkeypatch.setattr(
        onboard_commands,
        "_disable_memory_after_setup_problem",
    ...
    assert messages[0].startswith("A RinCode-compatible Myna adapter is required:")
