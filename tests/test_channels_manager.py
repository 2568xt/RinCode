"""Tests for rincode.channels.manager.ChannelManager — spec-based init
...
from rincode.channels.contract import Capabilities, ChannelSpec
from rincode.channels.manager import ChannelManager, _missing_dep_hint
...
def _manager(monkeypatch, specs, config) -> ChannelManager:
    monkeypatch.setattr("rincode.channels.registry.discover_specs", lambda: specs)
...
async def test_stop_all_times_out_one_transport_and_attempts_the_rest(monkeypatch) -> None:
    monkeypatch.setattr("rincode.channels.manager._CHANNEL_STOP_TIMEOUT_S", 0.01, raising=False)
    events: list[str] = []
...
_WHEEL_JSON = '{"url": "https://x/rincode_harness-0.1.2.whl", "archive_info": {}}'
...
def _patch_direct_url(monkeypatch, read_text_result, calls=None):
    ...
    monkeypatch.setattr("rincode.channels.manager.distribution", fake_distribution)
...
def test_hint_non_editable_points_to_source_install(monkeypatch, raw):
    """Any non-editable / malformed direct_url.json -> source-install hint, never raises."""
    _patch_direct_url(monkeypatch, raw)
    hint = _missing_dep_hint("wecom")
    assert "uv sync" not in hint
    assert "RinCode source checkout" in hint
    assert "uv pip install '.[channel-wecom]'" in hint
    assert "gitee.com" not in hint


def test_hint_package_not_found_points_to_source_install(monkeypatch):
    """RinCode distribution not found -> source-install hint, no exception."""

    def _raise(pkg):
        raise PackageNotFoundError(pkg)

    monkeypatch.setattr("rincode.channels.manager.distribution", _raise)
    assert "uv pip install '.[channel-qq]'" in _missing_dep_hint("qq")


@pytest.mark.parametrize(
    "direct_url, expected",
    [
        (_EDITABLE_JSON, "uv sync --extra channel-feishu"),
        (_WHEEL_JSON, "uv pip install '.[channel-feishu]'"),
    ],
    ids=["editable", "wheel"],
)
def test_init_warning_carries_install_hint(monkeypatch, direct_url, expected):
