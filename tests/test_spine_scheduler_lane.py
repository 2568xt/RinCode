...
import rincode.spine.scheduler as scheduler_module
...
from rincode.spine.scheduler import Lane, OriginPools
...
async def test_cancel_during_setup_window_stops_the_turn():
    ...
    assert stopped == 1
    assert ran == []
    assert [type(event) for event in events] == [TurnStarted, TurnFailed]
    assert events[-1].cancelled is True


async def test_start_sink_failure_before_write_still_gets_a_terminal_failure():
    events: list = []
    ran: list[str] = []
    ...
    lane = Lane(runner=R(), pools=OriginPools(user=1, system=1), sink=sink, conversation_id="c")
    assert await asyncio.wait_for(lane.submit(_req("X")), timeout=1.0) is None

    assert [type(event) for event in events] == [TurnFailed]
    assert events[0].cancelled is False
    assert "event store unavailable" in events[0].error
    assert ran == []


async def test_repeated_cancel_during_terminal_failure_cleanup_is_idempotent():
    runner = HangingRunner()
    events: list = []
...
async def test_cancel_during_terminal_success_cleanup_keeps_the_single_success_terminal():
    events: list = []
    terminal_entered = asyncio.Event()
    release_terminal = asyncio.Event()
    ...
    assert [type(event) for event in events] == [TurnStarted, Text, TurnEnded]


async def test_terminal_sink_that_swallows_cancel_cannot_hold_lane_past_deadline(monkeypatch):
    monkeypatch.setattr(scheduler_module, "_TERMINAL_SINK_TIMEOUT", 0.05)
    terminal_entered = asyncio.Event()
    release_terminal = asyncio.Event()
        ...
        if isinstance(event, TurnEnded):
            ...
            while not release_terminal.is_set():
                try:
                    ...
                    # Model a channel implementation that refuses to stop until its own
                    # transport cleanup is released by the test.
                    continue
