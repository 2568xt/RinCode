"""Tests for the native TUI behind the bare ``rincode`` entry point."""
...
def test_tui_check_node_ok(monkeypatch):
    ...
    monkeypatch.setattr(
        "rincode.cli.tui_commands.find_node",
        lambda: ("/usr/bin/node", (22, 5, 0)),
    )
    monkeypatch.setattr(
        "rincode.cli.tui_commands.run_subprocess",
        lambda *_args, **_kw: 0,
    )
...
def test_tui_color_flag_forwards_to_child_env(monkeypatch):
    """`--color` exports RINCODE_TUI_COLOR for the Node child (read by
    colorTier.ts)."""
    import os

    monkeypatch.delenv("RINCODE_TUI_COLOR", raising=False)
    monkeypatch.setattr(
        "rincode.cli.tui_commands.find_node",
        lambda: ("/usr/bin/node", (22, 5, 0)),
    )
    ...
    def fake_run_subprocess(*_a, **_k):
        captured["color"] = os.environ.get("RINCODE_TUI_COLOR")
        return 0
...
def test_tui_print_colors_uses_no_rpc_spawn(monkeypatch, tmp_path):
    ...
    (not run_subprocess_with_rpc) and set RINCODE_TUI_PRINT_COLORS."""
    import os
    ...
    monkeypatch.delenv("RINCODE_TUI_PRINT_COLORS", raising=False)
    monkeypatch.setattr("rincode.cli.tui_commands._UI_TUI_DIR", tmp_path)
    monkeypatch.setattr(
        "rincode.cli.tui_commands.find_node",
        lambda: ("/usr/bin/node", (22, 5, 0)),
    )
    ...
    def fake_plain(*_a, **_k):
        calls["plain"] = True
        calls["print_colors"] = os.environ.get("RINCODE_TUI_PRINT_COLORS")
        return 0
    ...
    monkeypatch.setattr("rincode.cli.tui_commands.run_subprocess", fake_plain)
    monkeypatch.setattr("rincode.cli.tui_commands.run_subprocess_with_rpc", fake_rpc)
...
def test_tui_preview_colors_uses_no_rpc_spawn(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("RINCODE_TUI_COLOR_PREVIEW", raising=False)
    monkeypatch.setattr("rincode.cli.tui_commands._UI_TUI_DIR", tmp_path)
    monkeypatch.setattr(
        "rincode.cli.tui_commands.find_node",
        lambda: ("/usr/bin/node", (22, 5, 0)),
    ...
    def fake_plain(*_a, **_k):
        calls["plain"] = True
        calls["preview"] = os.environ.get("RINCODE_TUI_COLOR_PREVIEW")
        return 0

    monkeypatch.setattr("rincode.cli.tui_commands.run_subprocess", fake_plain)
    monkeypatch.setattr(
        "rincode.cli.tui_commands.run_subprocess_with_rpc",
...
def test_rincode_node_override_no_fallback(monkeypatch, tmp_path):
    """When RINCODE_NODE is set but path missing, find_node returns (None, None)
    — must NOT fall back to venv/PATH."""
    from rincode.cli.tui_commands import find_node

    monkeypatch.setenv("RINCODE_NODE", str(tmp_path / "nonexistent-node"))

    node_path, version = find_node()
    assert node_path is None, "RINCODE_NODE override must not fall back to venv/PATH"
...
def test_find_node_discovers_windows_private_runtime(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("RINCODE_HOME", str(tmp_path))
    monkeypatch.delenv("RINCODE_NODE", raising=False)
...
def test_dev_npx_derived_from_node_path(monkeypatch, tmp_path):
    ...
    not from PATH — so RINCODE_NODE's version-pin semantics are honored."""
    ...
    monkeypatch.setattr(
        "rincode.cli.tui_commands.shutil.which",
...
def test_run_subprocess_returns_child_exit_code(monkeypatch):
    ...
    from rincode.cli.tui_commands import run_subprocess
...
def test_rpc_handshake_timeout(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("rincode.cli.tui_commands._UI_TUI_DIR", fake_ui_dir)
...
def test_rpc_socket_env_constants_exposed():
    ...
    assert tui_commands._RPC_SOCKET_ENV == "RINCODE_RPC_SOCKET"
    assert tui_commands._RPC_TOKEN_ENV == "RINCODE_RPC_TOKEN"
...
def test_production_dispatcher_includes_all_umbrella_methods():
    ...
    from rincode.tui_rpc.methods import (
    ...
    for required in (
        ...
        assert required in production_methods, (
            f"{required} missing from production dispatcher — `rincode` will return -32601 for it"
        )
...
def test_rpc_socket_transport_handshake_ok(tmp_path, monkeypatch):
    ...
    address in `RINCODE_RPC_SOCKET`, sends the `RINCODE_RPC_TOKEN` auth line, then
...
host, port = os.environ["RINCODE_RPC_SOCKET"].rsplit(":", 1)
s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
s.connect((host, int(port)))
tok = os.environ.get("RINCODE_RPC_TOKEN")
if tok:
    s.sendall((tok + "\\n").encode("utf-8"))
