"""E2E: `rincode` alt-screen `/status` round-trip — ACCEPTANCE.

This is the **live ACCEPTANCE gate**. It exercises the full harness
path:
1. spawn production `uv run rincode` (Python parent + Node child + unix
...
6. press ctrl+c twice (cancel input then exit per RinCode UX)
7. expect_exit 0 (proves Ink autonomy)

Zero LLM cost; exercises everything except chat streaming — see
test_e2e_rincode_tui_chat.py for the chat streaming path.
"""
...
def test_tui_status_slash_round_trip(harness):
    harness.spawn("uv run rincode")
    assert harness.wait(r"RinCode", timeout=25.0), (
        f"TUI RinCode readiness banner not seen in 25s; screen=\n{harness.screen()}"
    )
    harness.type("/status")
    harness.press("enter")
