"""Contract tests for the ``rincode.tracing.trace`` facade (standard-api.v1).
...
def trace_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("RINCODE_TRACING_DIR", str(tmp_path))
    _spans._store = None
    yield tmp_path
    _spans._store = None
...
def test_invocation_source_derives_from_enclosing_purpose(trace_dir):
    from rincode.tracing import semconv
...
def test_custom_node_uses_explicit_kind(trace_dir):
    with trace.span("rincode.plugin.refresh", {"plugin.reason": "x"}, kind="plugin"):
        pass

    spans = _spans_written(trace_dir)
    assert spans[0]["name"] == "rincode.plugin.refresh"
...
def test_audit_span_v1_record_shape_is_frozen(trace_dir):
    ...
    assert sp["attributes"]["framework"] == "rincode"


def test_span_kind_vocabulary_is_frozen():
    from rincode.tracing import trace as _t
