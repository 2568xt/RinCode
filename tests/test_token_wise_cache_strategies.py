"""Head-to-head: Hermes ``system_and_3`` vs RinCode ``CacheOptimizer``.
...
Variants per scenario:
    ...
    V2  RinCode current — tools(bp1) + system(bp2) + before-user(bp3) + mid(bp4)
    V3  Hermes faithful  — system(bp1) + last-3-messages(bp2–4)

Report: ``rincode/token_wise/EXPERIMENT_REPORT_CACHE_STRATEGIES.md``
...
from rincode.token_wise.cache_optimizer import CacheOptimizer
from rincode.token_wise.registry import StrategyRegistry
from rincode.token_wise.system_and_tail_cache import SystemAndTailCacheStrategy
from rincode.token_wise.usage_tracker import UsageTracker

REPORT_PATH = Path(__file__).resolve().parent.parent / "rincode" / "token_wise" / "EXPERIMENT_REPORT_CACHE_STRATEGIES.md"
...
def _medium_soul() -> str:
    return (
        "# Soul\n\nI am RinCode, a careful assistant.\n\n"
        "## Working principles\n\n"
...
def _tool_chain_soul() -> str:
    return (
        "# Soul\n\nI am RinCode, a tool-using assistant.\n\n"
        "## Tool-chain protocol\n\n"
...
def _write_report(scenarios: list[ScenarioResult]) -> str:
    lines: list[str] = []
    lines.append("# Hermes ``system_and_3`` vs RinCode ``CacheOptimizer`` — Head-to-Head\n")
    lines.append(
    ...
    lines.append("| **V2 RinCode v2** | tools[-1] (if tools) | system tail | rolling msg[-2] | rolling msg[-1] |")
    ...
    for sc in scenarios:
        ...
        if len(sc.variants) > 2:
            ...
            if v2.total_cost > 0:
                ...
                lines.append(f"- **Winner: {winner}** (RinCode vs Hermes: {vs:+.1f}%)\n")
    ...
    for sc in scenarios:
        if len(sc.variants) >= 3:
            ...
            if v2.total_cost > 0:
                ...
                winner = "Hermes" if margin > 1 else "RinCode" if margin < -1 else "Tie"
...
@pytest.mark.asyncio
async def test_hermes_vs_rincode(api_key: str, tmp_path: Path):
    cost: dict[str, float] = {}
    ...
    for vname, strategy_label, cache_strat, disable_auto in [
        ...
        ("V2_rincode", "tools+sys+rolling_tail", CacheOptimizer(max_breakpoints=4), True),
    ...
    for sc in scenarios:
        ...
        assert v2.total_cost < v1.total_cost, f"{sc.name}: RinCode not cheaper than baseline"
