...
from rincode.tui_rpc.methods.session import _default_session_info
...
class _FakeToolRegistry:
    """Stand-in for ``rincode.agent.tools.registry.ToolRegistry``."""
...
class _FakeStrategyRegistry:
    """Stand-in for ``rincode.token_wise.registry.StrategyRegistry``."""
...
def test_default_session_info_contains_real_version(fake_agent_loop, config) -> None:
    ...
    info = _default_session_info(fake_agent_loop, config)
    expected_version = importlib.metadata.version("rincode-harness")
    assert info["version"] == expected_version, (
        f"info.version must be importlib.metadata.version('rincode-harness') = {expected_version!r}"
    )
...
def test_default_session_info_falls_back_when_agent_loop_none(config) -> None:
    ...
    assert info["version"] == importlib.metadata.version("rincode-harness")
...
def test_resolve_context_window_helper_removed() -> None:
    ...
    session_py = Path(__file__).parent.parent / "rincode" / "tui_rpc" / "methods" / "session.py"
