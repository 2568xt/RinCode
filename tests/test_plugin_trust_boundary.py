...
from rincode.cli._plugin_stack import build_plugin_registry, plugin_discovery_sources
from rincode.config.rincode import MemoryConfig, RinCodeConfig, PluginsConfig
from rincode.plugin import PluginDiscovery, ServiceLocator, assemble_plugin_registry
...
def _write_tool_plugin(root: Path, *, marker: Path) -> Path:
    ...
    (plugin_dir / "rincode-plugin.toml").write_text(
        textwrap.dedent(
            """
            [plugin]
...
def _memory_off_config() -> RinCodeConfig:
    return RinCodeConfig(
...
def test_shared_cli_tui_gateway_registry_ignores_checkout_plugins(
    ...
    _write_tool_plugin(checkout / ".rincode" / "plugins", marker=marker)
...
def test_rincode_plugins_does_not_import_checkout_plugins(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    checkout = tmp_path / "checkout"
    ...
    result = CliRunner().invoke(
        ...
        env={**os.environ, "RINCODE_HOME": str(tmp_path / "rincode-home"), "COLUMNS": "200"},
    )
...
def test_rincode_plugins_inspects_user_manifest_without_importing_factory(tmp_path: Path) -> None:
    rincode_home = tmp_path / "rincode-home"
    marker = tmp_path / "user-command-imported"
    plugin_dir = _write_tool_plugin(rincode_home / "plugins", marker=marker)
    config_path = tmp_path / "config.json"
    config_path.write_text(json.dumps({"memory": {"backend": None}}), encoding="utf-8")
    ...
    result = CliRunner().invoke(
        app,
        ["plugins", "--config", str(config_path)],
        env={**os.environ, "RINCODE_HOME": str(rincode_home), "COLUMNS": "200"},
    )
