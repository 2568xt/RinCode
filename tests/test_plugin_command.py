"""``rincode plugins`` reports the installed Myna Plugin without starting it."""
...
def _installed_myna(monkeypatch: pytest.MonkeyPatch):
    module = types.ModuleType("myna.integrations.rincode.backend")
    ...
    monkeypatch.setattr("rincode.plugin.discover.PluginDiscovery.discover", lambda _self: [discovered])
...
def test_verbose_shows_myna_factory(tmp_path: Path) -> None:
    ...
    assert result.exit_code == 0, result.stdout
    assert "myna.integrations.rincode:make_backend" in result.stdout
