...
from rincode.agent.loop.recovery import RecoveryLimits
from rincode.config.rincode import CheckpointConfig, RuntimeConfig
from rincode.providers.base import LLMProvider, LLMResponse
...
async def test_d2_workspace_does_not_exist():
    ...
    bogus = Path(tempfile.gettempdir()) / "rincode_nonexistent_xyz"
    if bogus.exists():
        shutil.rmtree(bogus)
...
async def test_d2_corrupted_shadow_repo(workspace):
    ...
    gd = workspace / ".rincode" / "shadow.git"
...
def test_d3_repeated_stash_latest_wins(workspace):
    ...
    not stale files."""
    agent = _agent_with_checkpoint(workspace)
    from rincode.agent.loop import TurnOutcome
...
async def test_d2_loop_unaffected_when_shadow_blocked(workspace):
    ...
    (workspace / ".rincode").write_text("blocker", encoding="utf-8")
    agent = _agent_with_checkpoint(workspace)
...
async def test_d7_notice_txt_written_next_to_shadow(workspace):
    """S4 discoverability: a user noticing ``.rincode/`` should be able to
    identify what it is without grepping our codebase."""
    svc = CheckpointService(workspace)
    (workspace / "x.py").write_text("ok\n", encoding="utf-8")
    await svc.commit_turn("t1")
    notice = workspace / ".rincode" / "NOTICE.txt"
    assert notice.exists()
    text = notice.read_text()
    assert "RinCode" in text and "checkpoint" in text
...
async def test_d7_gc_invoked_every_n_commits(workspace, monkeypatch):
    ...
    import rincode.agent.loop.checkpoint as cp_module
...
async def test_d8_one_shot_mode_creates_no_shadow_dir(workspace):
    ...
    ``rincode run -m "..."`` leaves no ``.rincode/`` artifact on disk —
    ...
    assert outcome.status == "completed"
    assert outcome.checkpoint_id is None
    assert not (workspace / ".rincode").exists()
