"""Unit tests for the unified evolution launcher (rincode.evolver.launch)."""
...
from rincode.evolver.launch.config import (
    SMOKE_BUILTIN,
    RunSpecError,
    deep_merge,
    load_run_spec,
)
from rincode.evolver.launch.contract import validate_whitelist
from rincode.evolver.launch.registry import load_bench
from rincode.evolver.launch.state import RunMeta, atomic_write_json, config_fingerprint
...
def subject_repo(tmp_path: Path) -> tuple[Path, str]:
    ...
    (repo / "rincode/agent").mkdir(parents=True)
    (repo / "rincode/agent/loop.py").write_text("x = 1\n")
...
class TestRunSpec:
    ...
    def test_work_dir_must_be_outside_subject_repo(self, tmp_path, subject_repo):
        repo, sha = subject_repo
        path = _write_spec(tmp_path, repo, sha, work_dir=str(repo / ".rincode/evolution"))
...
class TestModelIdentity:
    def test_default_model_provider_uses_rincode_identity(self, monkeypatch):
        from rincode.evolver.launch import models

        monkeypatch.setattr(models, "_rincode_default_model", lambda: "provider/model")
    ...
    def test_unknown_provider_name_is_rejected(self):
        from rincode.evolver.launch.models import build_call_fn
...
class TestWhitelistValidation:
    def test_live_prefix_passes(self, subject_repo):
        repo, sha = subject_repo
        validate_whitelist(repo, sha, ("rincode/agent/",))

    def test_exact_file_entry_passes_without_widening_to_prefix(self, subject_repo):
        repo, sha = subject_repo
        validate_whitelist(repo, sha, ("rincode/agent/loop.py",))
        with pytest.raises(ValueError, match="match no files"):
            validate_whitelist(repo, sha, ("rincode/agent/loop",))
...
class TestAppWorldEntry:
    def _ctx(self, tmp_path, subject_repo, bench_config, smoke=False):
        from rincode.evolver.launch.contract import LaunchContext
    ...
    def test_smoke_caps_train_and_sealed_test_sets(self, tmp_path, subject_repo, monkeypatch):
        ...
        import rincode.evolver.orchestrator.sealed.runner as sealed_mod
...
class TestScorerImmutability:
    def test_scorer_surface_is_immutable(self):
        from rincode.evolver.applier.path_guard import check_patch_paths
        ...
        offenders = check_patch_paths(
            [
                ...
                "rincode/evolver/orchestrator/gates/pipeline.py",
            ]
        )
...
class TestCli:
    def test_parser_subcommands(self):
        from rincode.evolver.cli import build_parser

        p = build_parser()
        assert p.prog == "rincode evolve"
    ...
    def test_status_on_fresh_dir_reports_not_started(self, tmp_path, subject_repo, capsys):
        from rincode.evolver.launch.runner import cmd_status
    ...
    def test_run_refuses_after_unseal(self, tmp_path, subject_repo, capsys):
        from rincode.evolver.launch.runner import cmd_run
