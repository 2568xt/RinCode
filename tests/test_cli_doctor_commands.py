...
from rincode.cli import doctor_commands
from rincode.cli.commands import app
from rincode.config.loader import save_config, set_config_path
...
def healthy_config(tmp_config: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("rincode.cli.tui_commands.resolve_dist_entry", lambda: tui_bundle)
    from rincode.cli._plugin_stack import MemoryBackendStatus
...
def test_doctor_default_on_missing_config_exit1(tmp_config: Path) -> None:
    ...
    assert "rincode onboard" in r.stdout
...
def test_doctor_missing_tui_bundle_exit1(healthy_config: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("rincode.cli.tui_commands.resolve_dist_entry", lambda: None)
