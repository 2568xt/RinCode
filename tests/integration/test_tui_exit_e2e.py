"""`rincode` commands that build the agent loop must not segfault on exit.
...
Fix: the CLI exit chokepoint ``rincode.cli.commands.run`` hard-exits past
finalization (flush stdio + loguru, then ``os._exit``) when
``rincode.cli._exit.lancedb_finalization_hazard`` reports the thread live. These
