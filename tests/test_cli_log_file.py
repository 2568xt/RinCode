...
Covers parameterization (filename / file_level / terminal_level / RINCODE_CLI_DEBUG)
...
def tmp_logs(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.delenv("RINCODE_CLI_DEBUG", raising=False)
...
def test_env_debug_mirrors_to_stderr(tmp_logs: Path, capsys, monkeypatch) -> None:
    ...
    monkeypatch.setenv("RINCODE_CLI_DEBUG", "1")
...
def test_strip_tty_stream_handlers_removes_root_stdout_handler(tmp_logs: Path) -> None:
    ...
    from rincode.cli._log_file import _strip_tty_stream_handlers
...
def test_redirect_terminal_fds_captures_print_and_raw_fd_write(tmp_path, capfd) -> None:
    ...
    from rincode.cli._log_file import redirect_terminal_fds_to_file
