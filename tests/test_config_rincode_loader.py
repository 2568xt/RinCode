"""Tests for ``load_rincode_config`` reading RinCode extension blocks
...
from rincode.config import rincode as ec_module
...
def stub_config_path(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("rincode.config.loader.get_config_path", _stub)
    monkeypatch.setattr("rincode.config.rincode.get_config_path", _stub)
    return p


def test_missing_config_falls_through_to_defaults(stub_config_path) -> None:

    cfg = ec_module.load_rincode_config()
...
def test_skill_forge_block_loaded_from_camel_case(stub_config_path: Path) -> None:
    """Match the format ``rincode onboard`` writes (camelCase via
