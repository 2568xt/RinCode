...
from benchmarks.rincodebench.packs.tokenwise_cost.replay import main, verify_legacy_report
from rincode.call_efficiency.pricing import estimate_cost_from_rates
...
def test_replay_reprices_trials_and_rebuilds_the_historical_claim(tmp_path: Path) -> None:
    ...
    assert replay["schema"] == "rincode.rincodebench.call-efficiency-replay.report.v1"
    assert replay["source_schema"] == "rincode.rincodebench.tokenwise-cost.report.v1"
...
def test_replay_cli_writes_bound_report_and_returns_equivalence_status(tmp_path: Path) -> None:
    ...
    assert result == 0
    assert written["schema"] == "rincode.rincodebench.call-efficiency-replay.report.v1"
