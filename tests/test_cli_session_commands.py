"""Tests for the ``rincode sessions`` CLI subapp."""
...
from rincode.cli.session_commands import (
...
def patched_workspace(workspace: Path, monkeypatch) -> Path:
    monkeypatch.setattr(
        "rincode.cli.session_commands._active_session_state",
        lambda: workspace,
    )
    return workspace
...
def test_session_commands_use_current_project_state(
    ...
    monkeypatch.setattr("rincode.cli.session_commands.load_config", lambda: Config())

    from rincode.cli.session_commands import _open_manager
...
def test_group_registered_as_sessions_not_session() -> None:
    """The group is mounted as ``rincode sessions``; the legacy ``rincode
...
def test_export_by_bare_id_writes_verified_portable_artifact(
    ...
    files = list((patched_workspace / "exports").glob("*.rincode-session.json"))
    assert len(files) == 1
    from rincode.session.export import verify_export
...
def test_export_custom_output_path(patched_workspace: Path, manager: SessionManager, tmp_path: Path) -> None:
    cid = "20990101_000000_bbbbbb"
    _seed(manager, f"cli:{cid}")
    dest = tmp_path / "custom" / "out.rincode-session.json"
...
def test_export_write_failure_exits_cleanly(patched_workspace: Path, manager: SessionManager, tmp_path: Path) -> None:
    ...
    dest = blocker / "out.rincode-session.json"
    r = runner.invoke(session_app, ["export", cid, "--output", str(dest)])
    assert r.exit_code != 0
