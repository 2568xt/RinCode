...
"""Tests for the bare ``rincode`` TUI after the Hermes UI shell fork.
...
from rincode.cli.tui_commands import _UI_TUI_DIR
...
def test_check_propagates_nonzero_child_exit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "rincode.cli.tui_commands.resolve_dist_entry",
        lambda: dist_entry,
    )
    monkeypatch.setattr(
        "rincode.cli.tui_commands.run_subprocess",
...
def test_dev_mode_uses_rpc_socket_post_fork(monkeypatch, tmp_path):
    ...
    plain run_subprocess. entry.tsx requires RINCODE_RPC_SOCKET,
...
def test_dev_check_mode_keeps_plain_spawn(monkeypatch, tmp_path):
    ...
    circuits on RINCODE_TUI_CHECK before the socket guard, so the smoke path
...
def test_check_sets_rincode_tui_check_env_var(monkeypatch):
    """`--check` must propagate RINCODE_TUI_CHECK=1 into the environment so
    ...
    monkeypatch.delenv("RINCODE_TUI_CHECK", raising=False)
    monkeypatch.setattr(
        "rincode.cli.tui_commands.find_node",
        lambda: ("/usr/bin/node", (22, 5, 0)),
    )
    ...
    result = runner.invoke(app, ["--check"])
    assert result.exit_code == 0, result.output
    assert captured_env.get("RINCODE_TUI_CHECK") == "1", (
        "--check must export RINCODE_TUI_CHECK=1 so the node child can take "
