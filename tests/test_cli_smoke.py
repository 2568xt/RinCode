...
def test_root_help_does_not_crash() -> None:
    """``rincode --help`` should list every command without crashing."""
...
def test_status_command_body_does_not_crash(tmp_config: Path) -> None:
    """``rincode status`` reads config + prints rows without crashing."""
...
def test_channels_list_body_does_not_crash(tmp_config: Path) -> None:
    """``rincode channels list`` enumerates channels without crashing."""
...
def test_cron_list_body_does_not_crash(tmp_config: Path) -> None:
    """``rincode cron list`` reads cron jobs without crashing."""
...
def test_version_flag_matches_installed_metadata() -> None:
    """``rincode --version`` reports the installed distribution version, not a
    ...
    assert r.exception is None
    assert r.exit_code == 0
    assert f"RinCode v{pkg_version('rincode-harness')}" in r.stdout
...
def test_cli_import_does_not_pull_litellm() -> None:
    ...
    r = subprocess.run(
        [sys.executable, "-c", "import rincode.cli.commands, sys; assert 'litellm' not in sys.modules"],
        capture_output=True,
        text=True,
    )
...
def test_no_logs_subcommand_registered() -> None:
    """There is no ``rincode logs`` command; adding one must break this test."""
