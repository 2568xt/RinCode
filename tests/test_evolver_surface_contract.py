...
from rincode.evolver.judge.prompts import JUDGE_SYSTEM_PROMPT, PROPOSABLE_WHERE
from rincode.evolver.judge.schema import PatchWhere
...
def test_candidate_prompt_uses_current_rincode_paths() -> None:
    assert "src/domains/" not in JUDGE_SYSTEM_PROMPT
    assert "memory_engine/everos/" not in JUDGE_SYSTEM_PROMPT
    assert "rincode/plugin/memory/everos/" not in JUDGE_SYSTEM_PROMPT
    assert "public backend contract" in JUDGE_SYSTEM_PROMPT
    for immutable_path in (
        "rincode/eval_engine/prompts/",
        "rincode/eval_engine/hooks/",
        "rincode/memory_engine/skills/",
