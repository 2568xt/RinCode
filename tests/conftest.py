...
def _restore_loguru_enabled_state():
    """Undo any ``loguru.logger.disable("rincode")`` left over from a
    prior test.

    ``rincode/cli/agent_commands.py`` toggles ``logger.disable("rincode")``
    ...
    silently dropping every ``rincode.*`` log emission and breaking
...
def _no_openrouter_network(tmp_path):
    ...
    file so the real ~/.rincode/cache/ is never read or written.
    """
    from rincode.token_wise import model_catalog_cache, pricing
