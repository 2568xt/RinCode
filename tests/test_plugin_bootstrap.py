...
class TestEndToEnd:
    ...
    def test_missing_selected_myna_has_install_remediation(
        ...
        from rincode.cli._plugin_stack import maybe_build_memory_backend
        from rincode.config.rincode import RinCodeConfig

        with pytest.raises(
            PluginNotFoundError,
            match=(
                r"install a RinCode-compatible Myna adapter from the myna-memory distribution"
                r".*myna init.*memory\.backend.*null"
            ),
        ):
    ...
    def test_memory_off_does_not_touch_plugin_registry(
        ...
        config = RinCodeConfig(memory=MemoryConfig(backend=None))
        assert (
            maybe_build_memory_backend(
                tmp_path,
                config,
