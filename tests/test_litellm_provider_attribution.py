...
from rincode.providers.litellm_provider import _ANTHROPIC_EXTRA_KEYS, LiteLLMProvider


def _make_provider(provider_name: str, extra_headers: dict | None = None) -> LiteLLMProvider:
    with (
        patch("rincode.providers.litellm_provider.litellm"),
        patch("rincode.providers.litellm_provider.LiteLLMProvider._setup_env"),
    ):
...
def test_openrouter_injects_all_attribution_headers():
    ...
    assert provider.extra_headers["X-Title"] == "RinCode Agent Harness"
    assert provider.extra_headers["X-OpenRouter-Title"] == "RinCode Agent Harness"
