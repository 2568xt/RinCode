"""Tests for the ``rincode cron`` CLI subapp."""
...
def fake_cron_dir(tmp_path: Path, monkeypatch) -> Path:
    """Redirect ~/.rincode/cron/ to a tmp dir for the test."""
    cron_dir = tmp_path / "cron"
    cron_dir.mkdir(parents=True)
    monkeypatch.setattr(
        "rincode.cli.cron_commands.get_cron_dir",
        lambda: cron_dir,
    )
    return cron_dir
...
def test_run_warns_when_active_claim_present(
    ...
    from rincode.proactive_engine.schedulers.cron.types import CronSchedule as _Sched
...
from rincode.cli.cron_commands import _parse_duration
...
def isolated_config(tmp_path: Path, monkeypatch) -> Path:
    ...
    monkeypatch.setattr(
        "rincode.config.loader.get_config_path",
        lambda: config_path,
    )
    monkeypatch.setattr(
        "rincode.config.update.get_config_path",
        lambda: config_path,
    )
    return config_path
