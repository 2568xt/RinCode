...
from benchmarks.rincodebench.semantic_campaign import (
...
async def test_semantic_run_requires_exact_paid_approval(
    ...
    monkeypatch.setattr(
        "benchmarks.rincodebench.semantic_campaign._load_embedding_settings",
        _provider_settings,
    )
    monkeypatch.delenv(
        "RINCODEBENCH_SEMANTIC_PAID_APPROVAL",
        raising=False,
    )
    monkeypatch.setattr(
        "benchmarks.rincodebench.semantic_campaign._git_output",
...
async def test_complete_existing_root_rebuilds_before_paid_approval(
    ...
    _complete_artifact_root(
        ...
        rincode_commit=preflight.rincode_commit,
...
def test_semantic_rebuild_is_stable_and_keeps_claims_separate(
    ...
    manifest = {
        "schema": "rincode.rincodebench.semantic-addendum.v1",
        "experiment_id": "p" * 64,
        "plan_digest": "p" * 64,
        "track": "calibration",
        "preflight": {
...
def test_semantic_v2_complete_artifact_rebuild_is_stable(
    ...
    assert first["schema"] == "rincode.rincodebench.semantic-report.v2"
...
def _empty_record(
    ...
    *,
    ...
    record: dict[str, object] = {
        "schema": f"rincode.rincodebench.semantic-retrieval-record.{version}",
        "plan_digest": "p" * 64,
        "key": {
...
def _complete_artifact_root(
    ...
    *,
    ...
    manifest = {
        "schema": f"rincode.rincodebench.semantic-addendum.{version}",
