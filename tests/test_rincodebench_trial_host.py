...
from benchmarks.rincodebench.usage import RecordingProvider, UsageRecorder
from benchmarks.rincodebench.verifier import (
    JsonArtifactVerifier,
    VerifierSeal,
    run_sealed_verifier,
)
from rincode.agent.loop.main import ProviderTurnError
...
from rincode.spine.events import Text, Usage
...
async def test_runtime_trial_host_builds_through_runtime_assembly(
    ...
    assembly = _Assembly()
    calls: list[tuple[Config, RinCodeConfig]] = []

    def _assemble(config, rincode_config, **kwargs):
        calls.append((config, rincode_config))
        return assembly

    monkeypatch.setattr("rincode.cli._runtime_assembly.assemble_runtime", _assemble)
    ...
    assert calls == [(config, rincode_config)]
