...
def test_run_help_works() -> None:
    """``rincode run --help`` lists the key options."""
...
def test_run_without_api_key_exits_cleanly(tmp_config: Path) -> None:
    ...
    """
    from rincode.config.loader import save_config
...
def _invoke_agent_capturing_session(
    ...
    *,
    ...
    from rincode.config.loader import save_config
    from rincode.config.schema import Config
    from rincode.spine import Text, TurnOutcome, Usage
    ...
    monkeypatch.setattr("rincode.cli.agent_commands.make_provider", lambda _: object())
    monkeypatch.setattr("rincode.agent.loop.AgentLoop", _StubAgentLoop)
...
def test_interactive_setup_failure_closes_runtime(
    ...
    monkeypatch.setattr("rincode.cli.agent_commands.make_provider", lambda cfg: object())
    monkeypatch.setattr("rincode.cli.agent_commands._init_prompt_session", lambda: None)
    monkeypatch.setattr("rincode.cli.agent_commands.signal.signal", lambda *args: None)
    ...
    monkeypatch.setattr("rincode.cli._repl_spine.build_repl", _fail_build)
...
def test_is_exit_command(command: str, expected: bool) -> None:
    ...
    from rincode.cli.agent_commands import _is_exit_command
...
def test_exit_commands_set_contents() -> None:
    ...
    from rincode.cli.agent_commands import EXIT_COMMANDS
...
def test_print_agent_response_with_markdown(capsys: pytest.CaptureFixture) -> None:
    ...
    from rincode.cli.agent_commands import _print_agent_response
...
def test_agent_message_mode_mocked_provider(tmp_config: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "rincode.cli.agent_commands.make_provider",
...
def isolated_runtime(tmp_path: Path, monkeypatch) -> Path:
    ...
    cfg = tmp_path / "config.json"
    set_config_path(cfg)
    monkeypatch.setattr("rincode.config.update.get_config_path", lambda: cfg)
    yield tmp_path
    set_config_path(None)  # type: ignore[arg-type]
...
def _make_cron_job():
    from rincode.config.paths import get_cron_dir
    from rincode.proactive_engine.schedulers.cron.service import CronService
    from rincode.proactive_engine.schedulers.cron.types import CronSchedule
