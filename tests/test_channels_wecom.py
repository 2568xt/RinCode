"""Tests for rincode.channels.adapters.wecom — frame/body parsing, per-type
...
from rincode.channels.adapters.wecom.channel import WecomChannel
...
def test_extract_image_downloads_and_labels(monkeypatch):
    import rincode.channels.adapters.wecom.channel as wecom_mod
...
def test_wecom_spec_declares_beta_maturity():
    from rincode.channels.adapters.wecom.spec import SPEC
...
def test_wecom_spec_factory_raises_import_error_without_sdk(monkeypatch):
    ...
    monkeypatch.delitem(sys.modules, "rincode.channels.adapters.wecom.channel", raising=False)
...
def test_wecom_spec_import_is_cheap():
    ...
    code = (
        "import sys, rincode.channels.adapters.wecom.spec as s;"
