...
RinCode 选择了一个当前工具环境中不存在的外部 Memory Backend。
...
```bash
rincode onboard --skip-memory --reset
```
...
```bash
export RINCODE_GITEE_TOKEN="<your-gitee-token>"
./install.sh
```

不要把 Token 放进远程 URL、仓库文件或故障截图。需要固定制品时，使用维护者提供并
经过校验的 `RINCODE_WHEEL_URL`。
...
```bash
rincode provider test <provider-name>
...
4. `rincode channels get feishu` 显示 enabled，App ID 正确，密钥已经设置。
5. `rincode gateway --workspace "$PWD" --verbose` 仍在运行。
...
```bash
rincode gateway --workspace /absolute/path/to/project --verbose
```
...
默认情况下，RinCode 会保护已有配置。自动化场景可以使用 `--yes` 复用；只有明确需要
重做配置时才使用 `--reset`。

```bash
rincode onboard --skip-memory --yes
rincode onboard --skip-memory --reset
```

如果不确定配置属于谁，先运行 `rincode doctor --json` 和 `rincode channels list`，不要
直接重置。
