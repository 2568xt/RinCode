...
目标：在飞书里给机器人发一条消息，由正在运行的 RinCode Gateway 返回真实回复。

RinCode 使用飞书 WebSocket 长连接，不需要公网 IP 或 webhook 域名。

## 1. 创建企业自建应用

1. 打开[飞书开放平台](https://open.feishu.cn/)，创建“企业自建应用”。
2. 在应用能力中启用“机器人”。
3. 在“凭证与基础信息”中复制 App ID 和 App Secret。App Secret 只应输入本地 RinCode 配置，不要发到群聊、Issue 或截图里。
...
可以在 `rincode onboard` 的第 4 步填写，也可以直接运行：

```bash
rincode channels enable feishu \
...
```bash
rincode channels show feishu
rincode channels get feishu
```

## 5. 启动 Gateway，发送第一条消息

Gateway 只处理它启动时选定的 Workspace。先进入想让 RinCode 工作的仓库：
...
看到 Gateway 日志中出现 inbound accepted，并在飞书里收到回复，才算通路成功。[发送消息 API](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/create) 是 RinCode 回复时使用的飞书能力。
...
```bash
rincode channels set feishu --allow-from ou_xxxxxxxxxxxxxxxx
```
