...
This RinCodeBench track measures the local Runtime tax of RinCode's in-tree Tracing.
...
once, and returns `TRACE_OK`. The sole treatment axis is `RINCODE_TRACING=0` versus
`RINCODE_TRACING=1`.
...
The immutable manifest binds the RinCode commit, Python and platform identity,
...
```bash
make rincodebench-tracing-plan
make rincodebench-tracing-run
make rincodebench-tracing-verify
```

Set `RINCODE_TRACING_OUTPUT` to use a new evidence root. After the repository moves
past the measured candidate, set `RINCODE_TRACING_COMMIT` to the full commit bound
