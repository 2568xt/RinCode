...
```bash
make rincodebench-runtime-scheduler
```

The command writes immutable evidence under
`.rincode/evidence/rincodebench-runtime-scheduler/`. Generated evidence remains
...
session, later sessions wait. The treatment is RinCode's session Lane scheduler
...
```bash
make rincodebench-runtime-live-plan
```

Then pass the exact approved manifest digest and amount:

```bash
RINCODE_LIVE_PERF_APPROVAL_DIGEST=<digest> \
RINCODE_LIVE_PERF_APPROVED_CNY=<amount> \
make rincodebench-runtime-live-run
```
...
```bash
RINCODE_LIVE_PERF_EVIDENCE=<run-artifact.json> \
make rincodebench-runtime-live-verify
```
