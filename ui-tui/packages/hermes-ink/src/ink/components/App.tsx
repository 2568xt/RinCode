...
const RINCODE_HOME = process.env.RINCODE_HOME?.trim() || join(homedir(), '.rincode')
  ...
  handleSetRawMode = (isEnabled: boolean): void => {
    ...
    if (isEnabled) {
      ...
      if (this.rawModeEnabledCount === 0) {
        ...
        // RINCODE_TUI_DEBUG_TERMINFO=1. Useful when modifier+enter behavior
        // regresses on a new terminal/transport stack — set the env var,
        // launch the TUI, then inspect $RINCODE_HOME/logs/tui-node-diag.log.
        if (process.env.RINCODE_TUI_DEBUG_TERMINFO === '1') {
          try {
            const diagPath = join(RINCODE_HOME, 'logs', 'tui-node-diag.log')
