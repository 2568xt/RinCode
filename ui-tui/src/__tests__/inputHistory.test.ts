...
let rincodeDir: string

beforeAll(async () => {
  rincodeDir = mkdtempSync(join(tmpdir(), 'rincode-hist-'))

  vi.stubEnv('RINCODE_HOME', rincodeDir)
...
describe('input history store (backs cycleHistory)', () => {
  ...
  it('appends entries in insertion order with newest last', () => {
    ...
    expect(existsSync(join(rincodeDir, '.rincode_history'))).toBe(true)
  })
