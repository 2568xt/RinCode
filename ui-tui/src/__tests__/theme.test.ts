...
// 会闭包捕获 DEFAULT_THEME。若开发者 shell 设置 RINCODE_TUI_THEME=light，或将
// RINCODE_TUI_BACKGROUND 设为亮色，基准主题会翻转，使这些断言只在本地失败。
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('RinCode')
...
describe('cursorColorHex (OSC 12 hardware cursor)', () => {
  ...
  it('uses the light title color across all tiers when the scheme is light', async () => {
    const { cursorColorHex, resolveTheme } = await importThemeWithEnv({ RINCODE_TUI_THEME: 'light' })
...
describe('detectLightMode', () => {
  ...
  it('honors RINCODE_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ RINCODE_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ RINCODE_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ RINCODE_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ RINCODE_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ RINCODE_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets RINCODE_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', RINCODE_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors RINCODE_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ RINCODE_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ RINCODE_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', RINCODE_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', RINCODE_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses RINCODE_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // 三位十六进制颜色按 CSS 规则归一化。
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: '#fff' })).toBe(true)
    // 无效内容回退到默认暗色路径。
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // 长度错误同样应被拒绝，不能隐式补齐或截断。
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ RINCODE_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('applyDetectedBackground (OSC 11 reply)', () => {
  ...
  it('caches the measured color into RINCODE_TUI_BACKGROUND', async () => {
    const { applyDetectedBackground } = await importThemeWithCleanEnv()

    applyDetectedBackground('rgb:eaea/eaea/eaea')

    expect(process.env.RINCODE_TUI_BACKGROUND).toBe('#eaeaea')
  })

  it('lets an explicit RINCODE_TUI_THEME override the measured background', async () => {
    const { applyDetectedBackground, currentScheme } = await importThemeWithEnv({ RINCODE_TUI_THEME: 'dark' })
...
describe('fromSkin', () => {
  ...
  // 否则外部 RINCODE_TUI_THEME=light 会翻转基准调色板，使断言结果依赖开发者 shell。
