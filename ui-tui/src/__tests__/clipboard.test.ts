...
describe('readClipboardImage', () => {
  ...
  it('cleans clipboard directories left by dead TUI processes', async () => {
    const root = await mkdtemp(join(tmpdir(), 'rincode-clipboard-cleanup-test-'))
    const dead = join(root, 'rincode-clipboard-12345-dead')
    const alive = join(root, 'rincode-clipboard-67890-alive')
...
describe('pasteClipboardImage', () => {
  it('routes a materialized clipboard image through image.attach', async () => {
    ...
    const readImage = vi.fn().mockResolvedValue('/tmp/rincode-clipboard-test/clipboard.png')
    ...
    expect(rpc).toHaveBeenCalledWith('image.attach', {
      path: '/tmp/rincode-clipboard-test/clipboard.png',
  ...
  it('discards an image when a Turn starts during clipboard materialization', async () => {
    ...
    const readImage = vi.fn().mockImplementation(async () => {
      ...
      return '/tmp/rincode-clipboard-test/clipboard.png'
    })
    ...
    expect(releaseImage).toHaveBeenCalledWith('/tmp/rincode-clipboard-test/clipboard.png')
    expect(rpc).not.toHaveBeenCalled()
  })
  ...
  it('discards a clipboard read when the active Session changes before attach', async () => {
    ...
    resolveRead('/tmp/rincode-clipboard-test/clipboard.png')
  ...
  it('releases the clipboard image when image.attach rejects', async () => {
    ...
    expect(retainImage).toHaveBeenCalledWith('/tmp/rincode-clipboard-test/clipboard.png', 'tui:active')
    expect(releaseImage).toHaveBeenCalledWith('/tmp/rincode-clipboard-test/clipboard.png')
  })
  ...
  it('releases a retained image when the Session changes during image.attach', async () => {
    ...
    const path = '/tmp/rincode-clipboard-test/clipboard.png'
    const releaseImage = vi.fn()
    const retainImage = vi.fn()
