...
import { rincodeLogo, rincodeLogoWord, RINCODE_WORD_WIDTH } from '../banner.js'
...
describe('StartupLoader', () => {
  it('renders a spinner frame and the first startup message', () => {
    ...
    expect(lastFrame()).toContain('starting rincode')
...
describe('banner wordmark', () => {
  ...
  it('rincodeLogo renders the 6-row block wordmark', () => {
    const lines = rincodeLogo(ramp)
    expect(lines.length).toBe(6)
    expect(lines.map(([, text]) => text).join('')).toContain('█')
  })

  it('rincodeLogoWord renders just RINCODE within one-word width', () => {
    const lines = rincodeLogoWord(ramp)
    expect(lines.length).toBe(6)
    expect(lines.map(([, text]) => text).join('')).toContain('█')
    const maxWidth = Math.max(...lines.map(([, text]) => [...text].length))
    expect(maxWidth).toBeLessThanOrEqual(RINCODE_WORD_WIDTH)
  })

  it('RINCODE word is compact enough for a standard terminal', () => {
    expect(RINCODE_WORD_WIDTH).toBeLessThanOrEqual(40)
  })
})
