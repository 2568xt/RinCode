...
import { getRinCodeHomeLabel } from '../config/paths.js'
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `rincode model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // OAuth 或其他认证类型无需处理，警告信息会提示用户运行 rincode model。
          return
        }
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted} wrap="truncate-end">
          Saved to {getRinCodeHomeLabel()}/.env{showBase ? ' · Tab switches field' : ''}
        </Text>
