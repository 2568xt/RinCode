...
export interface RpcClientOptions {
  /** RPC 目标："host:port"（TCP 回环）或 Unix 套接字路径，默认读取 `RINCODE_RPC_SOCKET`。 */
  socketPath?: string
...
export class RpcClient {
  ...
  constructor(opts: RpcClientOptions = {}) {
    const target = opts.socketPath ?? process.env.RINCODE_RPC_SOCKET
    if (!target) {
      throw new Error('RpcClient: no RPC target supplied; pass `socketPath` or set ' + 'RINCODE_RPC_SOCKET env var.')
    }
    ...
    const authToken = process.env.RINCODE_RPC_TOKEN
