...
//
// 两者均由 RINCODE_DEV_PERF=1 启用，并输出 JSON Lines（默认 ~/.rincode/perf.log，
// 可用 RINCODE_DEV_PERF_LOG 覆盖）。通过 { src: 'react' | 'frame' } 标记供 jq
// 使用。RINCODE_DEV_PERF_MS 默认为 2，会跳过低于阈值的空闲帧；设为 0 可全部捕获。
//
...
import { getRinCodeHome } from '../config/paths.js'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.RINCODE_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.RINCODE_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.RINCODE_DEV_PERF_LOG?.trim() || join(getRinCodeHome(), 'perf.log')
