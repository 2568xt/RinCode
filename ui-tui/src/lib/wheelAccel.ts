...
/** RINCODE_TUI_SCROLL_SPEED（也可用 CLAUDE_CODE_SCROLL_SPEED 保持兼容），
 *  默认值为 1，并限制在 (0, 20]。 */
export function readScrollSpeedBase(): number {
  const n = parseFloat(process.env.RINCODE_TUI_SCROLL_SPEED ?? process.env.CLAUDE_CODE_SCROLL_SPEED ?? '')
