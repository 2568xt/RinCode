// `rincode --print-colors`（平面色块）与 `--preview-colors`（在真实界面上下文中
// 展示 token）所用的诊断颜色渲染器。
//
...
export function renderColorSwatches(theme: Theme, tier: 0 | 1 | 2 | 3): string {
  ...
  const lines: string[] = []
  lines.push(`RinCode TUI palette — tier ${tier} (${TIER_NAMES[tier] ?? 'unknown'})`)
  lines.push('')
  ...
  lines.push('')
  lines.push('Force a tier to compare:  rincode --color <truecolor|256|16|none> --print-colors')
...
export function renderColorPreview(theme: Theme, tier: 0 | 1 | 2 | 3): string {
  ...
  out.push(`RinCode TUI color usage preview — tier ${tier} (${TIER_NAMES[tier] ?? 'unknown'})`)
  ...
  out.push('')
  out.push(fg('Force a tier:  rincode --color <truecolor|256|16|none> --preview-colors', c.muted))
