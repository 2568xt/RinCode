 ...
 *
 * 配置渠道：
 *   - `RINCODE_TUI_COLOR` = auto | truecolor | 256 | 16 | none（`--color`
 *     参数会转发到这里）；
 *   - `RINCODE_TUI_TRUECOLOR` = 1/true/...，是
 *     `RINCODE_TUI_COLOR=truecolor` 的旧版别名。
 *
 * 优先级从高到低为 NO_COLOR、RINCODE_TUI_COLOR、旧版 RINCODE_TUI_TRUECOLOR、
...
/**
 * 从 RinCode 颜色环境变量解析请求的级别。返回强制级别；“auto”则返回 `null`，
 * 交由 Chalk 探测。
 */
export function parseColorOverride(env: NodeJS.ProcessEnv = process.env): ColorTier | null {
  const raw = (env.RINCODE_TUI_COLOR ?? '').trim().toLowerCase()
  ...
  // 兼容旧别名：RINCODE_TUI_TRUECOLOR=1 等价于 RINCODE_TUI_COLOR=truecolor。
  if (TRUE_RE.test((env.RINCODE_TUI_TRUECOLOR ?? '').trim())) {
    return 3
  }
