...
// ── 品牌黄色色阶（渐变标志/立体阴影）────────────────────────────────
//
// 品牌资产（rincode-tui-design-system 的“品牌色阶”），按从亮到暗排序，用于渐变横幅图案。
...
// 又很常见，因此无法检测时保持暗色，除非 RINCODE_TUI_THEME、RINCODE_TUI_LIGHT、
// RINCODE_TUI_BACKGROUND 或 COLORFGBG 明确表示浅色。仍允许注入，供测试验证优先级规则。
const LIGHT_DEFAULT_TERM_PROGRAMS = new Set<string>([])

// 尽力进行 RGB 到亮度检查。目前只接受 3 位或 6 位十六进制值，可带或不带前导 `#`。
// 环境变量名 `RINCODE_TUI_BACKGROUND` 刻意保持通用，未来 OSC11 查询辅助方法也可在其中缓存结果；
...
//
//   1. `RINCODE_TUI_LIGHT` 布尔值：`1`/`true`/`yes`/`on` 表示浅色，
//      `0`/`false`/`no`/`off` 表示暗色；任一显式值都优先于后续信号。
//   2. `RINCODE_TUI_THEME` 具名覆盖：`light` / `dark` 优先于下方所有信号。
//   3. `RINCODE_TUI_BACKGROUND` 的 3 位或 6 位十六进制提示：亮度不低于阈值时为浅色。
...
export function detectLightMode(
  ...
  const themeFlag = (env.RINCODE_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.RINCODE_TUI_BACKGROUND ?? '')
 ...
 *
 * 将解析后的颜色缓存到 RINCODE_TUI_BACKGROUND，再次运行 detectLightMode()，使
 * 现有优先级规则保持不变；显式 RINCODE_TUI_THEME/RINCODE_TUI_LIGHT 仍优先于实测
...
export function applyDetectedBackground(oscData: string): { changed: boolean; scheme: ColorScheme } | null {
  ...
  process.env.RINCODE_TUI_BACKGROUND = hex
