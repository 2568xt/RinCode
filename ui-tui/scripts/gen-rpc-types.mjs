...
//
// RinCode method-scoped types + components/schemas + JSON-RPC 2.0 envelopes.
...
async function generate() {
  ...
  const compiled = await compile(root, 'RinCodeRpcRoot', {
    bannerComment: '',
  ...
  const withoutRoot = compiled
    .replace(/export interface RinCodeRpcRoot\s*\{[\s\S]*?\n\}\n?/, '')
    .trim();
  ...
  // 检测规范名称：
  //   /**
  //    * This interface was referenced by `RinCodeRpcRoot`'s JSON-Schema
