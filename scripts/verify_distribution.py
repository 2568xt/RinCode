...
_REQUIRED_FILES = {
    "rincode/__init__.py",
    "rincode/templates/AGENTS.md",
    "rincode/templates/SOUL.md",
...
def _validate_wheel(snapshot: WheelSnapshot, entrypoint: str) -> None:
    ...
    tui_files = {name for name in names if name.startswith("rincode/ui-tui/")}
    expected_tui = {"rincode/ui-tui/dist/entry.js"}
    ...
    removed_everos_runtime = sorted(
        name
        for name in names
        if name.startswith("rincode/plugin/memory/everos/")
        or name
        in {
            "rincode/config/update_everos.py",
            "rincode/memory_engine/skill_forge/everos_source.py",
            "rincode/tracing/viewer/everos-deposits.js",
        }
    )
    ...
    if metadata.get("Name", "").lower() != "rincode-harness":
        raise VerificationError("wheel METADATA does not describe the rincode-harness distribution")
    ...
    expected_target = "rincode.cli.commands:run"
...
def _is_removed_channel_path(name: str) -> bool:
    ...
    return any(
        candidate[:1] == ("bridge",)
        or (
            candidate[:3] == ("rincode", "channels", "adapters")
            and len(candidate) > 3
...
def _is_removed_media_generation_path(name: str) -> bool:
    parts = PurePosixPath(name).parts
    return ("rincode", "agent", "tools", "media_gen.py") in (parts, parts[1:])
...
def _is_removed_skill_hub_path(name: str) -> bool:
    ...
    candidates = (parts, parts[1:])
    return any(
        candidate[:2] == ("rincode", "skill_hub")
        or candidate == ("rincode", "agent", "tools", "skill_hub.py")
        or candidate == ("rincode", "memory_engine", "skill_forge", "hub_source.py")
        for candidate in candidates
    )
...
def _is_removed_sentinel_path(name: str) -> bool:
    ...
    candidates = (parts, parts[1:])
    return any(
        candidate[:3] == ("rincode", "proactive_engine", "sentinel")
        or candidate
        in {
            ("rincode", "cli", "sentinel_commands.py"),
            ("rincode", "cli", "_proactive_stack.py"),
            ("rincode", "memory_engine", "consolidate", "attention.py"),
            ("rincode", "memory_engine", "consolidate", "behaviors.py"),
...
def _is_removed_heartbeat_path(name: str) -> bool:
    ...
    candidates = (parts, parts[1:])
    return any(
        candidate[:4] == ("rincode", "proactive_engine", "schedulers", "heartbeat")
        or candidate
        in {
...
def _is_removed_cli_path(name: str) -> bool:
    parts = PurePosixPath(name).parts
    return ("rincode", "cli", "upgrade_commands.py") in (parts, parts[1:])
...
def _validate_doctor_probe(completed: object, expected_config_path: Path) -> dict:
    ...
    if paths.get("config_path") != str(expected_config_path):
        raise VerificationError("doctor probe did not report the isolated RinCode config path")
    return report
...
def _validate_package_probe(completed: subprocess.CompletedProcess[str], plan: InstallPlan) -> dict:
    ...
    if root not in package.parents:
        raise VerificationError(f"{plan.name} imported RinCode's internal package outside its environment: {package}")
...
def _validate_rincode_module_probe(completed: subprocess.CompletedProcess[str], plan: InstallPlan) -> None:
    if completed.returncode != 0 or "RinCode v" not in completed.stdout:
        raise VerificationError(f"{plan.name} cannot run the RinCode module entrypoint")
...
def _validate_product_paths(
    ...
    *,
    ...
    expected = {
        "product_home": probe_home / ".rincode",
        "workspace_state": probe_home / ".rincode" / "projects" / f"{slug}-{digest}",
        "plugin_user": probe_home / ".rincode" / "plugins",
    }
    mismatches = {
    ...
    if report.get("plugin_entrypoints") != "rincode.plugins":
        mismatches["plugin_entrypoints"] = {
            "expected": "rincode.plugins",
            "actual": report.get("plugin_entrypoints"),
        }
    if mismatches:
        raise VerificationError(f"installed RinCode state roots drifted: {mismatches}")
...
def _install_environment(
    ...
    *,
    ...
    package_code = (
        "import json,rincode,sys,typer; "
        "from rincode.cli._plugin_stack import plugin_discovery_sources; "
        "from rincode.cli.commands import app; "
        "from rincode.cli.tui_commands import resolve_dist_entry; "
        "from rincode.product import get_product_home,get_project_state_dir; "
        "root=typer.main.get_command(app); "
        "entry=resolve_dist_entry(); "
        "sources=plugin_discovery_sources(); "
        "print(json.dumps({'package': rincode.__file__, 'tui': str(entry) if entry else '', "
    ...
    rincode_module = _run(
        [str(python), "-I", "-m", "rincode", "--version"],
        cwd=probe_cwd,
        env=probe_env,
        log_dir=log_dir,
        records=records,
    )
    _validate_rincode_module_probe(rincode_module, plan)
    _run(
        [str(executable), "--help"],
        cwd=probe_cwd,
        env=probe_env,
    ...
    doctor_report = _validate_doctor_probe(
        doctor,
        probe_home / ".rincode" / "config.json",
    )
    _run(
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build and verify an isolated RinCode distribution.")
