...
RinCode includes code from the upstream agent runtime maintained by htxoffical,
with maintenance changes and publication documentation by 2568xt. The upstream
notices below are retained as provenance. Copyright notices and license terms
are unchanged.

RinCode and its upstream runtime are licensed under the Apache License 2.0.
The runtime incorporates code from the
...
- Scope: forked at v0.1.5.post3 and modified throughout the corresponding
  `rincode/` Runtime packages, including agent, bus, channels, CLI, config, cron,
...
- Scope: the imported TUI remains under `ui-tui/`, including the vendored
  `@hermes/ink` fork. RinCode modifications include product branding, RinCode
...
- Scope: hermes-agent ships its own fork of community ink at
  `ui-tui/packages/hermes-ink/`. RinCode inherits this
...
## PinchBench (benchmark tasks and fixtures)

- Source: https://github.com/pinchbench/skill
- Copyright (c) 2026 PinchBench
- License: MIT — see `LICENSES/MIT-pinchbench.txt`.
- Scope: benchmark tasks and assets under `benchmarks/pinchbench/`, including
  `company_expenses.xlsx` and `quarterly_sales.csv`.

# External Runtime Tools (not vendored)

The following tools are invoked by RinCode via `subprocess` calls but are
**not bundled or redistributed** as part of any RinCode release artifact.
...
- Scope: invoked by `tests/tui/autotest/runner.py::Harness` for PTY-driven
  ...
  (2026-05-20) — all 5 acceptance gates S1-S5 passed. RinCode does NOT
